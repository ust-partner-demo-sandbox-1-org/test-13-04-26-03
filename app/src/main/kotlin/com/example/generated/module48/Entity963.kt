...
interface Entity963Repository : org.springframework.data.jpa.repository.JpaRepository<Entity963, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity963>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity963>
}
...
class Service963(private val repo: Entity963Repository) {
    ...
    fun findActive(): List<Entity963> = repo.findByField4(true)
    fun search(query: String): List<Entity963> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity963>): Map<String, List<Entity963>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity963): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity963")
class Controller963(private val service: Service963) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity963) = service.save(entity)
    @DeleteMapping("/{id}")
