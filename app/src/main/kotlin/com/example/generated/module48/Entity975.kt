...
interface Entity975Repository : org.springframework.data.jpa.repository.JpaRepository<Entity975, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity975>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity975>
}
...
class Service975(private val repo: Entity975Repository) {
    ...
    fun findActive(): List<Entity975> = repo.findByField4(true)
    fun search(query: String): List<Entity975> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity975>): Map<String, List<Entity975>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity975): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity975")
class Controller975(private val service: Service975) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity975) = service.save(entity)
    @DeleteMapping("/{id}")
