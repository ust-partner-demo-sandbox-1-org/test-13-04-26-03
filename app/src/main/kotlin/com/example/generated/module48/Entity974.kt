...
interface Entity974Repository : org.springframework.data.jpa.repository.JpaRepository<Entity974, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity974>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity974>
}
...
class Service974(private val repo: Entity974Repository) {
    ...
    fun findActive(): List<Entity974> = repo.findByField4(true)
    fun search(query: String): List<Entity974> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity974>): Map<String, List<Entity974>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity974): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity974")
class Controller974(private val service: Service974) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity974) = service.save(entity)
    @DeleteMapping("/{id}")
