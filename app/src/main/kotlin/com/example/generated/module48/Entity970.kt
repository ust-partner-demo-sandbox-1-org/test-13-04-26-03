...
interface Entity970Repository : org.springframework.data.jpa.repository.JpaRepository<Entity970, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity970>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity970>
}
...
class Service970(private val repo: Entity970Repository) {
    ...
    fun findActive(): List<Entity970> = repo.findByField4(true)
    fun search(query: String): List<Entity970> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity970>): Map<String, List<Entity970>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity970): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity970")
class Controller970(private val service: Service970) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity970) = service.save(entity)
    @DeleteMapping("/{id}")
