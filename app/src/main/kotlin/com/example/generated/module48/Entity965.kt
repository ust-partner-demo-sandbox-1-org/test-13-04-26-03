...
interface Entity965Repository : org.springframework.data.jpa.repository.JpaRepository<Entity965, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity965>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity965>
}
...
class Service965(private val repo: Entity965Repository) {
    ...
    fun findActive(): List<Entity965> = repo.findByField4(true)
    fun search(query: String): List<Entity965> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity965>): Map<String, List<Entity965>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity965): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity965")
class Controller965(private val service: Service965) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity965) = service.save(entity)
    @DeleteMapping("/{id}")
