...
interface Entity977Repository : org.springframework.data.jpa.repository.JpaRepository<Entity977, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity977>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity977>
}
...
class Service977(private val repo: Entity977Repository) {
    ...
    fun findActive(): List<Entity977> = repo.findByField4(true)
    fun search(query: String): List<Entity977> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity977>): Map<String, List<Entity977>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity977): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity977")
class Controller977(private val service: Service977) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity977) = service.save(entity)
    @DeleteMapping("/{id}")
