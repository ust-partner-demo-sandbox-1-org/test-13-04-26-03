...
interface Entity964Repository : org.springframework.data.jpa.repository.JpaRepository<Entity964, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity964>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity964>
}
...
class Service964(private val repo: Entity964Repository) {
    ...
    fun findActive(): List<Entity964> = repo.findByField4(true)
    fun search(query: String): List<Entity964> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity964>): Map<String, List<Entity964>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity964): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity964")
class Controller964(private val service: Service964) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity964) = service.save(entity)
    @DeleteMapping("/{id}")
