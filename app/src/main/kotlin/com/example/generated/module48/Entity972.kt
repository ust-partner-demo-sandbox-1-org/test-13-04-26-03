...
interface Entity972Repository : org.springframework.data.jpa.repository.JpaRepository<Entity972, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity972>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity972>
}
...
class Service972(private val repo: Entity972Repository) {
    ...
    fun findActive(): List<Entity972> = repo.findByField4(true)
    fun search(query: String): List<Entity972> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity972>): Map<String, List<Entity972>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity972): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity972")
class Controller972(private val service: Service972) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity972) = service.save(entity)
    @DeleteMapping("/{id}")
