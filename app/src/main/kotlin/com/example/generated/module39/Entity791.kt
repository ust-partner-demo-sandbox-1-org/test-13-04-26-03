...
interface Entity791Repository : org.springframework.data.jpa.repository.JpaRepository<Entity791, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity791>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity791>
}
...
class Service791(private val repo: Entity791Repository) {
    ...
    fun findActive(): List<Entity791> = repo.findByField4(true)
    fun search(query: String): List<Entity791> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity791>): Map<String, List<Entity791>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity791): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity791")
class Controller791(private val service: Service791) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity791) = service.save(entity)
    @DeleteMapping("/{id}")
