...
interface Entity797Repository : org.springframework.data.jpa.repository.JpaRepository<Entity797, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity797>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity797>
}
...
class Service797(private val repo: Entity797Repository) {
    ...
    fun findActive(): List<Entity797> = repo.findByField4(true)
    fun search(query: String): List<Entity797> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity797>): Map<String, List<Entity797>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity797): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity797")
class Controller797(private val service: Service797) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity797) = service.save(entity)
    @DeleteMapping("/{id}")
