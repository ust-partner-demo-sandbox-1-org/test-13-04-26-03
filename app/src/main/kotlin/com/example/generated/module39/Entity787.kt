...
interface Entity787Repository : org.springframework.data.jpa.repository.JpaRepository<Entity787, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity787>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity787>
}
...
class Service787(private val repo: Entity787Repository) {
    ...
    fun findActive(): List<Entity787> = repo.findByField4(true)
    fun search(query: String): List<Entity787> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity787>): Map<String, List<Entity787>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity787): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity787")
class Controller787(private val service: Service787) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity787) = service.save(entity)
    @DeleteMapping("/{id}")
