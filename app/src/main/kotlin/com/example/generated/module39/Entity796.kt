...
interface Entity796Repository : org.springframework.data.jpa.repository.JpaRepository<Entity796, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity796>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity796>
}
...
class Service796(private val repo: Entity796Repository) {
    ...
    fun findActive(): List<Entity796> = repo.findByField4(true)
    fun search(query: String): List<Entity796> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity796>): Map<String, List<Entity796>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity796): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity796")
class Controller796(private val service: Service796) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity796) = service.save(entity)
    @DeleteMapping("/{id}")
