...
interface Entity793Repository : org.springframework.data.jpa.repository.JpaRepository<Entity793, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity793>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity793>
}
...
class Service793(private val repo: Entity793Repository) {
    ...
    fun findActive(): List<Entity793> = repo.findByField4(true)
    fun search(query: String): List<Entity793> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity793>): Map<String, List<Entity793>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity793): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity793")
class Controller793(private val service: Service793) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity793) = service.save(entity)
    @DeleteMapping("/{id}")
