...
interface Entity789Repository : org.springframework.data.jpa.repository.JpaRepository<Entity789, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity789>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity789>
}
...
class Service789(private val repo: Entity789Repository) {
    ...
    fun findActive(): List<Entity789> = repo.findByField4(true)
    fun search(query: String): List<Entity789> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity789>): Map<String, List<Entity789>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity789): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity789")
class Controller789(private val service: Service789) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity789) = service.save(entity)
    @DeleteMapping("/{id}")
