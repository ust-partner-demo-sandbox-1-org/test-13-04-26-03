...
interface Entity785Repository : org.springframework.data.jpa.repository.JpaRepository<Entity785, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity785>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity785>
}
...
class Service785(private val repo: Entity785Repository) {
    ...
    fun findActive(): List<Entity785> = repo.findByField4(true)
    fun search(query: String): List<Entity785> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity785>): Map<String, List<Entity785>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity785): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity785")
class Controller785(private val service: Service785) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity785) = service.save(entity)
    @DeleteMapping("/{id}")
