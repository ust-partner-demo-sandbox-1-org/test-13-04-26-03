...
interface Entity786Repository : org.springframework.data.jpa.repository.JpaRepository<Entity786, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity786>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity786>
}
...
class Service786(private val repo: Entity786Repository) {
    ...
    fun findActive(): List<Entity786> = repo.findByField4(true)
    fun search(query: String): List<Entity786> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity786>): Map<String, List<Entity786>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity786): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity786")
class Controller786(private val service: Service786) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity786) = service.save(entity)
    @DeleteMapping("/{id}")
