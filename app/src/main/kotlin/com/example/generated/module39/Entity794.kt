...
interface Entity794Repository : org.springframework.data.jpa.repository.JpaRepository<Entity794, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity794>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity794>
}
...
class Service794(private val repo: Entity794Repository) {
    ...
    fun findActive(): List<Entity794> = repo.findByField4(true)
    fun search(query: String): List<Entity794> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity794>): Map<String, List<Entity794>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity794): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity794")
class Controller794(private val service: Service794) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity794) = service.save(entity)
    @DeleteMapping("/{id}")
