...
interface Entity928Repository : org.springframework.data.jpa.repository.JpaRepository<Entity928, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity928>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity928>
}
...
class Service928(private val repo: Entity928Repository) {
    ...
    fun findActive(): List<Entity928> = repo.findByField4(true)
    fun search(query: String): List<Entity928> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity928>): Map<String, List<Entity928>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity928): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity928")
class Controller928(private val service: Service928) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity928) = service.save(entity)
    @DeleteMapping("/{id}")
