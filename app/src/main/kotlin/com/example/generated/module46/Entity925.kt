...
interface Entity925Repository : org.springframework.data.jpa.repository.JpaRepository<Entity925, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity925>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity925>
}
...
class Service925(private val repo: Entity925Repository) {
    ...
    fun findActive(): List<Entity925> = repo.findByField4(true)
    fun search(query: String): List<Entity925> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity925>): Map<String, List<Entity925>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity925): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity925")
class Controller925(private val service: Service925) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity925) = service.save(entity)
    @DeleteMapping("/{id}")
