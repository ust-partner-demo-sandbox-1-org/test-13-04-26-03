...
interface Entity939Repository : org.springframework.data.jpa.repository.JpaRepository<Entity939, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity939>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity939>
}
...
class Service939(private val repo: Entity939Repository) {
    ...
    fun findActive(): List<Entity939> = repo.findByField4(true)
    fun search(query: String): List<Entity939> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity939>): Map<String, List<Entity939>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity939): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity939")
class Controller939(private val service: Service939) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity939) = service.save(entity)
    @DeleteMapping("/{id}")
