...
interface Entity934Repository : org.springframework.data.jpa.repository.JpaRepository<Entity934, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity934>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity934>
}
...
class Service934(private val repo: Entity934Repository) {
    ...
    fun findActive(): List<Entity934> = repo.findByField4(true)
    fun search(query: String): List<Entity934> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity934>): Map<String, List<Entity934>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity934): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity934")
class Controller934(private val service: Service934) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity934) = service.save(entity)
    @DeleteMapping("/{id}")
