...
interface Entity931Repository : org.springframework.data.jpa.repository.JpaRepository<Entity931, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity931>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity931>
}
...
class Service931(private val repo: Entity931Repository) {
    ...
    fun findActive(): List<Entity931> = repo.findByField4(true)
    fun search(query: String): List<Entity931> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity931>): Map<String, List<Entity931>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity931): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity931")
class Controller931(private val service: Service931) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity931) = service.save(entity)
    @DeleteMapping("/{id}")
