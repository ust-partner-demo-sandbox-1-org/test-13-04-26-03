...
interface Entity936Repository : org.springframework.data.jpa.repository.JpaRepository<Entity936, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity936>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity936>
}
...
class Service936(private val repo: Entity936Repository) {
    ...
    fun findActive(): List<Entity936> = repo.findByField4(true)
    fun search(query: String): List<Entity936> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity936>): Map<String, List<Entity936>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity936): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity936")
class Controller936(private val service: Service936) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity936) = service.save(entity)
    @DeleteMapping("/{id}")
