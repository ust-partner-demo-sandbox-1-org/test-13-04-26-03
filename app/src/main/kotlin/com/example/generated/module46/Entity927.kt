...
interface Entity927Repository : org.springframework.data.jpa.repository.JpaRepository<Entity927, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity927>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity927>
}
...
class Service927(private val repo: Entity927Repository) {
    ...
    fun findActive(): List<Entity927> = repo.findByField4(true)
    fun search(query: String): List<Entity927> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity927>): Map<String, List<Entity927>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity927): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity927")
class Controller927(private val service: Service927) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity927) = service.save(entity)
    @DeleteMapping("/{id}")
