...
interface Entity924Repository : org.springframework.data.jpa.repository.JpaRepository<Entity924, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity924>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity924>
}
...
class Service924(private val repo: Entity924Repository) {
    ...
    fun findActive(): List<Entity924> = repo.findByField4(true)
    fun search(query: String): List<Entity924> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity924>): Map<String, List<Entity924>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity924): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity924")
class Controller924(private val service: Service924) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity924) = service.save(entity)
    @DeleteMapping("/{id}")
