...
interface Entity285Repository : org.springframework.data.jpa.repository.JpaRepository<Entity285, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity285>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity285>
}
...
class Service285(private val repo: Entity285Repository) {
    ...
    fun findActive(): List<Entity285> = repo.findByField4(true)
    fun search(query: String): List<Entity285> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity285>): Map<String, List<Entity285>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity285): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity285")
class Controller285(private val service: Service285) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity285) = service.save(entity)
    @DeleteMapping("/{id}")
