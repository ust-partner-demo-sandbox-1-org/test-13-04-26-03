...
interface Entity297Repository : org.springframework.data.jpa.repository.JpaRepository<Entity297, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity297>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity297>
}
...
class Service297(private val repo: Entity297Repository) {
    ...
    fun findActive(): List<Entity297> = repo.findByField4(true)
    fun search(query: String): List<Entity297> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity297>): Map<String, List<Entity297>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity297): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity297")
class Controller297(private val service: Service297) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity297) = service.save(entity)
    @DeleteMapping("/{id}")
