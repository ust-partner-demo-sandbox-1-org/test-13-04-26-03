...
interface Entity294Repository : org.springframework.data.jpa.repository.JpaRepository<Entity294, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity294>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity294>
}
...
class Service294(private val repo: Entity294Repository) {
    ...
    fun findActive(): List<Entity294> = repo.findByField4(true)
    fun search(query: String): List<Entity294> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity294>): Map<String, List<Entity294>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity294): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity294")
class Controller294(private val service: Service294) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity294) = service.save(entity)
    @DeleteMapping("/{id}")
