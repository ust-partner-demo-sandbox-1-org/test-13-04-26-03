...
interface Entity284Repository : org.springframework.data.jpa.repository.JpaRepository<Entity284, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity284>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity284>
}
...
class Service284(private val repo: Entity284Repository) {
    ...
    fun findActive(): List<Entity284> = repo.findByField4(true)
    fun search(query: String): List<Entity284> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity284>): Map<String, List<Entity284>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity284): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity284")
class Controller284(private val service: Service284) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity284) = service.save(entity)
    @DeleteMapping("/{id}")
