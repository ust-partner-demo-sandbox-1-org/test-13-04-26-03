...
interface Entity295Repository : org.springframework.data.jpa.repository.JpaRepository<Entity295, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity295>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity295>
}
...
class Service295(private val repo: Entity295Repository) {
    ...
    fun findActive(): List<Entity295> = repo.findByField4(true)
    fun search(query: String): List<Entity295> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity295>): Map<String, List<Entity295>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity295): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity295")
class Controller295(private val service: Service295) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity295) = service.save(entity)
    @DeleteMapping("/{id}")
