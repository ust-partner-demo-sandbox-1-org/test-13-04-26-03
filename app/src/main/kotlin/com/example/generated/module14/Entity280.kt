...
interface Entity280Repository : org.springframework.data.jpa.repository.JpaRepository<Entity280, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity280>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity280>
}
...
class Service280(private val repo: Entity280Repository) {
    ...
    fun findActive(): List<Entity280> = repo.findByField4(true)
    fun search(query: String): List<Entity280> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity280>): Map<String, List<Entity280>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity280): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity280")
class Controller280(private val service: Service280) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity280) = service.save(entity)
    @DeleteMapping("/{id}")
