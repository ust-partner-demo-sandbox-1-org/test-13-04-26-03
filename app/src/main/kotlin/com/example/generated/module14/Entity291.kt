...
interface Entity291Repository : org.springframework.data.jpa.repository.JpaRepository<Entity291, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity291>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity291>
}
...
class Service291(private val repo: Entity291Repository) {
    ...
    fun findActive(): List<Entity291> = repo.findByField4(true)
    fun search(query: String): List<Entity291> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity291>): Map<String, List<Entity291>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity291): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity291")
class Controller291(private val service: Service291) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity291) = service.save(entity)
    @DeleteMapping("/{id}")
