...
interface Entity287Repository : org.springframework.data.jpa.repository.JpaRepository<Entity287, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity287>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity287>
}
...
class Service287(private val repo: Entity287Repository) {
    ...
    fun findActive(): List<Entity287> = repo.findByField4(true)
    fun search(query: String): List<Entity287> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity287>): Map<String, List<Entity287>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity287): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity287")
class Controller287(private val service: Service287) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity287) = service.save(entity)
    @DeleteMapping("/{id}")
