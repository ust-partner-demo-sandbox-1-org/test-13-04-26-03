...
interface Entity282Repository : org.springframework.data.jpa.repository.JpaRepository<Entity282, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity282>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity282>
}
...
class Service282(private val repo: Entity282Repository) {
    ...
    fun findActive(): List<Entity282> = repo.findByField4(true)
    fun search(query: String): List<Entity282> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity282>): Map<String, List<Entity282>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity282): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity282")
class Controller282(private val service: Service282) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity282) = service.save(entity)
    @DeleteMapping("/{id}")
