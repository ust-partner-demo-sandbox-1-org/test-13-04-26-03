...
interface Entity288Repository : org.springframework.data.jpa.repository.JpaRepository<Entity288, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity288>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity288>
}
...
class Service288(private val repo: Entity288Repository) {
    ...
    fun findActive(): List<Entity288> = repo.findByField4(true)
    fun search(query: String): List<Entity288> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity288>): Map<String, List<Entity288>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity288): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity288")
class Controller288(private val service: Service288) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity288) = service.save(entity)
    @DeleteMapping("/{id}")
