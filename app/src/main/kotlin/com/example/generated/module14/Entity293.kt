...
interface Entity293Repository : org.springframework.data.jpa.repository.JpaRepository<Entity293, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity293>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity293>
}
...
class Service293(private val repo: Entity293Repository) {
    ...
    fun findActive(): List<Entity293> = repo.findByField4(true)
    fun search(query: String): List<Entity293> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity293>): Map<String, List<Entity293>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity293): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity293")
class Controller293(private val service: Service293) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity293) = service.save(entity)
    @DeleteMapping("/{id}")
