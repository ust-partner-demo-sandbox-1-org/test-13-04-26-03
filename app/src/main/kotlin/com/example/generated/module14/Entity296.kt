...
interface Entity296Repository : org.springframework.data.jpa.repository.JpaRepository<Entity296, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity296>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity296>
}
...
class Service296(private val repo: Entity296Repository) {
    ...
    fun findActive(): List<Entity296> = repo.findByField4(true)
    fun search(query: String): List<Entity296> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity296>): Map<String, List<Entity296>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity296): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity296")
class Controller296(private val service: Service296) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity296) = service.save(entity)
    @DeleteMapping("/{id}")
