...
interface Entity943Repository : org.springframework.data.jpa.repository.JpaRepository<Entity943, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity943>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity943>
}
...
class Service943(private val repo: Entity943Repository) {
    ...
    fun findActive(): List<Entity943> = repo.findByField4(true)
    fun search(query: String): List<Entity943> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity943>): Map<String, List<Entity943>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity943): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity943")
class Controller943(private val service: Service943) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity943) = service.save(entity)
    @DeleteMapping("/{id}")
