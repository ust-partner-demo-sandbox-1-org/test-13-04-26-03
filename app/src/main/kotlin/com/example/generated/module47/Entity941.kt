...
interface Entity941Repository : org.springframework.data.jpa.repository.JpaRepository<Entity941, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity941>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity941>
}
...
class Service941(private val repo: Entity941Repository) {
    ...
    fun findActive(): List<Entity941> = repo.findByField4(true)
    fun search(query: String): List<Entity941> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity941>): Map<String, List<Entity941>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity941): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity941")
class Controller941(private val service: Service941) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity941) = service.save(entity)
    @DeleteMapping("/{id}")
