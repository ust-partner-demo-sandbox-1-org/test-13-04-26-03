...
interface Entity948Repository : org.springframework.data.jpa.repository.JpaRepository<Entity948, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity948>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity948>
}
...
class Service948(private val repo: Entity948Repository) {
    ...
    fun findActive(): List<Entity948> = repo.findByField4(true)
    fun search(query: String): List<Entity948> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity948>): Map<String, List<Entity948>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity948): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity948")
class Controller948(private val service: Service948) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity948) = service.save(entity)
    @DeleteMapping("/{id}")
