...
interface Entity947Repository : org.springframework.data.jpa.repository.JpaRepository<Entity947, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity947>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity947>
}
...
class Service947(private val repo: Entity947Repository) {
    ...
    fun findActive(): List<Entity947> = repo.findByField4(true)
    fun search(query: String): List<Entity947> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity947>): Map<String, List<Entity947>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity947): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity947")
class Controller947(private val service: Service947) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity947) = service.save(entity)
    @DeleteMapping("/{id}")
