...
interface Entity952Repository : org.springframework.data.jpa.repository.JpaRepository<Entity952, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity952>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity952>
}
...
class Service952(private val repo: Entity952Repository) {
    ...
    fun findActive(): List<Entity952> = repo.findByField4(true)
    fun search(query: String): List<Entity952> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity952>): Map<String, List<Entity952>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity952): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity952")
class Controller952(private val service: Service952) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity952) = service.save(entity)
    @DeleteMapping("/{id}")
