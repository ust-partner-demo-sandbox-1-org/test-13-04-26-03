...
interface Entity955Repository : org.springframework.data.jpa.repository.JpaRepository<Entity955, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity955>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity955>
}
...
class Service955(private val repo: Entity955Repository) {
    ...
    fun findActive(): List<Entity955> = repo.findByField4(true)
    fun search(query: String): List<Entity955> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity955>): Map<String, List<Entity955>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity955): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity955")
class Controller955(private val service: Service955) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity955) = service.save(entity)
    @DeleteMapping("/{id}")
