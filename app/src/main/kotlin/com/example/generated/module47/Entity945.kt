...
interface Entity945Repository : org.springframework.data.jpa.repository.JpaRepository<Entity945, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity945>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity945>
}
...
class Service945(private val repo: Entity945Repository) {
    ...
    fun findActive(): List<Entity945> = repo.findByField4(true)
    fun search(query: String): List<Entity945> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity945>): Map<String, List<Entity945>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity945): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity945")
class Controller945(private val service: Service945) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity945) = service.save(entity)
    @DeleteMapping("/{id}")
