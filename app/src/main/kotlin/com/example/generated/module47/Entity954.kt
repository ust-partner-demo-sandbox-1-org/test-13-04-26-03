...
interface Entity954Repository : org.springframework.data.jpa.repository.JpaRepository<Entity954, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity954>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity954>
}
...
class Service954(private val repo: Entity954Repository) {
    ...
    fun findActive(): List<Entity954> = repo.findByField4(true)
    fun search(query: String): List<Entity954> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity954>): Map<String, List<Entity954>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity954): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity954")
class Controller954(private val service: Service954) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity954) = service.save(entity)
    @DeleteMapping("/{id}")
