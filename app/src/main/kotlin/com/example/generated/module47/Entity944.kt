...
interface Entity944Repository : org.springframework.data.jpa.repository.JpaRepository<Entity944, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity944>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity944>
}
...
class Service944(private val repo: Entity944Repository) {
    ...
    fun findActive(): List<Entity944> = repo.findByField4(true)
    fun search(query: String): List<Entity944> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity944>): Map<String, List<Entity944>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity944): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity944")
class Controller944(private val service: Service944) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity944) = service.save(entity)
    @DeleteMapping("/{id}")
