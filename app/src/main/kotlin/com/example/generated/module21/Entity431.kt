...
interface Entity431Repository : org.springframework.data.jpa.repository.JpaRepository<Entity431, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity431>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity431>
}
...
class Service431(private val repo: Entity431Repository) {
    ...
    fun findActive(): List<Entity431> = repo.findByField4(true)
    fun search(query: String): List<Entity431> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity431>): Map<String, List<Entity431>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity431): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity431")
class Controller431(private val service: Service431) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity431) = service.save(entity)
    @DeleteMapping("/{id}")
