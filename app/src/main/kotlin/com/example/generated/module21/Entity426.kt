...
interface Entity426Repository : org.springframework.data.jpa.repository.JpaRepository<Entity426, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity426>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity426>
}
...
class Service426(private val repo: Entity426Repository) {
    ...
    fun findActive(): List<Entity426> = repo.findByField4(true)
    fun search(query: String): List<Entity426> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity426>): Map<String, List<Entity426>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity426): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity426")
class Controller426(private val service: Service426) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity426) = service.save(entity)
    @DeleteMapping("/{id}")
