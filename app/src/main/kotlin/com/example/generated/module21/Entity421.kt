...
interface Entity421Repository : org.springframework.data.jpa.repository.JpaRepository<Entity421, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity421>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity421>
}
...
class Service421(private val repo: Entity421Repository) {
    ...
    fun findActive(): List<Entity421> = repo.findByField4(true)
    fun search(query: String): List<Entity421> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity421>): Map<String, List<Entity421>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity421): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity421")
class Controller421(private val service: Service421) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity421) = service.save(entity)
    @DeleteMapping("/{id}")
