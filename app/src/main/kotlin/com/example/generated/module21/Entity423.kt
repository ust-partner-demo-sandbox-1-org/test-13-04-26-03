...
interface Entity423Repository : org.springframework.data.jpa.repository.JpaRepository<Entity423, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity423>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity423>
}
...
class Service423(private val repo: Entity423Repository) {
    ...
    fun findActive(): List<Entity423> = repo.findByField4(true)
    fun search(query: String): List<Entity423> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity423>): Map<String, List<Entity423>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity423): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity423")
class Controller423(private val service: Service423) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity423) = service.save(entity)
    @DeleteMapping("/{id}")
