...
interface Entity438Repository : org.springframework.data.jpa.repository.JpaRepository<Entity438, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity438>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity438>
}
...
class Service438(private val repo: Entity438Repository) {
    ...
    fun findActive(): List<Entity438> = repo.findByField4(true)
    fun search(query: String): List<Entity438> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity438>): Map<String, List<Entity438>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity438): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity438")
class Controller438(private val service: Service438) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity438) = service.save(entity)
    @DeleteMapping("/{id}")
