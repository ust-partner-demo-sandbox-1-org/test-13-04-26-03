...
interface Entity422Repository : org.springframework.data.jpa.repository.JpaRepository<Entity422, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity422>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity422>
}
...
class Service422(private val repo: Entity422Repository) {
    ...
    fun findActive(): List<Entity422> = repo.findByField4(true)
    fun search(query: String): List<Entity422> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity422>): Map<String, List<Entity422>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity422): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity422")
class Controller422(private val service: Service422) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity422) = service.save(entity)
    @DeleteMapping("/{id}")
