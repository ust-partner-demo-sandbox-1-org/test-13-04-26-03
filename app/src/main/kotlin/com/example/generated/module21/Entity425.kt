...
interface Entity425Repository : org.springframework.data.jpa.repository.JpaRepository<Entity425, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity425>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity425>
}
...
class Service425(private val repo: Entity425Repository) {
    ...
    fun findActive(): List<Entity425> = repo.findByField4(true)
    fun search(query: String): List<Entity425> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity425>): Map<String, List<Entity425>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity425): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity425")
class Controller425(private val service: Service425) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity425) = service.save(entity)
    @DeleteMapping("/{id}")
