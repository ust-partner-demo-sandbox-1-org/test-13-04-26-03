...
interface Entity435Repository : org.springframework.data.jpa.repository.JpaRepository<Entity435, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity435>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity435>
}
...
class Service435(private val repo: Entity435Repository) {
    ...
    fun findActive(): List<Entity435> = repo.findByField4(true)
    fun search(query: String): List<Entity435> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity435>): Map<String, List<Entity435>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity435): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity435")
class Controller435(private val service: Service435) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity435) = service.save(entity)
    @DeleteMapping("/{id}")
