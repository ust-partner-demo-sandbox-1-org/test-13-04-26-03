...
interface Entity429Repository : org.springframework.data.jpa.repository.JpaRepository<Entity429, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity429>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity429>
}
...
class Service429(private val repo: Entity429Repository) {
    ...
    fun findActive(): List<Entity429> = repo.findByField4(true)
    fun search(query: String): List<Entity429> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity429>): Map<String, List<Entity429>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity429): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity429")
class Controller429(private val service: Service429) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity429) = service.save(entity)
    @DeleteMapping("/{id}")
