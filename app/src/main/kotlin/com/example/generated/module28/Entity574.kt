...
interface Entity574Repository : org.springframework.data.jpa.repository.JpaRepository<Entity574, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity574>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity574>
}
...
class Service574(private val repo: Entity574Repository) {
    ...
    fun findActive(): List<Entity574> = repo.findByField4(true)
    fun search(query: String): List<Entity574> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity574>): Map<String, List<Entity574>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity574): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity574")
class Controller574(private val service: Service574) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity574) = service.save(entity)
    @DeleteMapping("/{id}")
