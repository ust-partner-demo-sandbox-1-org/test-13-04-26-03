...
interface Entity576Repository : org.springframework.data.jpa.repository.JpaRepository<Entity576, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity576>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity576>
}
...
class Service576(private val repo: Entity576Repository) {
    ...
    fun findActive(): List<Entity576> = repo.findByField4(true)
    fun search(query: String): List<Entity576> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity576>): Map<String, List<Entity576>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity576): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity576")
class Controller576(private val service: Service576) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity576) = service.save(entity)
    @DeleteMapping("/{id}")
