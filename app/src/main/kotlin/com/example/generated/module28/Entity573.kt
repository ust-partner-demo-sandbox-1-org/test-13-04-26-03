...
interface Entity573Repository : org.springframework.data.jpa.repository.JpaRepository<Entity573, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity573>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity573>
}
...
class Service573(private val repo: Entity573Repository) {
    ...
    fun findActive(): List<Entity573> = repo.findByField4(true)
    fun search(query: String): List<Entity573> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity573>): Map<String, List<Entity573>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity573): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity573")
class Controller573(private val service: Service573) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity573) = service.save(entity)
    @DeleteMapping("/{id}")
