...
interface Entity571Repository : org.springframework.data.jpa.repository.JpaRepository<Entity571, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity571>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity571>
}
...
class Service571(private val repo: Entity571Repository) {
    ...
    fun findActive(): List<Entity571> = repo.findByField4(true)
    fun search(query: String): List<Entity571> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity571>): Map<String, List<Entity571>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity571): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity571")
class Controller571(private val service: Service571) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity571) = service.save(entity)
    @DeleteMapping("/{id}")
