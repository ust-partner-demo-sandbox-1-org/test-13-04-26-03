...
interface Entity575Repository : org.springframework.data.jpa.repository.JpaRepository<Entity575, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity575>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity575>
}
...
class Service575(private val repo: Entity575Repository) {
    ...
    fun findActive(): List<Entity575> = repo.findByField4(true)
    fun search(query: String): List<Entity575> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity575>): Map<String, List<Entity575>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity575): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity575")
class Controller575(private val service: Service575) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity575) = service.save(entity)
    @DeleteMapping("/{id}")
