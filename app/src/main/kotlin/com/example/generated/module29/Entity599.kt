...
interface Entity599Repository : org.springframework.data.jpa.repository.JpaRepository<Entity599, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity599>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity599>
}
...
class Service599(private val repo: Entity599Repository) {
    ...
    fun findActive(): List<Entity599> = repo.findByField4(true)
    fun search(query: String): List<Entity599> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity599>): Map<String, List<Entity599>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity599): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity599")
class Controller599(private val service: Service599) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity599) = service.save(entity)
    @DeleteMapping("/{id}")
