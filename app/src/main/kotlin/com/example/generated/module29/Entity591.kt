...
interface Entity591Repository : org.springframework.data.jpa.repository.JpaRepository<Entity591, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity591>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity591>
}
...
class Service591(private val repo: Entity591Repository) {
    ...
    fun findActive(): List<Entity591> = repo.findByField4(true)
    fun search(query: String): List<Entity591> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity591>): Map<String, List<Entity591>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity591): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity591")
class Controller591(private val service: Service591) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity591) = service.save(entity)
    @DeleteMapping("/{id}")
