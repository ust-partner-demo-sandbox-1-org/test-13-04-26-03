...
interface Entity596Repository : org.springframework.data.jpa.repository.JpaRepository<Entity596, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity596>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity596>
}
...
class Service596(private val repo: Entity596Repository) {
    ...
    fun findActive(): List<Entity596> = repo.findByField4(true)
    fun search(query: String): List<Entity596> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity596>): Map<String, List<Entity596>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity596): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity596")
class Controller596(private val service: Service596) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity596) = service.save(entity)
    @DeleteMapping("/{id}")
