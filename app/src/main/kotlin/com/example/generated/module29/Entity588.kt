...
interface Entity588Repository : org.springframework.data.jpa.repository.JpaRepository<Entity588, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity588>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity588>
}
...
class Service588(private val repo: Entity588Repository) {
    ...
    fun findActive(): List<Entity588> = repo.findByField4(true)
    fun search(query: String): List<Entity588> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity588>): Map<String, List<Entity588>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity588): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity588")
class Controller588(private val service: Service588) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity588) = service.save(entity)
    @DeleteMapping("/{id}")
