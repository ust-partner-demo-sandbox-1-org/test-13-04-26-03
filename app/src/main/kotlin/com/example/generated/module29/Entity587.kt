...
interface Entity587Repository : org.springframework.data.jpa.repository.JpaRepository<Entity587, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity587>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity587>
}
...
class Service587(private val repo: Entity587Repository) {
    ...
    fun findActive(): List<Entity587> = repo.findByField4(true)
    fun search(query: String): List<Entity587> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity587>): Map<String, List<Entity587>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity587): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity587")
class Controller587(private val service: Service587) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity587) = service.save(entity)
    @DeleteMapping("/{id}")
