...
interface Entity595Repository : org.springframework.data.jpa.repository.JpaRepository<Entity595, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity595>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity595>
}
...
class Service595(private val repo: Entity595Repository) {
    ...
    fun findActive(): List<Entity595> = repo.findByField4(true)
    fun search(query: String): List<Entity595> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity595>): Map<String, List<Entity595>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity595): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity595")
class Controller595(private val service: Service595) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity595) = service.save(entity)
    @DeleteMapping("/{id}")
