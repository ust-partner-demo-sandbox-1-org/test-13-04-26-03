...
interface Entity589Repository : org.springframework.data.jpa.repository.JpaRepository<Entity589, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity589>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity589>
}
...
class Service589(private val repo: Entity589Repository) {
    ...
    fun findActive(): List<Entity589> = repo.findByField4(true)
    fun search(query: String): List<Entity589> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity589>): Map<String, List<Entity589>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity589): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity589")
class Controller589(private val service: Service589) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity589) = service.save(entity)
    @DeleteMapping("/{id}")
