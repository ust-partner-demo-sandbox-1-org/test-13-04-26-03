...
interface Entity582Repository : org.springframework.data.jpa.repository.JpaRepository<Entity582, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity582>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity582>
}
...
class Service582(private val repo: Entity582Repository) {
    ...
    fun findActive(): List<Entity582> = repo.findByField4(true)
    fun search(query: String): List<Entity582> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity582>): Map<String, List<Entity582>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity582): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity582")
class Controller582(private val service: Service582) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity582) = service.save(entity)
    @DeleteMapping("/{id}")
