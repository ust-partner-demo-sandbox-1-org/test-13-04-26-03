...
interface Entity581Repository : org.springframework.data.jpa.repository.JpaRepository<Entity581, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity581>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity581>
}
...
class Service581(private val repo: Entity581Repository) {
    ...
    fun findActive(): List<Entity581> = repo.findByField4(true)
    fun search(query: String): List<Entity581> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity581>): Map<String, List<Entity581>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity581): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity581")
class Controller581(private val service: Service581) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity581) = service.save(entity)
    @DeleteMapping("/{id}")
