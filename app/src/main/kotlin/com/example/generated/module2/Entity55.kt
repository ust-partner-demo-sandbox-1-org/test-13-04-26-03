...
interface Entity55Repository : org.springframework.data.jpa.repository.JpaRepository<Entity55, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity55>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity55>
}
...
class Service55(private val repo: Entity55Repository) {
    ...
    fun findActive(): List<Entity55> = repo.findByField4(true)
    fun search(query: String): List<Entity55> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity55>): Map<String, List<Entity55>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity55): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity55")
class Controller55(private val service: Service55) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity55) = service.save(entity)
    @DeleteMapping("/{id}")
