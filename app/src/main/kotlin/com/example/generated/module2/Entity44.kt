...
interface Entity44Repository : org.springframework.data.jpa.repository.JpaRepository<Entity44, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity44>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity44>
}
...
class Service44(private val repo: Entity44Repository) {
    ...
    fun findActive(): List<Entity44> = repo.findByField4(true)
    fun search(query: String): List<Entity44> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity44>): Map<String, List<Entity44>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity44): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity44")
class Controller44(private val service: Service44) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity44) = service.save(entity)
    @DeleteMapping("/{id}")
