...
interface Entity43Repository : org.springframework.data.jpa.repository.JpaRepository<Entity43, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity43>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity43>
}
...
class Service43(private val repo: Entity43Repository) {
    ...
    fun findActive(): List<Entity43> = repo.findByField4(true)
    fun search(query: String): List<Entity43> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity43>): Map<String, List<Entity43>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity43): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity43")
class Controller43(private val service: Service43) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity43) = service.save(entity)
    @DeleteMapping("/{id}")
