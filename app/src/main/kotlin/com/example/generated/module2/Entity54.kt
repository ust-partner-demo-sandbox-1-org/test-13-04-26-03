...
interface Entity54Repository : org.springframework.data.jpa.repository.JpaRepository<Entity54, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity54>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity54>
}
...
class Service54(private val repo: Entity54Repository) {
    ...
    fun findActive(): List<Entity54> = repo.findByField4(true)
    fun search(query: String): List<Entity54> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity54>): Map<String, List<Entity54>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity54): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity54")
class Controller54(private val service: Service54) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity54) = service.save(entity)
    @DeleteMapping("/{id}")
