...
interface Entity51Repository : org.springframework.data.jpa.repository.JpaRepository<Entity51, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity51>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity51>
}
...
class Service51(private val repo: Entity51Repository) {
    ...
    fun findActive(): List<Entity51> = repo.findByField4(true)
    fun search(query: String): List<Entity51> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity51>): Map<String, List<Entity51>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity51): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity51")
class Controller51(private val service: Service51) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity51) = service.save(entity)
    @DeleteMapping("/{id}")
