...
interface Entity59Repository : org.springframework.data.jpa.repository.JpaRepository<Entity59, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity59>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity59>
}
...
class Service59(private val repo: Entity59Repository) {
    ...
    fun findActive(): List<Entity59> = repo.findByField4(true)
    fun search(query: String): List<Entity59> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity59>): Map<String, List<Entity59>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity59): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity59")
class Controller59(private val service: Service59) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity59) = service.save(entity)
    @DeleteMapping("/{id}")
