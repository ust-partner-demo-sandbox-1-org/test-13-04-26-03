...
interface Entity45Repository : org.springframework.data.jpa.repository.JpaRepository<Entity45, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity45>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity45>
}
...
class Service45(private val repo: Entity45Repository) {
    ...
    fun findActive(): List<Entity45> = repo.findByField4(true)
    fun search(query: String): List<Entity45> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity45>): Map<String, List<Entity45>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity45): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity45")
class Controller45(private val service: Service45) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity45) = service.save(entity)
    @DeleteMapping("/{id}")
