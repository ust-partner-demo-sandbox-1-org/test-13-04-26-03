...
interface Entity46Repository : org.springframework.data.jpa.repository.JpaRepository<Entity46, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity46>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity46>
}
...
class Service46(private val repo: Entity46Repository) {
    ...
    fun findActive(): List<Entity46> = repo.findByField4(true)
    fun search(query: String): List<Entity46> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity46>): Map<String, List<Entity46>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity46): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity46")
class Controller46(private val service: Service46) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity46) = service.save(entity)
    @DeleteMapping("/{id}")
