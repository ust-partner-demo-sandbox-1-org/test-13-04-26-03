...
interface Entity49Repository : org.springframework.data.jpa.repository.JpaRepository<Entity49, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity49>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity49>
}
...
class Service49(private val repo: Entity49Repository) {
    ...
    fun findActive(): List<Entity49> = repo.findByField4(true)
    fun search(query: String): List<Entity49> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity49>): Map<String, List<Entity49>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity49): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity49")
class Controller49(private val service: Service49) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity49) = service.save(entity)
    @DeleteMapping("/{id}")
