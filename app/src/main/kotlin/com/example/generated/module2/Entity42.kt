...
interface Entity42Repository : org.springframework.data.jpa.repository.JpaRepository<Entity42, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity42>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity42>
}
...
class Service42(private val repo: Entity42Repository) {
    ...
    fun findActive(): List<Entity42> = repo.findByField4(true)
    fun search(query: String): List<Entity42> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity42>): Map<String, List<Entity42>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity42): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity42")
class Controller42(private val service: Service42) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity42) = service.save(entity)
    @DeleteMapping("/{id}")
