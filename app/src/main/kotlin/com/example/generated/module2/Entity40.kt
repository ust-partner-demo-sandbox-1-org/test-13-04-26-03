...
interface Entity40Repository : org.springframework.data.jpa.repository.JpaRepository<Entity40, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity40>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity40>
}
...
class Service40(private val repo: Entity40Repository) {
    ...
    fun findActive(): List<Entity40> = repo.findByField4(true)
    fun search(query: String): List<Entity40> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity40>): Map<String, List<Entity40>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity40): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity40")
class Controller40(private val service: Service40) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity40) = service.save(entity)
    @DeleteMapping("/{id}")
