...
interface Entity56Repository : org.springframework.data.jpa.repository.JpaRepository<Entity56, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity56>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity56>
}
...
class Service56(private val repo: Entity56Repository) {
    ...
    fun findActive(): List<Entity56> = repo.findByField4(true)
    fun search(query: String): List<Entity56> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity56>): Map<String, List<Entity56>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity56): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity56")
class Controller56(private val service: Service56) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity56) = service.save(entity)
    @DeleteMapping("/{id}")
