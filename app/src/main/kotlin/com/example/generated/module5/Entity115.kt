...
interface Entity115Repository : org.springframework.data.jpa.repository.JpaRepository<Entity115, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity115>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity115>
}
...
class Service115(private val repo: Entity115Repository) {
    ...
    fun findActive(): List<Entity115> = repo.findByField4(true)
    fun search(query: String): List<Entity115> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity115>): Map<String, List<Entity115>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity115): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity115")
class Controller115(private val service: Service115) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity115) = service.save(entity)
    @DeleteMapping("/{id}")
