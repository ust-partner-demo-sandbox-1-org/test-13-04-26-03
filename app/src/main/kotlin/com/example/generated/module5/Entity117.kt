...
interface Entity117Repository : org.springframework.data.jpa.repository.JpaRepository<Entity117, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity117>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity117>
}
...
class Service117(private val repo: Entity117Repository) {
    ...
    fun findActive(): List<Entity117> = repo.findByField4(true)
    fun search(query: String): List<Entity117> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity117>): Map<String, List<Entity117>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity117): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity117")
class Controller117(private val service: Service117) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity117) = service.save(entity)
    @DeleteMapping("/{id}")
