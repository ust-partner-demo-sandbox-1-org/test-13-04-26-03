...
interface Entity107Repository : org.springframework.data.jpa.repository.JpaRepository<Entity107, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity107>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity107>
}
...
class Service107(private val repo: Entity107Repository) {
    ...
    fun findActive(): List<Entity107> = repo.findByField4(true)
    fun search(query: String): List<Entity107> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity107>): Map<String, List<Entity107>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity107): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity107")
class Controller107(private val service: Service107) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity107) = service.save(entity)
    @DeleteMapping("/{id}")
