...
interface Entity109Repository : org.springframework.data.jpa.repository.JpaRepository<Entity109, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity109>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity109>
}
...
class Service109(private val repo: Entity109Repository) {
    ...
    fun findActive(): List<Entity109> = repo.findByField4(true)
    fun search(query: String): List<Entity109> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity109>): Map<String, List<Entity109>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity109): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity109")
class Controller109(private val service: Service109) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity109) = service.save(entity)
    @DeleteMapping("/{id}")
