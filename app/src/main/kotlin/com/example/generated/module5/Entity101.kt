...
interface Entity101Repository : org.springframework.data.jpa.repository.JpaRepository<Entity101, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity101>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity101>
}
...
class Service101(private val repo: Entity101Repository) {
    ...
    fun findActive(): List<Entity101> = repo.findByField4(true)
    fun search(query: String): List<Entity101> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity101>): Map<String, List<Entity101>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity101): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity101")
class Controller101(private val service: Service101) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity101) = service.save(entity)
    @DeleteMapping("/{id}")
