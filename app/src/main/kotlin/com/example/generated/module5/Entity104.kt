...
interface Entity104Repository : org.springframework.data.jpa.repository.JpaRepository<Entity104, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity104>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity104>
}
...
class Service104(private val repo: Entity104Repository) {
    ...
    fun findActive(): List<Entity104> = repo.findByField4(true)
    fun search(query: String): List<Entity104> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity104>): Map<String, List<Entity104>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity104): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity104")
class Controller104(private val service: Service104) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity104) = service.save(entity)
    @DeleteMapping("/{id}")
