...
interface Entity116Repository : org.springframework.data.jpa.repository.JpaRepository<Entity116, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity116>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity116>
}
...
class Service116(private val repo: Entity116Repository) {
    ...
    fun findActive(): List<Entity116> = repo.findByField4(true)
    fun search(query: String): List<Entity116> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity116>): Map<String, List<Entity116>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity116): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity116")
class Controller116(private val service: Service116) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity116) = service.save(entity)
    @DeleteMapping("/{id}")
