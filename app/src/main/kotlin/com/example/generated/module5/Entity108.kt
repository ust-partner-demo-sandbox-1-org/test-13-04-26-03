...
interface Entity108Repository : org.springframework.data.jpa.repository.JpaRepository<Entity108, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity108>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity108>
}
...
class Service108(private val repo: Entity108Repository) {
    ...
    fun findActive(): List<Entity108> = repo.findByField4(true)
    fun search(query: String): List<Entity108> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity108>): Map<String, List<Entity108>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity108): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity108")
class Controller108(private val service: Service108) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity108) = service.save(entity)
    @DeleteMapping("/{id}")
