...
interface Entity102Repository : org.springframework.data.jpa.repository.JpaRepository<Entity102, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity102>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity102>
}
...
class Service102(private val repo: Entity102Repository) {
    ...
    fun findActive(): List<Entity102> = repo.findByField4(true)
    fun search(query: String): List<Entity102> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity102>): Map<String, List<Entity102>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity102): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity102")
class Controller102(private val service: Service102) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity102) = service.save(entity)
    @DeleteMapping("/{id}")
