...
interface Entity804Repository : org.springframework.data.jpa.repository.JpaRepository<Entity804, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity804>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity804>
}
...
class Service804(private val repo: Entity804Repository) {
    ...
    fun findActive(): List<Entity804> = repo.findByField4(true)
    fun search(query: String): List<Entity804> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity804>): Map<String, List<Entity804>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity804): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity804")
class Controller804(private val service: Service804) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity804) = service.save(entity)
    @DeleteMapping("/{id}")
