...
interface Entity813Repository : org.springframework.data.jpa.repository.JpaRepository<Entity813, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity813>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity813>
}
...
class Service813(private val repo: Entity813Repository) {
    ...
    fun findActive(): List<Entity813> = repo.findByField4(true)
    fun search(query: String): List<Entity813> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity813>): Map<String, List<Entity813>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity813): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity813")
class Controller813(private val service: Service813) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity813) = service.save(entity)
    @DeleteMapping("/{id}")
