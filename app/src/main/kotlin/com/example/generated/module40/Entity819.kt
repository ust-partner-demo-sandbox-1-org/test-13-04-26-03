...
interface Entity819Repository : org.springframework.data.jpa.repository.JpaRepository<Entity819, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity819>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity819>
}
...
class Service819(private val repo: Entity819Repository) {
    ...
    fun findActive(): List<Entity819> = repo.findByField4(true)
    fun search(query: String): List<Entity819> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity819>): Map<String, List<Entity819>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity819): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity819")
class Controller819(private val service: Service819) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity819) = service.save(entity)
    @DeleteMapping("/{id}")
