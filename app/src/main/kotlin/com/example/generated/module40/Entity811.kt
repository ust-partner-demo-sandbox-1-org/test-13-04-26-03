...
interface Entity811Repository : org.springframework.data.jpa.repository.JpaRepository<Entity811, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity811>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity811>
}
...
class Service811(private val repo: Entity811Repository) {
    ...
    fun findActive(): List<Entity811> = repo.findByField4(true)
    fun search(query: String): List<Entity811> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity811>): Map<String, List<Entity811>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity811): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity811")
class Controller811(private val service: Service811) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity811) = service.save(entity)
    @DeleteMapping("/{id}")
