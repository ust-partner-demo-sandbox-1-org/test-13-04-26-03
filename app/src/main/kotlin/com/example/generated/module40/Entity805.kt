...
interface Entity805Repository : org.springframework.data.jpa.repository.JpaRepository<Entity805, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity805>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity805>
}
...
class Service805(private val repo: Entity805Repository) {
    ...
    fun findActive(): List<Entity805> = repo.findByField4(true)
    fun search(query: String): List<Entity805> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity805>): Map<String, List<Entity805>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity805): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity805")
class Controller805(private val service: Service805) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity805) = service.save(entity)
    @DeleteMapping("/{id}")
