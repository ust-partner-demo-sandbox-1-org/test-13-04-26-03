...
interface Entity806Repository : org.springframework.data.jpa.repository.JpaRepository<Entity806, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity806>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity806>
}
...
class Service806(private val repo: Entity806Repository) {
    ...
    fun findActive(): List<Entity806> = repo.findByField4(true)
    fun search(query: String): List<Entity806> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity806>): Map<String, List<Entity806>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity806): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity806")
class Controller806(private val service: Service806) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity806) = service.save(entity)
    @DeleteMapping("/{id}")
