...
interface Entity812Repository : org.springframework.data.jpa.repository.JpaRepository<Entity812, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity812>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity812>
}
...
class Service812(private val repo: Entity812Repository) {
    ...
    fun findActive(): List<Entity812> = repo.findByField4(true)
    fun search(query: String): List<Entity812> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity812>): Map<String, List<Entity812>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity812): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity812")
class Controller812(private val service: Service812) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity812) = service.save(entity)
    @DeleteMapping("/{id}")
