...
interface Entity807Repository : org.springframework.data.jpa.repository.JpaRepository<Entity807, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity807>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity807>
}
...
class Service807(private val repo: Entity807Repository) {
    ...
    fun findActive(): List<Entity807> = repo.findByField4(true)
    fun search(query: String): List<Entity807> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity807>): Map<String, List<Entity807>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity807): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity807")
class Controller807(private val service: Service807) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity807) = service.save(entity)
    @DeleteMapping("/{id}")
