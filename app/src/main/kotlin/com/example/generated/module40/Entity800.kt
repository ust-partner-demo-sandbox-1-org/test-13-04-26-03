...
interface Entity800Repository : org.springframework.data.jpa.repository.JpaRepository<Entity800, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity800>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity800>
}
...
class Service800(private val repo: Entity800Repository) {
    ...
    fun findActive(): List<Entity800> = repo.findByField4(true)
    fun search(query: String): List<Entity800> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity800>): Map<String, List<Entity800>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity800): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity800")
class Controller800(private val service: Service800) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity800) = service.save(entity)
    @DeleteMapping("/{id}")
