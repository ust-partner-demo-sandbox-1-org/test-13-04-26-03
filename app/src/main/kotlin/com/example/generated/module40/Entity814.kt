...
interface Entity814Repository : org.springframework.data.jpa.repository.JpaRepository<Entity814, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity814>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity814>
}
...
class Service814(private val repo: Entity814Repository) {
    ...
    fun findActive(): List<Entity814> = repo.findByField4(true)
    fun search(query: String): List<Entity814> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity814>): Map<String, List<Entity814>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity814): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity814")
class Controller814(private val service: Service814) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity814) = service.save(entity)
    @DeleteMapping("/{id}")
