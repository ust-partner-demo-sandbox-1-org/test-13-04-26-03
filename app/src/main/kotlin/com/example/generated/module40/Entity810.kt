...
interface Entity810Repository : org.springframework.data.jpa.repository.JpaRepository<Entity810, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity810>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity810>
}
...
class Service810(private val repo: Entity810Repository) {
    ...
    fun findActive(): List<Entity810> = repo.findByField4(true)
    fun search(query: String): List<Entity810> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity810>): Map<String, List<Entity810>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity810): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity810")
class Controller810(private val service: Service810) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity810) = service.save(entity)
    @DeleteMapping("/{id}")
