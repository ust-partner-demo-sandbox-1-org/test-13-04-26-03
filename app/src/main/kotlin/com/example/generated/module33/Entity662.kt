...
interface Entity662Repository : org.springframework.data.jpa.repository.JpaRepository<Entity662, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity662>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity662>
}
...
class Service662(private val repo: Entity662Repository) {
    ...
    fun findActive(): List<Entity662> = repo.findByField4(true)
    fun search(query: String): List<Entity662> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity662>): Map<String, List<Entity662>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity662): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity662")
class Controller662(private val service: Service662) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity662) = service.save(entity)
    @DeleteMapping("/{id}")
