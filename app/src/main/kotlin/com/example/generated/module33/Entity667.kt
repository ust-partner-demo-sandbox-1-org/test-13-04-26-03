...
interface Entity667Repository : org.springframework.data.jpa.repository.JpaRepository<Entity667, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity667>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity667>
}
...
class Service667(private val repo: Entity667Repository) {
    ...
    fun findActive(): List<Entity667> = repo.findByField4(true)
    fun search(query: String): List<Entity667> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity667>): Map<String, List<Entity667>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity667): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity667")
class Controller667(private val service: Service667) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity667) = service.save(entity)
    @DeleteMapping("/{id}")
