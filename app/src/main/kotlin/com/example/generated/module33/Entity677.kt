...
interface Entity677Repository : org.springframework.data.jpa.repository.JpaRepository<Entity677, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity677>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity677>
}
...
class Service677(private val repo: Entity677Repository) {
    ...
    fun findActive(): List<Entity677> = repo.findByField4(true)
    fun search(query: String): List<Entity677> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity677>): Map<String, List<Entity677>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity677): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity677")
class Controller677(private val service: Service677) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity677) = service.save(entity)
    @DeleteMapping("/{id}")
