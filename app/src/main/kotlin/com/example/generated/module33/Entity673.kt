...
interface Entity673Repository : org.springframework.data.jpa.repository.JpaRepository<Entity673, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity673>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity673>
}
...
class Service673(private val repo: Entity673Repository) {
    ...
    fun findActive(): List<Entity673> = repo.findByField4(true)
    fun search(query: String): List<Entity673> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity673>): Map<String, List<Entity673>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity673): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity673")
class Controller673(private val service: Service673) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity673) = service.save(entity)
    @DeleteMapping("/{id}")
