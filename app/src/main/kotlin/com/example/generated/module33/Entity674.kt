...
interface Entity674Repository : org.springframework.data.jpa.repository.JpaRepository<Entity674, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity674>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity674>
}
...
class Service674(private val repo: Entity674Repository) {
    ...
    fun findActive(): List<Entity674> = repo.findByField4(true)
    fun search(query: String): List<Entity674> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity674>): Map<String, List<Entity674>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity674): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity674")
class Controller674(private val service: Service674) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity674) = service.save(entity)
    @DeleteMapping("/{id}")
