...
interface Entity668Repository : org.springframework.data.jpa.repository.JpaRepository<Entity668, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity668>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity668>
}
...
class Service668(private val repo: Entity668Repository) {
    ...
    fun findActive(): List<Entity668> = repo.findByField4(true)
    fun search(query: String): List<Entity668> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity668>): Map<String, List<Entity668>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity668): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity668")
class Controller668(private val service: Service668) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity668) = service.save(entity)
    @DeleteMapping("/{id}")
