...
interface Entity518Repository : org.springframework.data.jpa.repository.JpaRepository<Entity518, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity518>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity518>
}
...
class Service518(private val repo: Entity518Repository) {
    ...
    fun findActive(): List<Entity518> = repo.findByField4(true)
    fun search(query: String): List<Entity518> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity518>): Map<String, List<Entity518>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity518): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity518")
class Controller518(private val service: Service518) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity518) = service.save(entity)
    @DeleteMapping("/{id}")
