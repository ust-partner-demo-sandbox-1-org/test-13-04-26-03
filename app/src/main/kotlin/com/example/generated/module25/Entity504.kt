...
interface Entity504Repository : org.springframework.data.jpa.repository.JpaRepository<Entity504, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity504>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity504>
}
...
class Service504(private val repo: Entity504Repository) {
    ...
    fun findActive(): List<Entity504> = repo.findByField4(true)
    fun search(query: String): List<Entity504> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity504>): Map<String, List<Entity504>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity504): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity504")
class Controller504(private val service: Service504) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity504) = service.save(entity)
    @DeleteMapping("/{id}")
