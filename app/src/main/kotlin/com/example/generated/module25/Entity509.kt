...
interface Entity509Repository : org.springframework.data.jpa.repository.JpaRepository<Entity509, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity509>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity509>
}
...
class Service509(private val repo: Entity509Repository) {
    ...
    fun findActive(): List<Entity509> = repo.findByField4(true)
    fun search(query: String): List<Entity509> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity509>): Map<String, List<Entity509>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity509): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity509")
class Controller509(private val service: Service509) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity509) = service.save(entity)
    @DeleteMapping("/{id}")
