...
interface Entity513Repository : org.springframework.data.jpa.repository.JpaRepository<Entity513, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity513>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity513>
}
...
class Service513(private val repo: Entity513Repository) {
    ...
    fun findActive(): List<Entity513> = repo.findByField4(true)
    fun search(query: String): List<Entity513> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity513>): Map<String, List<Entity513>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity513): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity513")
class Controller513(private val service: Service513) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity513) = service.save(entity)
    @DeleteMapping("/{id}")
