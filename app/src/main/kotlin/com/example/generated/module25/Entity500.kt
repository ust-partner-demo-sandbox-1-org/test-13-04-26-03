...
interface Entity500Repository : org.springframework.data.jpa.repository.JpaRepository<Entity500, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity500>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity500>
}
...
class Service500(private val repo: Entity500Repository) {
    ...
    fun findActive(): List<Entity500> = repo.findByField4(true)
    fun search(query: String): List<Entity500> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity500>): Map<String, List<Entity500>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity500): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity500")
class Controller500(private val service: Service500) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity500) = service.save(entity)
    @DeleteMapping("/{id}")
