...
interface Entity514Repository : org.springframework.data.jpa.repository.JpaRepository<Entity514, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity514>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity514>
}
...
class Service514(private val repo: Entity514Repository) {
    ...
    fun findActive(): List<Entity514> = repo.findByField4(true)
    fun search(query: String): List<Entity514> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity514>): Map<String, List<Entity514>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity514): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity514")
class Controller514(private val service: Service514) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity514) = service.save(entity)
    @DeleteMapping("/{id}")
