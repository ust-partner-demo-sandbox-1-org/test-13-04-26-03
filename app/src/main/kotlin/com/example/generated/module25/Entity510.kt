...
interface Entity510Repository : org.springframework.data.jpa.repository.JpaRepository<Entity510, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity510>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity510>
}
...
class Service510(private val repo: Entity510Repository) {
    ...
    fun findActive(): List<Entity510> = repo.findByField4(true)
    fun search(query: String): List<Entity510> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity510>): Map<String, List<Entity510>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity510): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity510")
class Controller510(private val service: Service510) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity510) = service.save(entity)
    @DeleteMapping("/{id}")
