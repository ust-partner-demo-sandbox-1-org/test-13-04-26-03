...
interface Entity515Repository : org.springframework.data.jpa.repository.JpaRepository<Entity515, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity515>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity515>
}
...
class Service515(private val repo: Entity515Repository) {
    ...
    fun findActive(): List<Entity515> = repo.findByField4(true)
    fun search(query: String): List<Entity515> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity515>): Map<String, List<Entity515>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity515): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity515")
class Controller515(private val service: Service515) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity515) = service.save(entity)
    @DeleteMapping("/{id}")
