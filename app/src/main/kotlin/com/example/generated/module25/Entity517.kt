...
interface Entity517Repository : org.springframework.data.jpa.repository.JpaRepository<Entity517, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity517>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity517>
}
...
class Service517(private val repo: Entity517Repository) {
    ...
    fun findActive(): List<Entity517> = repo.findByField4(true)
    fun search(query: String): List<Entity517> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity517>): Map<String, List<Entity517>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity517): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity517")
class Controller517(private val service: Service517) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity517) = service.save(entity)
    @DeleteMapping("/{id}")
