...
interface Entity503Repository : org.springframework.data.jpa.repository.JpaRepository<Entity503, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity503>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity503>
}
...
class Service503(private val repo: Entity503Repository) {
    ...
    fun findActive(): List<Entity503> = repo.findByField4(true)
    fun search(query: String): List<Entity503> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity503>): Map<String, List<Entity503>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity503): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity503")
class Controller503(private val service: Service503) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity503) = service.save(entity)
    @DeleteMapping("/{id}")
