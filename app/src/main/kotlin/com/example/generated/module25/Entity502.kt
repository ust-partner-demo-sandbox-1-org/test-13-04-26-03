...
interface Entity502Repository : org.springframework.data.jpa.repository.JpaRepository<Entity502, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity502>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity502>
}
...
class Service502(private val repo: Entity502Repository) {
    ...
    fun findActive(): List<Entity502> = repo.findByField4(true)
    fun search(query: String): List<Entity502> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity502>): Map<String, List<Entity502>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity502): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity502")
class Controller502(private val service: Service502) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity502) = service.save(entity)
    @DeleteMapping("/{id}")
