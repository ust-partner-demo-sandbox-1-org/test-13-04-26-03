...
interface Entity519Repository : org.springframework.data.jpa.repository.JpaRepository<Entity519, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity519>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity519>
}
...
class Service519(private val repo: Entity519Repository) {
    ...
    fun findActive(): List<Entity519> = repo.findByField4(true)
    fun search(query: String): List<Entity519> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity519>): Map<String, List<Entity519>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity519): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity519")
class Controller519(private val service: Service519) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity519) = service.save(entity)
    @DeleteMapping("/{id}")
