...
interface Entity609Repository : org.springframework.data.jpa.repository.JpaRepository<Entity609, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity609>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity609>
}
...
class Service609(private val repo: Entity609Repository) {
    ...
    fun findActive(): List<Entity609> = repo.findByField4(true)
    fun search(query: String): List<Entity609> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity609>): Map<String, List<Entity609>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity609): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity609")
class Controller609(private val service: Service609) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity609) = service.save(entity)
    @DeleteMapping("/{id}")
