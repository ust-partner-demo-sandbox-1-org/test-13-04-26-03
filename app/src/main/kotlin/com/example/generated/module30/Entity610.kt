...
interface Entity610Repository : org.springframework.data.jpa.repository.JpaRepository<Entity610, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity610>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity610>
}
...
class Service610(private val repo: Entity610Repository) {
    ...
    fun findActive(): List<Entity610> = repo.findByField4(true)
    fun search(query: String): List<Entity610> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity610>): Map<String, List<Entity610>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity610): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity610")
class Controller610(private val service: Service610) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity610) = service.save(entity)
    @DeleteMapping("/{id}")
