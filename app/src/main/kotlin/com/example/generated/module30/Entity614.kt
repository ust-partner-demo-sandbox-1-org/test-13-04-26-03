...
interface Entity614Repository : org.springframework.data.jpa.repository.JpaRepository<Entity614, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity614>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity614>
}
...
class Service614(private val repo: Entity614Repository) {
    ...
    fun findActive(): List<Entity614> = repo.findByField4(true)
    fun search(query: String): List<Entity614> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity614>): Map<String, List<Entity614>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity614): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity614")
class Controller614(private val service: Service614) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity614) = service.save(entity)
    @DeleteMapping("/{id}")
