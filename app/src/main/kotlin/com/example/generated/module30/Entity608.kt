...
interface Entity608Repository : org.springframework.data.jpa.repository.JpaRepository<Entity608, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity608>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity608>
}
...
class Service608(private val repo: Entity608Repository) {
    ...
    fun findActive(): List<Entity608> = repo.findByField4(true)
    fun search(query: String): List<Entity608> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity608>): Map<String, List<Entity608>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity608): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity608")
class Controller608(private val service: Service608) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity608) = service.save(entity)
    @DeleteMapping("/{id}")
