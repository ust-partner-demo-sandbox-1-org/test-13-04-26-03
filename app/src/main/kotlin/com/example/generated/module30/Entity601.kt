...
interface Entity601Repository : org.springframework.data.jpa.repository.JpaRepository<Entity601, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity601>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity601>
}
...
class Service601(private val repo: Entity601Repository) {
    ...
    fun findActive(): List<Entity601> = repo.findByField4(true)
    fun search(query: String): List<Entity601> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity601>): Map<String, List<Entity601>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity601): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity601")
class Controller601(private val service: Service601) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity601) = service.save(entity)
    @DeleteMapping("/{id}")
