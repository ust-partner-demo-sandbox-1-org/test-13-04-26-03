...
interface Entity613Repository : org.springframework.data.jpa.repository.JpaRepository<Entity613, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity613>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity613>
}
...
class Service613(private val repo: Entity613Repository) {
    ...
    fun findActive(): List<Entity613> = repo.findByField4(true)
    fun search(query: String): List<Entity613> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity613>): Map<String, List<Entity613>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity613): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity613")
class Controller613(private val service: Service613) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity613) = service.save(entity)
    @DeleteMapping("/{id}")
