...
interface Entity748Repository : org.springframework.data.jpa.repository.JpaRepository<Entity748, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity748>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity748>
}
...
class Service748(private val repo: Entity748Repository) {
    ...
    fun findActive(): List<Entity748> = repo.findByField4(true)
    fun search(query: String): List<Entity748> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity748>): Map<String, List<Entity748>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity748): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity748")
class Controller748(private val service: Service748) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity748) = service.save(entity)
    @DeleteMapping("/{id}")
