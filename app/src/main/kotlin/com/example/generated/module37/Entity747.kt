...
interface Entity747Repository : org.springframework.data.jpa.repository.JpaRepository<Entity747, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity747>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity747>
}
...
class Service747(private val repo: Entity747Repository) {
    ...
    fun findActive(): List<Entity747> = repo.findByField4(true)
    fun search(query: String): List<Entity747> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity747>): Map<String, List<Entity747>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity747): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity747")
class Controller747(private val service: Service747) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity747) = service.save(entity)
    @DeleteMapping("/{id}")
