...
interface Entity751Repository : org.springframework.data.jpa.repository.JpaRepository<Entity751, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity751>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity751>
}
...
class Service751(private val repo: Entity751Repository) {
    ...
    fun findActive(): List<Entity751> = repo.findByField4(true)
    fun search(query: String): List<Entity751> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity751>): Map<String, List<Entity751>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity751): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity751")
class Controller751(private val service: Service751) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity751) = service.save(entity)
    @DeleteMapping("/{id}")
