...
interface Entity749Repository : org.springframework.data.jpa.repository.JpaRepository<Entity749, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity749>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity749>
}
...
class Service749(private val repo: Entity749Repository) {
    ...
    fun findActive(): List<Entity749> = repo.findByField4(true)
    fun search(query: String): List<Entity749> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity749>): Map<String, List<Entity749>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity749): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity749")
class Controller749(private val service: Service749) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity749) = service.save(entity)
    @DeleteMapping("/{id}")
