...
interface Entity758Repository : org.springframework.data.jpa.repository.JpaRepository<Entity758, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity758>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity758>
}
...
class Service758(private val repo: Entity758Repository) {
    ...
    fun findActive(): List<Entity758> = repo.findByField4(true)
    fun search(query: String): List<Entity758> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity758>): Map<String, List<Entity758>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity758): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity758")
class Controller758(private val service: Service758) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity758) = service.save(entity)
    @DeleteMapping("/{id}")
