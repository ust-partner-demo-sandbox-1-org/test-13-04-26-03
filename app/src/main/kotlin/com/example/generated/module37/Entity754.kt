...
interface Entity754Repository : org.springframework.data.jpa.repository.JpaRepository<Entity754, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity754>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity754>
}
...
class Service754(private val repo: Entity754Repository) {
    ...
    fun findActive(): List<Entity754> = repo.findByField4(true)
    fun search(query: String): List<Entity754> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity754>): Map<String, List<Entity754>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity754): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity754")
class Controller754(private val service: Service754) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity754) = service.save(entity)
    @DeleteMapping("/{id}")
