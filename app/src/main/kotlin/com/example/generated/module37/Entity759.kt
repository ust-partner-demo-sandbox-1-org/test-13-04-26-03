...
interface Entity759Repository : org.springframework.data.jpa.repository.JpaRepository<Entity759, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity759>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity759>
}
...
class Service759(private val repo: Entity759Repository) {
    ...
    fun findActive(): List<Entity759> = repo.findByField4(true)
    fun search(query: String): List<Entity759> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity759>): Map<String, List<Entity759>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity759): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity759")
class Controller759(private val service: Service759) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity759) = service.save(entity)
    @DeleteMapping("/{id}")
