...
interface Entity756Repository : org.springframework.data.jpa.repository.JpaRepository<Entity756, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity756>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity756>
}
...
class Service756(private val repo: Entity756Repository) {
    ...
    fun findActive(): List<Entity756> = repo.findByField4(true)
    fun search(query: String): List<Entity756> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity756>): Map<String, List<Entity756>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity756): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity756")
class Controller756(private val service: Service756) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity756) = service.save(entity)
    @DeleteMapping("/{id}")
