...
interface Entity637Repository : org.springframework.data.jpa.repository.JpaRepository<Entity637, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity637>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity637>
}
...
class Service637(private val repo: Entity637Repository) {
    ...
    fun findActive(): List<Entity637> = repo.findByField4(true)
    fun search(query: String): List<Entity637> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity637>): Map<String, List<Entity637>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity637): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity637")
class Controller637(private val service: Service637) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity637) = service.save(entity)
    @DeleteMapping("/{id}")
