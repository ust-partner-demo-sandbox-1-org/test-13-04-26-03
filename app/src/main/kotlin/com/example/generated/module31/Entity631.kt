...
interface Entity631Repository : org.springframework.data.jpa.repository.JpaRepository<Entity631, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity631>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity631>
}
...
class Service631(private val repo: Entity631Repository) {
    ...
    fun findActive(): List<Entity631> = repo.findByField4(true)
    fun search(query: String): List<Entity631> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity631>): Map<String, List<Entity631>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity631): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity631")
class Controller631(private val service: Service631) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity631) = service.save(entity)
    @DeleteMapping("/{id}")
