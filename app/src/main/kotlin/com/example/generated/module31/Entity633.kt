...
interface Entity633Repository : org.springframework.data.jpa.repository.JpaRepository<Entity633, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity633>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity633>
}
...
class Service633(private val repo: Entity633Repository) {
    ...
    fun findActive(): List<Entity633> = repo.findByField4(true)
    fun search(query: String): List<Entity633> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity633>): Map<String, List<Entity633>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity633): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity633")
class Controller633(private val service: Service633) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity633) = service.save(entity)
    @DeleteMapping("/{id}")
