...
interface Entity628Repository : org.springframework.data.jpa.repository.JpaRepository<Entity628, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity628>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity628>
}
...
class Service628(private val repo: Entity628Repository) {
    ...
    fun findActive(): List<Entity628> = repo.findByField4(true)
    fun search(query: String): List<Entity628> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity628>): Map<String, List<Entity628>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity628): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity628")
class Controller628(private val service: Service628) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity628) = service.save(entity)
    @DeleteMapping("/{id}")
