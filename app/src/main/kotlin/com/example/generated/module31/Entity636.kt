...
interface Entity636Repository : org.springframework.data.jpa.repository.JpaRepository<Entity636, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity636>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity636>
}
...
class Service636(private val repo: Entity636Repository) {
    ...
    fun findActive(): List<Entity636> = repo.findByField4(true)
    fun search(query: String): List<Entity636> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity636>): Map<String, List<Entity636>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity636): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity636")
class Controller636(private val service: Service636) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity636) = service.save(entity)
    @DeleteMapping("/{id}")
