...
interface Entity630Repository : org.springframework.data.jpa.repository.JpaRepository<Entity630, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity630>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity630>
}
...
class Service630(private val repo: Entity630Repository) {
    ...
    fun findActive(): List<Entity630> = repo.findByField4(true)
    fun search(query: String): List<Entity630> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity630>): Map<String, List<Entity630>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity630): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity630")
class Controller630(private val service: Service630) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity630) = service.save(entity)
    @DeleteMapping("/{id}")
