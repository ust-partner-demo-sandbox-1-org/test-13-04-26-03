...
interface Entity639Repository : org.springframework.data.jpa.repository.JpaRepository<Entity639, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity639>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity639>
}
...
class Service639(private val repo: Entity639Repository) {
    ...
    fun findActive(): List<Entity639> = repo.findByField4(true)
    fun search(query: String): List<Entity639> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity639>): Map<String, List<Entity639>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity639): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity639")
class Controller639(private val service: Service639) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity639) = service.save(entity)
    @DeleteMapping("/{id}")
