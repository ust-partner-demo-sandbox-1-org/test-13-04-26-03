...
interface Entity634Repository : org.springframework.data.jpa.repository.JpaRepository<Entity634, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity634>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity634>
}
...
class Service634(private val repo: Entity634Repository) {
    ...
    fun findActive(): List<Entity634> = repo.findByField4(true)
    fun search(query: String): List<Entity634> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity634>): Map<String, List<Entity634>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity634): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity634")
class Controller634(private val service: Service634) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity634) = service.save(entity)
    @DeleteMapping("/{id}")
