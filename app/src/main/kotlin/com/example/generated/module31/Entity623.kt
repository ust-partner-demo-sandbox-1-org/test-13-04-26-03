...
interface Entity623Repository : org.springframework.data.jpa.repository.JpaRepository<Entity623, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity623>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity623>
}
...
class Service623(private val repo: Entity623Repository) {
    ...
    fun findActive(): List<Entity623> = repo.findByField4(true)
    fun search(query: String): List<Entity623> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity623>): Map<String, List<Entity623>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity623): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity623")
class Controller623(private val service: Service623) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity623) = service.save(entity)
    @DeleteMapping("/{id}")
