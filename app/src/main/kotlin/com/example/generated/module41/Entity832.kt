...
interface Entity832Repository : org.springframework.data.jpa.repository.JpaRepository<Entity832, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity832>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity832>
}
...
class Service832(private val repo: Entity832Repository) {
    ...
    fun findActive(): List<Entity832> = repo.findByField4(true)
    fun search(query: String): List<Entity832> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity832>): Map<String, List<Entity832>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity832): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity832")
class Controller832(private val service: Service832) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity832) = service.save(entity)
    @DeleteMapping("/{id}")
