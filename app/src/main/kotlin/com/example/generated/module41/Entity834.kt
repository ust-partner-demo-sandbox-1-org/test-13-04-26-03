...
interface Entity834Repository : org.springframework.data.jpa.repository.JpaRepository<Entity834, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity834>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity834>
}
...
class Service834(private val repo: Entity834Repository) {
    ...
    fun findActive(): List<Entity834> = repo.findByField4(true)
    fun search(query: String): List<Entity834> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity834>): Map<String, List<Entity834>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity834): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity834")
class Controller834(private val service: Service834) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity834) = service.save(entity)
    @DeleteMapping("/{id}")
