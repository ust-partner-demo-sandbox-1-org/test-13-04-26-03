...
interface Entity830Repository : org.springframework.data.jpa.repository.JpaRepository<Entity830, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity830>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity830>
}
...
class Service830(private val repo: Entity830Repository) {
    ...
    fun findActive(): List<Entity830> = repo.findByField4(true)
    fun search(query: String): List<Entity830> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity830>): Map<String, List<Entity830>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity830): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity830")
class Controller830(private val service: Service830) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity830) = service.save(entity)
    @DeleteMapping("/{id}")
