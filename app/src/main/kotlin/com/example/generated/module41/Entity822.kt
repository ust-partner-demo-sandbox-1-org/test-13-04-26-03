...
interface Entity822Repository : org.springframework.data.jpa.repository.JpaRepository<Entity822, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity822>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity822>
}
...
class Service822(private val repo: Entity822Repository) {
    ...
    fun findActive(): List<Entity822> = repo.findByField4(true)
    fun search(query: String): List<Entity822> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity822>): Map<String, List<Entity822>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity822): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity822")
class Controller822(private val service: Service822) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity822) = service.save(entity)
    @DeleteMapping("/{id}")
