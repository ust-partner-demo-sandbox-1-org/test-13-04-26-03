...
interface Entity824Repository : org.springframework.data.jpa.repository.JpaRepository<Entity824, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity824>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity824>
}
...
class Service824(private val repo: Entity824Repository) {
    ...
    fun findActive(): List<Entity824> = repo.findByField4(true)
    fun search(query: String): List<Entity824> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity824>): Map<String, List<Entity824>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity824): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity824")
class Controller824(private val service: Service824) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity824) = service.save(entity)
    @DeleteMapping("/{id}")
