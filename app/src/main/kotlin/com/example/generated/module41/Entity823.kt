...
interface Entity823Repository : org.springframework.data.jpa.repository.JpaRepository<Entity823, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity823>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity823>
}
...
class Service823(private val repo: Entity823Repository) {
    ...
    fun findActive(): List<Entity823> = repo.findByField4(true)
    fun search(query: String): List<Entity823> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity823>): Map<String, List<Entity823>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity823): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity823")
class Controller823(private val service: Service823) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity823) = service.save(entity)
    @DeleteMapping("/{id}")
