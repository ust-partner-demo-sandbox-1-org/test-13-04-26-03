...
interface Entity828Repository : org.springframework.data.jpa.repository.JpaRepository<Entity828, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity828>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity828>
}
...
class Service828(private val repo: Entity828Repository) {
    ...
    fun findActive(): List<Entity828> = repo.findByField4(true)
    fun search(query: String): List<Entity828> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity828>): Map<String, List<Entity828>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity828): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity828")
class Controller828(private val service: Service828) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity828) = service.save(entity)
    @DeleteMapping("/{id}")
