...
interface Entity836Repository : org.springframework.data.jpa.repository.JpaRepository<Entity836, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity836>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity836>
}
...
class Service836(private val repo: Entity836Repository) {
    ...
    fun findActive(): List<Entity836> = repo.findByField4(true)
    fun search(query: String): List<Entity836> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity836>): Map<String, List<Entity836>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity836): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity836")
class Controller836(private val service: Service836) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity836) = service.save(entity)
    @DeleteMapping("/{id}")
