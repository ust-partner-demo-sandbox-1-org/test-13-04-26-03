...
interface Entity833Repository : org.springframework.data.jpa.repository.JpaRepository<Entity833, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity833>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity833>
}
...
class Service833(private val repo: Entity833Repository) {
    ...
    fun findActive(): List<Entity833> = repo.findByField4(true)
    fun search(query: String): List<Entity833> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity833>): Map<String, List<Entity833>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity833): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity833")
class Controller833(private val service: Service833) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity833) = service.save(entity)
    @DeleteMapping("/{id}")
