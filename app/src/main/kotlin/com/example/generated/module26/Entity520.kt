...
interface Entity520Repository : org.springframework.data.jpa.repository.JpaRepository<Entity520, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity520>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity520>
}
...
class Service520(private val repo: Entity520Repository) {
    ...
    fun findActive(): List<Entity520> = repo.findByField4(true)
    fun search(query: String): List<Entity520> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity520>): Map<String, List<Entity520>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity520): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity520")
class Controller520(private val service: Service520) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity520) = service.save(entity)
    @DeleteMapping("/{id}")
