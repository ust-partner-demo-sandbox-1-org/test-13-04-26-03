...
interface Entity527Repository : org.springframework.data.jpa.repository.JpaRepository<Entity527, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity527>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity527>
}
...
class Service527(private val repo: Entity527Repository) {
    ...
    fun findActive(): List<Entity527> = repo.findByField4(true)
    fun search(query: String): List<Entity527> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity527>): Map<String, List<Entity527>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity527): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity527")
class Controller527(private val service: Service527) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity527) = service.save(entity)
    @DeleteMapping("/{id}")
