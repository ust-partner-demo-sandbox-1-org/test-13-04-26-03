...
interface Entity526Repository : org.springframework.data.jpa.repository.JpaRepository<Entity526, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity526>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity526>
}
...
class Service526(private val repo: Entity526Repository) {
    ...
    fun findActive(): List<Entity526> = repo.findByField4(true)
    fun search(query: String): List<Entity526> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity526>): Map<String, List<Entity526>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity526): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity526")
class Controller526(private val service: Service526) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity526) = service.save(entity)
    @DeleteMapping("/{id}")
