...
interface Entity537Repository : org.springframework.data.jpa.repository.JpaRepository<Entity537, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity537>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity537>
}
...
class Service537(private val repo: Entity537Repository) {
    ...
    fun findActive(): List<Entity537> = repo.findByField4(true)
    fun search(query: String): List<Entity537> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity537>): Map<String, List<Entity537>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity537): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity537")
class Controller537(private val service: Service537) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity537) = service.save(entity)
    @DeleteMapping("/{id}")
