...
interface Entity523Repository : org.springframework.data.jpa.repository.JpaRepository<Entity523, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity523>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity523>
}
...
class Service523(private val repo: Entity523Repository) {
    ...
    fun findActive(): List<Entity523> = repo.findByField4(true)
    fun search(query: String): List<Entity523> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity523>): Map<String, List<Entity523>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity523): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity523")
class Controller523(private val service: Service523) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity523) = service.save(entity)
    @DeleteMapping("/{id}")
