...
interface Entity532Repository : org.springframework.data.jpa.repository.JpaRepository<Entity532, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity532>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity532>
}
...
class Service532(private val repo: Entity532Repository) {
    ...
    fun findActive(): List<Entity532> = repo.findByField4(true)
    fun search(query: String): List<Entity532> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity532>): Map<String, List<Entity532>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity532): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity532")
class Controller532(private val service: Service532) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity532) = service.save(entity)
    @DeleteMapping("/{id}")
