...
interface Entity531Repository : org.springframework.data.jpa.repository.JpaRepository<Entity531, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity531>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity531>
}
...
class Service531(private val repo: Entity531Repository) {
    ...
    fun findActive(): List<Entity531> = repo.findByField4(true)
    fun search(query: String): List<Entity531> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity531>): Map<String, List<Entity531>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity531): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity531")
class Controller531(private val service: Service531) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity531) = service.save(entity)
    @DeleteMapping("/{id}")
