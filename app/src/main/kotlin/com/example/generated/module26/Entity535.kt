...
interface Entity535Repository : org.springframework.data.jpa.repository.JpaRepository<Entity535, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity535>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity535>
}
...
class Service535(private val repo: Entity535Repository) {
    ...
    fun findActive(): List<Entity535> = repo.findByField4(true)
    fun search(query: String): List<Entity535> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity535>): Map<String, List<Entity535>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity535): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity535")
class Controller535(private val service: Service535) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity535) = service.save(entity)
    @DeleteMapping("/{id}")
