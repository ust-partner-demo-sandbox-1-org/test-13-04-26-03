...
interface Entity524Repository : org.springframework.data.jpa.repository.JpaRepository<Entity524, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity524>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity524>
}
...
class Service524(private val repo: Entity524Repository) {
    ...
    fun findActive(): List<Entity524> = repo.findByField4(true)
    fun search(query: String): List<Entity524> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity524>): Map<String, List<Entity524>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity524): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity524")
class Controller524(private val service: Service524) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity524) = service.save(entity)
    @DeleteMapping("/{id}")
