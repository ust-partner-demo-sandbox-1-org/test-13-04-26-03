...
interface Entity529Repository : org.springframework.data.jpa.repository.JpaRepository<Entity529, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity529>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity529>
}
...
class Service529(private val repo: Entity529Repository) {
    ...
    fun findActive(): List<Entity529> = repo.findByField4(true)
    fun search(query: String): List<Entity529> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity529>): Map<String, List<Entity529>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity529): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity529")
class Controller529(private val service: Service529) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity529) = service.save(entity)
    @DeleteMapping("/{id}")
