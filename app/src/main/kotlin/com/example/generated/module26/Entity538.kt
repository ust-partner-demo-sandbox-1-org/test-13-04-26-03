...
interface Entity538Repository : org.springframework.data.jpa.repository.JpaRepository<Entity538, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity538>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity538>
}
...
class Service538(private val repo: Entity538Repository) {
    ...
    fun findActive(): List<Entity538> = repo.findByField4(true)
    fun search(query: String): List<Entity538> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity538>): Map<String, List<Entity538>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity538): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity538")
class Controller538(private val service: Service538) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity538) = service.save(entity)
    @DeleteMapping("/{id}")
