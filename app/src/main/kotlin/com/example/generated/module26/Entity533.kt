...
interface Entity533Repository : org.springframework.data.jpa.repository.JpaRepository<Entity533, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity533>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity533>
}
...
class Service533(private val repo: Entity533Repository) {
    ...
    fun findActive(): List<Entity533> = repo.findByField4(true)
    fun search(query: String): List<Entity533> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity533>): Map<String, List<Entity533>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity533): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity533")
class Controller533(private val service: Service533) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity533) = service.save(entity)
    @DeleteMapping("/{id}")
