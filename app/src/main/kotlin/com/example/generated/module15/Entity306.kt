...
interface Entity306Repository : org.springframework.data.jpa.repository.JpaRepository<Entity306, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity306>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity306>
}
...
class Service306(private val repo: Entity306Repository) {
    ...
    fun findActive(): List<Entity306> = repo.findByField4(true)
    fun search(query: String): List<Entity306> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity306>): Map<String, List<Entity306>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity306): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity306")
class Controller306(private val service: Service306) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity306) = service.save(entity)
    @DeleteMapping("/{id}")
