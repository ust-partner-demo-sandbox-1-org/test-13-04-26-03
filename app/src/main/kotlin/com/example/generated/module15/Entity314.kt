...
interface Entity314Repository : org.springframework.data.jpa.repository.JpaRepository<Entity314, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity314>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity314>
}
...
class Service314(private val repo: Entity314Repository) {
    ...
    fun findActive(): List<Entity314> = repo.findByField4(true)
    fun search(query: String): List<Entity314> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity314>): Map<String, List<Entity314>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity314): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity314")
class Controller314(private val service: Service314) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity314) = service.save(entity)
    @DeleteMapping("/{id}")
