...
interface Entity307Repository : org.springframework.data.jpa.repository.JpaRepository<Entity307, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity307>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity307>
}
...
class Service307(private val repo: Entity307Repository) {
    ...
    fun findActive(): List<Entity307> = repo.findByField4(true)
    fun search(query: String): List<Entity307> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity307>): Map<String, List<Entity307>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity307): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity307")
class Controller307(private val service: Service307) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity307) = service.save(entity)
    @DeleteMapping("/{id}")
