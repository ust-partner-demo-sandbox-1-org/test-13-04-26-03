...
interface Entity302Repository : org.springframework.data.jpa.repository.JpaRepository<Entity302, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity302>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity302>
}
...
class Service302(private val repo: Entity302Repository) {
    ...
    fun findActive(): List<Entity302> = repo.findByField4(true)
    fun search(query: String): List<Entity302> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity302>): Map<String, List<Entity302>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity302): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity302")
class Controller302(private val service: Service302) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity302) = service.save(entity)
    @DeleteMapping("/{id}")
