...
interface Entity300Repository : org.springframework.data.jpa.repository.JpaRepository<Entity300, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity300>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity300>
}
...
class Service300(private val repo: Entity300Repository) {
    ...
    fun findActive(): List<Entity300> = repo.findByField4(true)
    fun search(query: String): List<Entity300> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity300>): Map<String, List<Entity300>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity300): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity300")
class Controller300(private val service: Service300) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity300) = service.save(entity)
    @DeleteMapping("/{id}")
