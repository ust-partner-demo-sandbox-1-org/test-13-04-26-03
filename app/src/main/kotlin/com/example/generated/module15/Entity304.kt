...
interface Entity304Repository : org.springframework.data.jpa.repository.JpaRepository<Entity304, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity304>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity304>
}
...
class Service304(private val repo: Entity304Repository) {
    ...
    fun findActive(): List<Entity304> = repo.findByField4(true)
    fun search(query: String): List<Entity304> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity304>): Map<String, List<Entity304>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity304): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity304")
class Controller304(private val service: Service304) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity304) = service.save(entity)
    @DeleteMapping("/{id}")
