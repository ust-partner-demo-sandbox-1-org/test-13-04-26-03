...
interface Entity308Repository : org.springframework.data.jpa.repository.JpaRepository<Entity308, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity308>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity308>
}
...
class Service308(private val repo: Entity308Repository) {
    ...
    fun findActive(): List<Entity308> = repo.findByField4(true)
    fun search(query: String): List<Entity308> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity308>): Map<String, List<Entity308>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity308): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity308")
class Controller308(private val service: Service308) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity308) = service.save(entity)
    @DeleteMapping("/{id}")
