...
interface Entity303Repository : org.springframework.data.jpa.repository.JpaRepository<Entity303, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity303>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity303>
}
...
class Service303(private val repo: Entity303Repository) {
    ...
    fun findActive(): List<Entity303> = repo.findByField4(true)
    fun search(query: String): List<Entity303> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity303>): Map<String, List<Entity303>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity303): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity303")
class Controller303(private val service: Service303) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity303) = service.save(entity)
    @DeleteMapping("/{id}")
