...
interface Entity305Repository : org.springframework.data.jpa.repository.JpaRepository<Entity305, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity305>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity305>
}
...
class Service305(private val repo: Entity305Repository) {
    ...
    fun findActive(): List<Entity305> = repo.findByField4(true)
    fun search(query: String): List<Entity305> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity305>): Map<String, List<Entity305>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity305): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity305")
class Controller305(private val service: Service305) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity305) = service.save(entity)
    @DeleteMapping("/{id}")
