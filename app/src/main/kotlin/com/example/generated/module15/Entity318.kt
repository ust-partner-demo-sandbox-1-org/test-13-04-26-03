...
interface Entity318Repository : org.springframework.data.jpa.repository.JpaRepository<Entity318, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity318>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity318>
}
...
class Service318(private val repo: Entity318Repository) {
    ...
    fun findActive(): List<Entity318> = repo.findByField4(true)
    fun search(query: String): List<Entity318> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity318>): Map<String, List<Entity318>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity318): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity318")
class Controller318(private val service: Service318) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity318) = service.save(entity)
    @DeleteMapping("/{id}")
