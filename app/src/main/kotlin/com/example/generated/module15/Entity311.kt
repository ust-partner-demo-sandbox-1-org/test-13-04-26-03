...
interface Entity311Repository : org.springframework.data.jpa.repository.JpaRepository<Entity311, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity311>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity311>
}
...
class Service311(private val repo: Entity311Repository) {
    ...
    fun findActive(): List<Entity311> = repo.findByField4(true)
    fun search(query: String): List<Entity311> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity311>): Map<String, List<Entity311>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity311): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity311")
class Controller311(private val service: Service311) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity311) = service.save(entity)
    @DeleteMapping("/{id}")
