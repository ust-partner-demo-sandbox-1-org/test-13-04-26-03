...
interface Entity315Repository : org.springframework.data.jpa.repository.JpaRepository<Entity315, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity315>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity315>
}
...
class Service315(private val repo: Entity315Repository) {
    ...
    fun findActive(): List<Entity315> = repo.findByField4(true)
    fun search(query: String): List<Entity315> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity315>): Map<String, List<Entity315>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity315): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity315")
class Controller315(private val service: Service315) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity315) = service.save(entity)
    @DeleteMapping("/{id}")
