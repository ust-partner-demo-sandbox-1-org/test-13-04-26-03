...
interface Entity687Repository : org.springframework.data.jpa.repository.JpaRepository<Entity687, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity687>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity687>
}
...
class Service687(private val repo: Entity687Repository) {
    ...
    fun findActive(): List<Entity687> = repo.findByField4(true)
    fun search(query: String): List<Entity687> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity687>): Map<String, List<Entity687>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity687): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity687")
class Controller687(private val service: Service687) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity687) = service.save(entity)
    @DeleteMapping("/{id}")
