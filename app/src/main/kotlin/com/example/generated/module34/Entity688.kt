...
interface Entity688Repository : org.springframework.data.jpa.repository.JpaRepository<Entity688, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity688>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity688>
}
...
class Service688(private val repo: Entity688Repository) {
    ...
    fun findActive(): List<Entity688> = repo.findByField4(true)
    fun search(query: String): List<Entity688> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity688>): Map<String, List<Entity688>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity688): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity688")
class Controller688(private val service: Service688) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity688) = service.save(entity)
    @DeleteMapping("/{id}")
