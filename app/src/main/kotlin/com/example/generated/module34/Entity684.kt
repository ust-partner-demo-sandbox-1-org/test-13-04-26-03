...
interface Entity684Repository : org.springframework.data.jpa.repository.JpaRepository<Entity684, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity684>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity684>
}
...
class Service684(private val repo: Entity684Repository) {
    ...
    fun findActive(): List<Entity684> = repo.findByField4(true)
    fun search(query: String): List<Entity684> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity684>): Map<String, List<Entity684>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity684): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity684")
class Controller684(private val service: Service684) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity684) = service.save(entity)
    @DeleteMapping("/{id}")
