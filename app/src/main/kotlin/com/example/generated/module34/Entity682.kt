...
interface Entity682Repository : org.springframework.data.jpa.repository.JpaRepository<Entity682, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity682>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity682>
}
...
class Service682(private val repo: Entity682Repository) {
    ...
    fun findActive(): List<Entity682> = repo.findByField4(true)
    fun search(query: String): List<Entity682> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity682>): Map<String, List<Entity682>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity682): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity682")
class Controller682(private val service: Service682) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity682) = service.save(entity)
    @DeleteMapping("/{id}")
