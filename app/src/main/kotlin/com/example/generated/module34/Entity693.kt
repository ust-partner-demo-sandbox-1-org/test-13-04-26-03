...
interface Entity693Repository : org.springframework.data.jpa.repository.JpaRepository<Entity693, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity693>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity693>
}
...
class Service693(private val repo: Entity693Repository) {
    ...
    fun findActive(): List<Entity693> = repo.findByField4(true)
    fun search(query: String): List<Entity693> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity693>): Map<String, List<Entity693>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity693): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity693")
class Controller693(private val service: Service693) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity693) = service.save(entity)
    @DeleteMapping("/{id}")
