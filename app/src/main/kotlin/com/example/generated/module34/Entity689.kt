...
interface Entity689Repository : org.springframework.data.jpa.repository.JpaRepository<Entity689, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity689>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity689>
}
...
class Service689(private val repo: Entity689Repository) {
    ...
    fun findActive(): List<Entity689> = repo.findByField4(true)
    fun search(query: String): List<Entity689> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity689>): Map<String, List<Entity689>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity689): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity689")
class Controller689(private val service: Service689) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity689) = service.save(entity)
    @DeleteMapping("/{id}")
