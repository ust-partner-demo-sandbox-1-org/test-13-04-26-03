...
interface Entity690Repository : org.springframework.data.jpa.repository.JpaRepository<Entity690, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity690>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity690>
}
...
class Service690(private val repo: Entity690Repository) {
    ...
    fun findActive(): List<Entity690> = repo.findByField4(true)
    fun search(query: String): List<Entity690> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity690>): Map<String, List<Entity690>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity690): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity690")
class Controller690(private val service: Service690) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity690) = service.save(entity)
    @DeleteMapping("/{id}")
