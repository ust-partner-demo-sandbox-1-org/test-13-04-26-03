...
interface Entity697Repository : org.springframework.data.jpa.repository.JpaRepository<Entity697, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity697>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity697>
}
...
class Service697(private val repo: Entity697Repository) {
    ...
    fun findActive(): List<Entity697> = repo.findByField4(true)
    fun search(query: String): List<Entity697> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity697>): Map<String, List<Entity697>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity697): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity697")
class Controller697(private val service: Service697) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity697) = service.save(entity)
    @DeleteMapping("/{id}")
