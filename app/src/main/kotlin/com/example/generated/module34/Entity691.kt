...
interface Entity691Repository : org.springframework.data.jpa.repository.JpaRepository<Entity691, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity691>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity691>
}
...
class Service691(private val repo: Entity691Repository) {
    ...
    fun findActive(): List<Entity691> = repo.findByField4(true)
    fun search(query: String): List<Entity691> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity691>): Map<String, List<Entity691>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity691): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity691")
class Controller691(private val service: Service691) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity691) = service.save(entity)
    @DeleteMapping("/{id}")
