...
interface Entity698Repository : org.springframework.data.jpa.repository.JpaRepository<Entity698, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity698>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity698>
}
...
class Service698(private val repo: Entity698Repository) {
    ...
    fun findActive(): List<Entity698> = repo.findByField4(true)
    fun search(query: String): List<Entity698> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity698>): Map<String, List<Entity698>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity698): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity698")
class Controller698(private val service: Service698) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity698) = service.save(entity)
    @DeleteMapping("/{id}")
