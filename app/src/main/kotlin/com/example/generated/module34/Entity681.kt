...
interface Entity681Repository : org.springframework.data.jpa.repository.JpaRepository<Entity681, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity681>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity681>
}
...
class Service681(private val repo: Entity681Repository) {
    ...
    fun findActive(): List<Entity681> = repo.findByField4(true)
    fun search(query: String): List<Entity681> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity681>): Map<String, List<Entity681>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity681): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity681")
class Controller681(private val service: Service681) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity681) = service.save(entity)
    @DeleteMapping("/{id}")
