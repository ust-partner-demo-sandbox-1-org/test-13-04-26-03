...
interface Entity897Repository : org.springframework.data.jpa.repository.JpaRepository<Entity897, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity897>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity897>
}
...
class Service897(private val repo: Entity897Repository) {
    ...
    fun findActive(): List<Entity897> = repo.findByField4(true)
    fun search(query: String): List<Entity897> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity897>): Map<String, List<Entity897>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity897): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity897")
class Controller897(private val service: Service897) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity897) = service.save(entity)
    @DeleteMapping("/{id}")
