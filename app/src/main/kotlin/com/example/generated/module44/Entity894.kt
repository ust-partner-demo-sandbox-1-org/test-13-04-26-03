...
interface Entity894Repository : org.springframework.data.jpa.repository.JpaRepository<Entity894, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity894>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity894>
}
...
class Service894(private val repo: Entity894Repository) {
    ...
    fun findActive(): List<Entity894> = repo.findByField4(true)
    fun search(query: String): List<Entity894> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity894>): Map<String, List<Entity894>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity894): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity894")
class Controller894(private val service: Service894) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity894) = service.save(entity)
    @DeleteMapping("/{id}")
