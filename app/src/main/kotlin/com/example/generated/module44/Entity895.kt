...
interface Entity895Repository : org.springframework.data.jpa.repository.JpaRepository<Entity895, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity895>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity895>
}
...
class Service895(private val repo: Entity895Repository) {
    ...
    fun findActive(): List<Entity895> = repo.findByField4(true)
    fun search(query: String): List<Entity895> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity895>): Map<String, List<Entity895>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity895): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity895")
class Controller895(private val service: Service895) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity895) = service.save(entity)
    @DeleteMapping("/{id}")
