...
interface Entity889Repository : org.springframework.data.jpa.repository.JpaRepository<Entity889, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity889>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity889>
}
...
class Service889(private val repo: Entity889Repository) {
    ...
    fun findActive(): List<Entity889> = repo.findByField4(true)
    fun search(query: String): List<Entity889> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity889>): Map<String, List<Entity889>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity889): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity889")
class Controller889(private val service: Service889) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity889) = service.save(entity)
    @DeleteMapping("/{id}")
