...
interface Entity899Repository : org.springframework.data.jpa.repository.JpaRepository<Entity899, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity899>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity899>
}
...
class Service899(private val repo: Entity899Repository) {
    ...
    fun findActive(): List<Entity899> = repo.findByField4(true)
    fun search(query: String): List<Entity899> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity899>): Map<String, List<Entity899>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity899): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity899")
class Controller899(private val service: Service899) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity899) = service.save(entity)
    @DeleteMapping("/{id}")
