...
interface Entity886Repository : org.springframework.data.jpa.repository.JpaRepository<Entity886, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity886>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity886>
}
...
class Service886(private val repo: Entity886Repository) {
    ...
    fun findActive(): List<Entity886> = repo.findByField4(true)
    fun search(query: String): List<Entity886> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity886>): Map<String, List<Entity886>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity886): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity886")
class Controller886(private val service: Service886) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity886) = service.save(entity)
    @DeleteMapping("/{id}")
