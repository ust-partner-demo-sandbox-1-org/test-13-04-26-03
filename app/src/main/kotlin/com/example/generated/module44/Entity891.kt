...
interface Entity891Repository : org.springframework.data.jpa.repository.JpaRepository<Entity891, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity891>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity891>
}
...
class Service891(private val repo: Entity891Repository) {
    ...
    fun findActive(): List<Entity891> = repo.findByField4(true)
    fun search(query: String): List<Entity891> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity891>): Map<String, List<Entity891>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity891): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity891")
class Controller891(private val service: Service891) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity891) = service.save(entity)
    @DeleteMapping("/{id}")
