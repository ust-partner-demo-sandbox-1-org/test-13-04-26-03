...
interface Entity896Repository : org.springframework.data.jpa.repository.JpaRepository<Entity896, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity896>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity896>
}
...
class Service896(private val repo: Entity896Repository) {
    ...
    fun findActive(): List<Entity896> = repo.findByField4(true)
    fun search(query: String): List<Entity896> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity896>): Map<String, List<Entity896>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity896): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity896")
class Controller896(private val service: Service896) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity896) = service.save(entity)
    @DeleteMapping("/{id}")
