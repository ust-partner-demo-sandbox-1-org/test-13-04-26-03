...
interface Entity390Repository : org.springframework.data.jpa.repository.JpaRepository<Entity390, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity390>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity390>
}
...
class Service390(private val repo: Entity390Repository) {
    ...
    fun findActive(): List<Entity390> = repo.findByField4(true)
    fun search(query: String): List<Entity390> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity390>): Map<String, List<Entity390>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity390): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity390")
class Controller390(private val service: Service390) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity390) = service.save(entity)
    @DeleteMapping("/{id}")
