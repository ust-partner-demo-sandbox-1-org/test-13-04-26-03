...
interface Entity398Repository : org.springframework.data.jpa.repository.JpaRepository<Entity398, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity398>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity398>
}
...
class Service398(private val repo: Entity398Repository) {
    ...
    fun findActive(): List<Entity398> = repo.findByField4(true)
    fun search(query: String): List<Entity398> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity398>): Map<String, List<Entity398>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity398): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity398")
class Controller398(private val service: Service398) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity398) = service.save(entity)
    @DeleteMapping("/{id}")
