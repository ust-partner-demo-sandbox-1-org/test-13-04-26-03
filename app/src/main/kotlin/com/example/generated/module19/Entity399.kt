...
interface Entity399Repository : org.springframework.data.jpa.repository.JpaRepository<Entity399, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity399>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity399>
}
...
class Service399(private val repo: Entity399Repository) {
    ...
    fun findActive(): List<Entity399> = repo.findByField4(true)
    fun search(query: String): List<Entity399> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity399>): Map<String, List<Entity399>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity399): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity399")
class Controller399(private val service: Service399) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity399) = service.save(entity)
    @DeleteMapping("/{id}")
