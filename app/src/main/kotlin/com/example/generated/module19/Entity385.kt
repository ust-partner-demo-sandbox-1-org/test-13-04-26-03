...
interface Entity385Repository : org.springframework.data.jpa.repository.JpaRepository<Entity385, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity385>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity385>
}
...
class Service385(private val repo: Entity385Repository) {
    ...
    fun findActive(): List<Entity385> = repo.findByField4(true)
    fun search(query: String): List<Entity385> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity385>): Map<String, List<Entity385>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity385): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity385")
class Controller385(private val service: Service385) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity385) = service.save(entity)
    @DeleteMapping("/{id}")
