...
interface Entity397Repository : org.springframework.data.jpa.repository.JpaRepository<Entity397, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity397>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity397>
}
...
class Service397(private val repo: Entity397Repository) {
    ...
    fun findActive(): List<Entity397> = repo.findByField4(true)
    fun search(query: String): List<Entity397> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity397>): Map<String, List<Entity397>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity397): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity397")
class Controller397(private val service: Service397) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity397) = service.save(entity)
    @DeleteMapping("/{id}")
