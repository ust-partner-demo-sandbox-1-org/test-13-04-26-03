...
interface Entity391Repository : org.springframework.data.jpa.repository.JpaRepository<Entity391, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity391>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity391>
}
...
class Service391(private val repo: Entity391Repository) {
    ...
    fun findActive(): List<Entity391> = repo.findByField4(true)
    fun search(query: String): List<Entity391> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity391>): Map<String, List<Entity391>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity391): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity391")
class Controller391(private val service: Service391) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity391) = service.save(entity)
    @DeleteMapping("/{id}")
