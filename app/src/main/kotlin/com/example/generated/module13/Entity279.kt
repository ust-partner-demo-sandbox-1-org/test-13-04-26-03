...
interface Entity279Repository : org.springframework.data.jpa.repository.JpaRepository<Entity279, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity279>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity279>
}
...
class Service279(private val repo: Entity279Repository) {
    ...
    fun findActive(): List<Entity279> = repo.findByField4(true)
    fun search(query: String): List<Entity279> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity279>): Map<String, List<Entity279>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity279): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity279")
class Controller279(private val service: Service279) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity279) = service.save(entity)
    @DeleteMapping("/{id}")
