...
interface Entity260Repository : org.springframework.data.jpa.repository.JpaRepository<Entity260, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity260>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity260>
}
...
class Service260(private val repo: Entity260Repository) {
    ...
    fun findActive(): List<Entity260> = repo.findByField4(true)
    fun search(query: String): List<Entity260> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity260>): Map<String, List<Entity260>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity260): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity260")
class Controller260(private val service: Service260) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity260) = service.save(entity)
    @DeleteMapping("/{id}")
