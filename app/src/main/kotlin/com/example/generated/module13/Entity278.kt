...
interface Entity278Repository : org.springframework.data.jpa.repository.JpaRepository<Entity278, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity278>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity278>
}
...
class Service278(private val repo: Entity278Repository) {
    ...
    fun findActive(): List<Entity278> = repo.findByField4(true)
    fun search(query: String): List<Entity278> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity278>): Map<String, List<Entity278>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity278): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity278")
class Controller278(private val service: Service278) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity278) = service.save(entity)
    @DeleteMapping("/{id}")
