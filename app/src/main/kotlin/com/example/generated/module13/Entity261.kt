...
interface Entity261Repository : org.springframework.data.jpa.repository.JpaRepository<Entity261, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity261>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity261>
}
...
class Service261(private val repo: Entity261Repository) {
    ...
    fun findActive(): List<Entity261> = repo.findByField4(true)
    fun search(query: String): List<Entity261> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity261>): Map<String, List<Entity261>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity261): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity261")
class Controller261(private val service: Service261) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity261) = service.save(entity)
    @DeleteMapping("/{id}")
