...
interface Entity262Repository : org.springframework.data.jpa.repository.JpaRepository<Entity262, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity262>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity262>
}
...
class Service262(private val repo: Entity262Repository) {
    ...
    fun findActive(): List<Entity262> = repo.findByField4(true)
    fun search(query: String): List<Entity262> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity262>): Map<String, List<Entity262>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity262): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity262")
class Controller262(private val service: Service262) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity262) = service.save(entity)
    @DeleteMapping("/{id}")
