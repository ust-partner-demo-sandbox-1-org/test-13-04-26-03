...
interface Entity270Repository : org.springframework.data.jpa.repository.JpaRepository<Entity270, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity270>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity270>
}
...
class Service270(private val repo: Entity270Repository) {
    ...
    fun findActive(): List<Entity270> = repo.findByField4(true)
    fun search(query: String): List<Entity270> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity270>): Map<String, List<Entity270>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity270): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity270")
class Controller270(private val service: Service270) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity270) = service.save(entity)
    @DeleteMapping("/{id}")
