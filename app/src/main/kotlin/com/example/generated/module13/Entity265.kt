...
interface Entity265Repository : org.springframework.data.jpa.repository.JpaRepository<Entity265, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity265>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity265>
}
...
class Service265(private val repo: Entity265Repository) {
    ...
    fun findActive(): List<Entity265> = repo.findByField4(true)
    fun search(query: String): List<Entity265> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity265>): Map<String, List<Entity265>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity265): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity265")
class Controller265(private val service: Service265) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity265) = service.save(entity)
    @DeleteMapping("/{id}")
