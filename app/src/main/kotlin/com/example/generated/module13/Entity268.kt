...
interface Entity268Repository : org.springframework.data.jpa.repository.JpaRepository<Entity268, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity268>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity268>
}
...
class Service268(private val repo: Entity268Repository) {
    ...
    fun findActive(): List<Entity268> = repo.findByField4(true)
    fun search(query: String): List<Entity268> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity268>): Map<String, List<Entity268>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity268): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity268")
class Controller268(private val service: Service268) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity268) = service.save(entity)
    @DeleteMapping("/{id}")
