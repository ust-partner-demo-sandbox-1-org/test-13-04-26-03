...
interface Entity221Repository : org.springframework.data.jpa.repository.JpaRepository<Entity221, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity221>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity221>
}
...
class Service221(private val repo: Entity221Repository) {
    ...
    fun findActive(): List<Entity221> = repo.findByField4(true)
    fun search(query: String): List<Entity221> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity221>): Map<String, List<Entity221>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity221): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity221")
class Controller221(private val service: Service221) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity221) = service.save(entity)
    @DeleteMapping("/{id}")
