...
interface Entity224Repository : org.springframework.data.jpa.repository.JpaRepository<Entity224, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity224>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity224>
}
...
class Service224(private val repo: Entity224Repository) {
    ...
    fun findActive(): List<Entity224> = repo.findByField4(true)
    fun search(query: String): List<Entity224> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity224>): Map<String, List<Entity224>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity224): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity224")
class Controller224(private val service: Service224) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity224) = service.save(entity)
    @DeleteMapping("/{id}")
