...
interface Entity236Repository : org.springframework.data.jpa.repository.JpaRepository<Entity236, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity236>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity236>
}
...
class Service236(private val repo: Entity236Repository) {
    ...
    fun findActive(): List<Entity236> = repo.findByField4(true)
    fun search(query: String): List<Entity236> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity236>): Map<String, List<Entity236>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity236): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity236")
class Controller236(private val service: Service236) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity236) = service.save(entity)
    @DeleteMapping("/{id}")
