...
interface Entity234Repository : org.springframework.data.jpa.repository.JpaRepository<Entity234, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity234>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity234>
}
...
class Service234(private val repo: Entity234Repository) {
    ...
    fun findActive(): List<Entity234> = repo.findByField4(true)
    fun search(query: String): List<Entity234> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity234>): Map<String, List<Entity234>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity234): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity234")
class Controller234(private val service: Service234) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity234) = service.save(entity)
    @DeleteMapping("/{id}")
