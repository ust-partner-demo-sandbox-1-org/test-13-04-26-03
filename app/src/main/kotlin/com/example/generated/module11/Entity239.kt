...
interface Entity239Repository : org.springframework.data.jpa.repository.JpaRepository<Entity239, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity239>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity239>
}
...
class Service239(private val repo: Entity239Repository) {
    ...
    fun findActive(): List<Entity239> = repo.findByField4(true)
    fun search(query: String): List<Entity239> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity239>): Map<String, List<Entity239>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity239): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity239")
class Controller239(private val service: Service239) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity239) = service.save(entity)
    @DeleteMapping("/{id}")
