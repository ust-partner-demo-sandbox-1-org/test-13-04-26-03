...
interface Entity238Repository : org.springframework.data.jpa.repository.JpaRepository<Entity238, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity238>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity238>
}
...
class Service238(private val repo: Entity238Repository) {
    ...
    fun findActive(): List<Entity238> = repo.findByField4(true)
    fun search(query: String): List<Entity238> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity238>): Map<String, List<Entity238>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity238): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity238")
class Controller238(private val service: Service238) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity238) = service.save(entity)
    @DeleteMapping("/{id}")
