...
interface Entity222Repository : org.springframework.data.jpa.repository.JpaRepository<Entity222, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity222>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity222>
}
...
class Service222(private val repo: Entity222Repository) {
    ...
    fun findActive(): List<Entity222> = repo.findByField4(true)
    fun search(query: String): List<Entity222> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity222>): Map<String, List<Entity222>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity222): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity222")
class Controller222(private val service: Service222) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity222) = service.save(entity)
    @DeleteMapping("/{id}")
