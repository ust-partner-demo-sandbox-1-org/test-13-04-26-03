...
interface Entity231Repository : org.springframework.data.jpa.repository.JpaRepository<Entity231, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity231>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity231>
}
...
class Service231(private val repo: Entity231Repository) {
    ...
    fun findActive(): List<Entity231> = repo.findByField4(true)
    fun search(query: String): List<Entity231> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity231>): Map<String, List<Entity231>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity231): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity231")
class Controller231(private val service: Service231) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity231) = service.save(entity)
    @DeleteMapping("/{id}")
