...
interface Entity220Repository : org.springframework.data.jpa.repository.JpaRepository<Entity220, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity220>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity220>
}
...
class Service220(private val repo: Entity220Repository) {
    ...
    fun findActive(): List<Entity220> = repo.findByField4(true)
    fun search(query: String): List<Entity220> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity220>): Map<String, List<Entity220>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity220): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity220")
class Controller220(private val service: Service220) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity220) = service.save(entity)
    @DeleteMapping("/{id}")
