...
interface Entity237Repository : org.springframework.data.jpa.repository.JpaRepository<Entity237, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity237>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity237>
}
...
class Service237(private val repo: Entity237Repository) {
    ...
    fun findActive(): List<Entity237> = repo.findByField4(true)
    fun search(query: String): List<Entity237> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity237>): Map<String, List<Entity237>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity237): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity237")
class Controller237(private val service: Service237) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity237) = service.save(entity)
    @DeleteMapping("/{id}")
