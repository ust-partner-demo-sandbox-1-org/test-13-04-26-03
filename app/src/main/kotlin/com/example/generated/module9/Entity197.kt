...
interface Entity197Repository : org.springframework.data.jpa.repository.JpaRepository<Entity197, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity197>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity197>
}
...
class Service197(private val repo: Entity197Repository) {
    ...
    fun findActive(): List<Entity197> = repo.findByField4(true)
    fun search(query: String): List<Entity197> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity197>): Map<String, List<Entity197>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity197): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity197")
class Controller197(private val service: Service197) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity197) = service.save(entity)
    @DeleteMapping("/{id}")
