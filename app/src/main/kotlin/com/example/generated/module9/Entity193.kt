...
interface Entity193Repository : org.springframework.data.jpa.repository.JpaRepository<Entity193, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity193>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity193>
}
...
class Service193(private val repo: Entity193Repository) {
    ...
    fun findActive(): List<Entity193> = repo.findByField4(true)
    fun search(query: String): List<Entity193> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity193>): Map<String, List<Entity193>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity193): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity193")
class Controller193(private val service: Service193) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity193) = service.save(entity)
    @DeleteMapping("/{id}")
