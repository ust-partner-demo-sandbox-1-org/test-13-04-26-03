...
interface Entity196Repository : org.springframework.data.jpa.repository.JpaRepository<Entity196, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity196>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity196>
}
...
class Service196(private val repo: Entity196Repository) {
    ...
    fun findActive(): List<Entity196> = repo.findByField4(true)
    fun search(query: String): List<Entity196> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity196>): Map<String, List<Entity196>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity196): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity196")
class Controller196(private val service: Service196) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity196) = service.save(entity)
    @DeleteMapping("/{id}")
