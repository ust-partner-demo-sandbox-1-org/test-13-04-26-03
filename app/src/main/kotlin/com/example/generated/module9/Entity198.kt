...
interface Entity198Repository : org.springframework.data.jpa.repository.JpaRepository<Entity198, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity198>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity198>
}
...
class Service198(private val repo: Entity198Repository) {
    ...
    fun findActive(): List<Entity198> = repo.findByField4(true)
    fun search(query: String): List<Entity198> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity198>): Map<String, List<Entity198>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity198): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity198")
class Controller198(private val service: Service198) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity198) = service.save(entity)
    @DeleteMapping("/{id}")
