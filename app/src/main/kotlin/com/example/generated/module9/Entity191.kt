...
interface Entity191Repository : org.springframework.data.jpa.repository.JpaRepository<Entity191, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity191>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity191>
}
...
class Service191(private val repo: Entity191Repository) {
    ...
    fun findActive(): List<Entity191> = repo.findByField4(true)
    fun search(query: String): List<Entity191> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity191>): Map<String, List<Entity191>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity191): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity191")
class Controller191(private val service: Service191) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity191) = service.save(entity)
    @DeleteMapping("/{id}")
