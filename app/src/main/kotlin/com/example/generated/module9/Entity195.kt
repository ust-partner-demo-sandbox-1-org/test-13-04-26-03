...
interface Entity195Repository : org.springframework.data.jpa.repository.JpaRepository<Entity195, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity195>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity195>
}
...
class Service195(private val repo: Entity195Repository) {
    ...
    fun findActive(): List<Entity195> = repo.findByField4(true)
    fun search(query: String): List<Entity195> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity195>): Map<String, List<Entity195>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity195): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity195")
class Controller195(private val service: Service195) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity195) = service.save(entity)
    @DeleteMapping("/{id}")
