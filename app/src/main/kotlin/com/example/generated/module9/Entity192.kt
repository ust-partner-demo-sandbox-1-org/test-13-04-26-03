...
interface Entity192Repository : org.springframework.data.jpa.repository.JpaRepository<Entity192, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity192>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity192>
}
...
class Service192(private val repo: Entity192Repository) {
    ...
    fun findActive(): List<Entity192> = repo.findByField4(true)
    fun search(query: String): List<Entity192> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity192>): Map<String, List<Entity192>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity192): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity192")
class Controller192(private val service: Service192) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity192) = service.save(entity)
    @DeleteMapping("/{id}")
