...
interface Entity187Repository : org.springframework.data.jpa.repository.JpaRepository<Entity187, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity187>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity187>
}
...
class Service187(private val repo: Entity187Repository) {
    ...
    fun findActive(): List<Entity187> = repo.findByField4(true)
    fun search(query: String): List<Entity187> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity187>): Map<String, List<Entity187>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity187): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity187")
class Controller187(private val service: Service187) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity187) = service.save(entity)
    @DeleteMapping("/{id}")
