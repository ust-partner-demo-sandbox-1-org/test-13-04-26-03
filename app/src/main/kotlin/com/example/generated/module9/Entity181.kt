...
interface Entity181Repository : org.springframework.data.jpa.repository.JpaRepository<Entity181, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity181>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity181>
}
...
class Service181(private val repo: Entity181Repository) {
    ...
    fun findActive(): List<Entity181> = repo.findByField4(true)
    fun search(query: String): List<Entity181> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity181>): Map<String, List<Entity181>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity181): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity181")
class Controller181(private val service: Service181) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity181) = service.save(entity)
    @DeleteMapping("/{id}")
