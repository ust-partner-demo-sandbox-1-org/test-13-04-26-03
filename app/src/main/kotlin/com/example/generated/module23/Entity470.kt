...
interface Entity470Repository : org.springframework.data.jpa.repository.JpaRepository<Entity470, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity470>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity470>
}
...
class Service470(private val repo: Entity470Repository) {
    ...
    fun findActive(): List<Entity470> = repo.findByField4(true)
    fun search(query: String): List<Entity470> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity470>): Map<String, List<Entity470>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity470): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity470")
class Controller470(private val service: Service470) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity470) = service.save(entity)
    @DeleteMapping("/{id}")
