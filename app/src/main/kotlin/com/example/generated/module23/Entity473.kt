...
interface Entity473Repository : org.springframework.data.jpa.repository.JpaRepository<Entity473, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity473>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity473>
}
...
class Service473(private val repo: Entity473Repository) {
    ...
    fun findActive(): List<Entity473> = repo.findByField4(true)
    fun search(query: String): List<Entity473> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity473>): Map<String, List<Entity473>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity473): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity473")
class Controller473(private val service: Service473) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity473) = service.save(entity)
    @DeleteMapping("/{id}")
