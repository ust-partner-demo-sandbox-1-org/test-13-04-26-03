...
interface Entity471Repository : org.springframework.data.jpa.repository.JpaRepository<Entity471, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity471>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity471>
}
...
class Service471(private val repo: Entity471Repository) {
    ...
    fun findActive(): List<Entity471> = repo.findByField4(true)
    fun search(query: String): List<Entity471> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity471>): Map<String, List<Entity471>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity471): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity471")
class Controller471(private val service: Service471) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity471) = service.save(entity)
    @DeleteMapping("/{id}")
