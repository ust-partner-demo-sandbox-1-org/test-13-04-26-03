...
interface Entity476Repository : org.springframework.data.jpa.repository.JpaRepository<Entity476, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity476>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity476>
}
...
class Service476(private val repo: Entity476Repository) {
    ...
    fun findActive(): List<Entity476> = repo.findByField4(true)
    fun search(query: String): List<Entity476> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity476>): Map<String, List<Entity476>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity476): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity476")
class Controller476(private val service: Service476) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity476) = service.save(entity)
    @DeleteMapping("/{id}")
