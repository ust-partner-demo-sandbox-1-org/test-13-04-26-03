...
interface Entity460Repository : org.springframework.data.jpa.repository.JpaRepository<Entity460, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity460>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity460>
}
...
class Service460(private val repo: Entity460Repository) {
    ...
    fun findActive(): List<Entity460> = repo.findByField4(true)
    fun search(query: String): List<Entity460> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity460>): Map<String, List<Entity460>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity460): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity460")
class Controller460(private val service: Service460) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity460) = service.save(entity)
    @DeleteMapping("/{id}")
