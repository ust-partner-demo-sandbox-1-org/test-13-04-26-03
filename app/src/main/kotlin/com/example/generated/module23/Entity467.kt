...
interface Entity467Repository : org.springframework.data.jpa.repository.JpaRepository<Entity467, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity467>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity467>
}
...
class Service467(private val repo: Entity467Repository) {
    ...
    fun findActive(): List<Entity467> = repo.findByField4(true)
    fun search(query: String): List<Entity467> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity467>): Map<String, List<Entity467>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity467): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity467")
class Controller467(private val service: Service467) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity467) = service.save(entity)
    @DeleteMapping("/{id}")
