...
interface Entity465Repository : org.springframework.data.jpa.repository.JpaRepository<Entity465, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity465>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity465>
}
...
class Service465(private val repo: Entity465Repository) {
    ...
    fun findActive(): List<Entity465> = repo.findByField4(true)
    fun search(query: String): List<Entity465> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity465>): Map<String, List<Entity465>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity465): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity465")
class Controller465(private val service: Service465) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity465) = service.save(entity)
    @DeleteMapping("/{id}")
