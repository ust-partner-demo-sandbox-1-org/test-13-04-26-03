...
interface Entity479Repository : org.springframework.data.jpa.repository.JpaRepository<Entity479, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity479>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity479>
}
...
class Service479(private val repo: Entity479Repository) {
    ...
    fun findActive(): List<Entity479> = repo.findByField4(true)
    fun search(query: String): List<Entity479> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity479>): Map<String, List<Entity479>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity479): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity479")
class Controller479(private val service: Service479) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity479) = service.save(entity)
    @DeleteMapping("/{id}")
