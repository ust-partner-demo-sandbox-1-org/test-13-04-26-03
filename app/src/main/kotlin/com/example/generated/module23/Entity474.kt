...
interface Entity474Repository : org.springframework.data.jpa.repository.JpaRepository<Entity474, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity474>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity474>
}
...
class Service474(private val repo: Entity474Repository) {
    ...
    fun findActive(): List<Entity474> = repo.findByField4(true)
    fun search(query: String): List<Entity474> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity474>): Map<String, List<Entity474>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity474): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity474")
class Controller474(private val service: Service474) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity474) = service.save(entity)
    @DeleteMapping("/{id}")
