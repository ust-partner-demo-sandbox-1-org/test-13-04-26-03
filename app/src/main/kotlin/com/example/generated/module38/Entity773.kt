...
interface Entity773Repository : org.springframework.data.jpa.repository.JpaRepository<Entity773, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity773>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity773>
}
...
class Service773(private val repo: Entity773Repository) {
    ...
    fun findActive(): List<Entity773> = repo.findByField4(true)
    fun search(query: String): List<Entity773> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity773>): Map<String, List<Entity773>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity773): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity773")
class Controller773(private val service: Service773) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity773) = service.save(entity)
    @DeleteMapping("/{id}")
