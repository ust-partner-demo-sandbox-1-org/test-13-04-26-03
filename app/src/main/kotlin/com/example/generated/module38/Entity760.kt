...
interface Entity760Repository : org.springframework.data.jpa.repository.JpaRepository<Entity760, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity760>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity760>
}
...
class Service760(private val repo: Entity760Repository) {
    ...
    fun findActive(): List<Entity760> = repo.findByField4(true)
    fun search(query: String): List<Entity760> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity760>): Map<String, List<Entity760>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity760): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity760")
class Controller760(private val service: Service760) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity760) = service.save(entity)
    @DeleteMapping("/{id}")
