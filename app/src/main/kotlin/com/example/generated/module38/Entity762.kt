...
interface Entity762Repository : org.springframework.data.jpa.repository.JpaRepository<Entity762, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity762>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity762>
}
...
class Service762(private val repo: Entity762Repository) {
    ...
    fun findActive(): List<Entity762> = repo.findByField4(true)
    fun search(query: String): List<Entity762> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity762>): Map<String, List<Entity762>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity762): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity762")
class Controller762(private val service: Service762) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity762) = service.save(entity)
    @DeleteMapping("/{id}")
