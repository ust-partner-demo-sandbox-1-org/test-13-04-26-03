...
interface Entity778Repository : org.springframework.data.jpa.repository.JpaRepository<Entity778, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity778>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity778>
}
...
class Service778(private val repo: Entity778Repository) {
    ...
    fun findActive(): List<Entity778> = repo.findByField4(true)
    fun search(query: String): List<Entity778> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity778>): Map<String, List<Entity778>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity778): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity778")
class Controller778(private val service: Service778) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity778) = service.save(entity)
    @DeleteMapping("/{id}")
