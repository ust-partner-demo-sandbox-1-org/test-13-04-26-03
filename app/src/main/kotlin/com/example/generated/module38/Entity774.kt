...
interface Entity774Repository : org.springframework.data.jpa.repository.JpaRepository<Entity774, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity774>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity774>
}
...
class Service774(private val repo: Entity774Repository) {
    ...
    fun findActive(): List<Entity774> = repo.findByField4(true)
    fun search(query: String): List<Entity774> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity774>): Map<String, List<Entity774>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity774): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity774")
class Controller774(private val service: Service774) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity774) = service.save(entity)
    @DeleteMapping("/{id}")
