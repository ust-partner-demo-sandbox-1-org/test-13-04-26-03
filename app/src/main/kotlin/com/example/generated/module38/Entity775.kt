...
interface Entity775Repository : org.springframework.data.jpa.repository.JpaRepository<Entity775, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity775>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity775>
}
...
class Service775(private val repo: Entity775Repository) {
    ...
    fun findActive(): List<Entity775> = repo.findByField4(true)
    fun search(query: String): List<Entity775> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity775>): Map<String, List<Entity775>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity775): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity775")
class Controller775(private val service: Service775) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity775) = service.save(entity)
    @DeleteMapping("/{id}")
