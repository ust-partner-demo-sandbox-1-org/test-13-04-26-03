...
interface Entity761Repository : org.springframework.data.jpa.repository.JpaRepository<Entity761, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity761>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity761>
}
...
class Service761(private val repo: Entity761Repository) {
    ...
    fun findActive(): List<Entity761> = repo.findByField4(true)
    fun search(query: String): List<Entity761> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity761>): Map<String, List<Entity761>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity761): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity761")
class Controller761(private val service: Service761) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity761) = service.save(entity)
    @DeleteMapping("/{id}")
