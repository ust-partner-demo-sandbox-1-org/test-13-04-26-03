...
interface Entity766Repository : org.springframework.data.jpa.repository.JpaRepository<Entity766, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity766>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity766>
}
...
class Service766(private val repo: Entity766Repository) {
    ...
    fun findActive(): List<Entity766> = repo.findByField4(true)
    fun search(query: String): List<Entity766> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity766>): Map<String, List<Entity766>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity766): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity766")
class Controller766(private val service: Service766) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity766) = service.save(entity)
    @DeleteMapping("/{id}")
