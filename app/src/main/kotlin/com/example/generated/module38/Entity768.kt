...
interface Entity768Repository : org.springframework.data.jpa.repository.JpaRepository<Entity768, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity768>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity768>
}
...
class Service768(private val repo: Entity768Repository) {
    ...
    fun findActive(): List<Entity768> = repo.findByField4(true)
    fun search(query: String): List<Entity768> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity768>): Map<String, List<Entity768>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity768): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity768")
class Controller768(private val service: Service768) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity768) = service.save(entity)
    @DeleteMapping("/{id}")
