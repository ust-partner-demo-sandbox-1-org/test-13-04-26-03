...
interface Entity765Repository : org.springframework.data.jpa.repository.JpaRepository<Entity765, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity765>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity765>
}
...
class Service765(private val repo: Entity765Repository) {
    ...
    fun findActive(): List<Entity765> = repo.findByField4(true)
    fun search(query: String): List<Entity765> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity765>): Map<String, List<Entity765>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity765): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity765")
class Controller765(private val service: Service765) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity765) = service.save(entity)
    @DeleteMapping("/{id}")
