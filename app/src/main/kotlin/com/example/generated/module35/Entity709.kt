...
interface Entity709Repository : org.springframework.data.jpa.repository.JpaRepository<Entity709, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity709>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity709>
}
...
class Service709(private val repo: Entity709Repository) {
    ...
    fun findActive(): List<Entity709> = repo.findByField4(true)
    fun search(query: String): List<Entity709> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity709>): Map<String, List<Entity709>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity709): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity709")
class Controller709(private val service: Service709) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity709) = service.save(entity)
    @DeleteMapping("/{id}")
