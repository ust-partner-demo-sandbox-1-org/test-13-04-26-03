...
interface Entity707Repository : org.springframework.data.jpa.repository.JpaRepository<Entity707, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity707>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity707>
}
...
class Service707(private val repo: Entity707Repository) {
    ...
    fun findActive(): List<Entity707> = repo.findByField4(true)
    fun search(query: String): List<Entity707> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity707>): Map<String, List<Entity707>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity707): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity707")
class Controller707(private val service: Service707) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity707) = service.save(entity)
    @DeleteMapping("/{id}")
