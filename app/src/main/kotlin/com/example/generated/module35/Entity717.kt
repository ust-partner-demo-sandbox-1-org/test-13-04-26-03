...
interface Entity717Repository : org.springframework.data.jpa.repository.JpaRepository<Entity717, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity717>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity717>
}
...
class Service717(private val repo: Entity717Repository) {
    ...
    fun findActive(): List<Entity717> = repo.findByField4(true)
    fun search(query: String): List<Entity717> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity717>): Map<String, List<Entity717>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity717): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity717")
class Controller717(private val service: Service717) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity717) = service.save(entity)
    @DeleteMapping("/{id}")
