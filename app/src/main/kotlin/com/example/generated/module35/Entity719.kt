...
interface Entity719Repository : org.springframework.data.jpa.repository.JpaRepository<Entity719, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity719>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity719>
}
...
class Service719(private val repo: Entity719Repository) {
    ...
    fun findActive(): List<Entity719> = repo.findByField4(true)
    fun search(query: String): List<Entity719> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity719>): Map<String, List<Entity719>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity719): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity719")
class Controller719(private val service: Service719) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity719) = service.save(entity)
    @DeleteMapping("/{id}")
