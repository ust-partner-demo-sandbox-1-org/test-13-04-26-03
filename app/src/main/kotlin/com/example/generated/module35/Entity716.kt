...
interface Entity716Repository : org.springframework.data.jpa.repository.JpaRepository<Entity716, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity716>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity716>
}
...
class Service716(private val repo: Entity716Repository) {
    ...
    fun findActive(): List<Entity716> = repo.findByField4(true)
    fun search(query: String): List<Entity716> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity716>): Map<String, List<Entity716>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity716): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity716")
class Controller716(private val service: Service716) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity716) = service.save(entity)
    @DeleteMapping("/{id}")
