...
interface Entity702Repository : org.springframework.data.jpa.repository.JpaRepository<Entity702, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity702>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity702>
}
...
class Service702(private val repo: Entity702Repository) {
    ...
    fun findActive(): List<Entity702> = repo.findByField4(true)
    fun search(query: String): List<Entity702> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity702>): Map<String, List<Entity702>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity702): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity702")
class Controller702(private val service: Service702) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity702) = service.save(entity)
    @DeleteMapping("/{id}")
