...
interface Entity703Repository : org.springframework.data.jpa.repository.JpaRepository<Entity703, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity703>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity703>
}
...
class Service703(private val repo: Entity703Repository) {
    ...
    fun findActive(): List<Entity703> = repo.findByField4(true)
    fun search(query: String): List<Entity703> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity703>): Map<String, List<Entity703>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity703): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity703")
class Controller703(private val service: Service703) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity703) = service.save(entity)
    @DeleteMapping("/{id}")
