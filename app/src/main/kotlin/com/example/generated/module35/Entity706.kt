...
interface Entity706Repository : org.springframework.data.jpa.repository.JpaRepository<Entity706, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity706>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity706>
}
...
class Service706(private val repo: Entity706Repository) {
    ...
    fun findActive(): List<Entity706> = repo.findByField4(true)
    fun search(query: String): List<Entity706> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity706>): Map<String, List<Entity706>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity706): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity706")
class Controller706(private val service: Service706) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity706) = service.save(entity)
    @DeleteMapping("/{id}")
