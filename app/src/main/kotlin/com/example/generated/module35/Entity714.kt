...
interface Entity714Repository : org.springframework.data.jpa.repository.JpaRepository<Entity714, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity714>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity714>
}
...
class Service714(private val repo: Entity714Repository) {
    ...
    fun findActive(): List<Entity714> = repo.findByField4(true)
    fun search(query: String): List<Entity714> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity714>): Map<String, List<Entity714>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity714): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity714")
class Controller714(private val service: Service714) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity714) = service.save(entity)
    @DeleteMapping("/{id}")
