...
interface Entity711Repository : org.springframework.data.jpa.repository.JpaRepository<Entity711, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity711>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity711>
}
...
class Service711(private val repo: Entity711Repository) {
    ...
    fun findActive(): List<Entity711> = repo.findByField4(true)
    fun search(query: String): List<Entity711> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity711>): Map<String, List<Entity711>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity711): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity711")
class Controller711(private val service: Service711) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity711) = service.save(entity)
    @DeleteMapping("/{id}")
