...
interface Entity36Repository : org.springframework.data.jpa.repository.JpaRepository<Entity36, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity36>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity36>
}
...
class Service36(private val repo: Entity36Repository) {
    ...
    fun findActive(): List<Entity36> = repo.findByField4(true)
    fun search(query: String): List<Entity36> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity36>): Map<String, List<Entity36>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity36): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity36")
class Controller36(private val service: Service36) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity36) = service.save(entity)
    @DeleteMapping("/{id}")
