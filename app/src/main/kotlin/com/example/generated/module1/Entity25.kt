...
interface Entity25Repository : org.springframework.data.jpa.repository.JpaRepository<Entity25, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity25>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity25>
}
...
class Service25(private val repo: Entity25Repository) {
    ...
    fun findActive(): List<Entity25> = repo.findByField4(true)
    fun search(query: String): List<Entity25> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity25>): Map<String, List<Entity25>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity25): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity25")
class Controller25(private val service: Service25) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity25) = service.save(entity)
    @DeleteMapping("/{id}")
