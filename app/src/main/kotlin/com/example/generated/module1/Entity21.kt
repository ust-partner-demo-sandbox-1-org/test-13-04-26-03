...
interface Entity21Repository : org.springframework.data.jpa.repository.JpaRepository<Entity21, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity21>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity21>
}
...
class Service21(private val repo: Entity21Repository) {
    ...
    fun findActive(): List<Entity21> = repo.findByField4(true)
    fun search(query: String): List<Entity21> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity21>): Map<String, List<Entity21>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity21): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity21")
class Controller21(private val service: Service21) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity21) = service.save(entity)
    @DeleteMapping("/{id}")
