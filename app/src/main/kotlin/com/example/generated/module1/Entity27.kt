...
interface Entity27Repository : org.springframework.data.jpa.repository.JpaRepository<Entity27, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity27>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity27>
}
...
class Service27(private val repo: Entity27Repository) {
    ...
    fun findActive(): List<Entity27> = repo.findByField4(true)
    fun search(query: String): List<Entity27> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity27>): Map<String, List<Entity27>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity27): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity27")
class Controller27(private val service: Service27) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity27) = service.save(entity)
    @DeleteMapping("/{id}")
