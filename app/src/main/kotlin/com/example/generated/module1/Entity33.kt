...
interface Entity33Repository : org.springframework.data.jpa.repository.JpaRepository<Entity33, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity33>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity33>
}
...
class Service33(private val repo: Entity33Repository) {
    ...
    fun findActive(): List<Entity33> = repo.findByField4(true)
    fun search(query: String): List<Entity33> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity33>): Map<String, List<Entity33>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity33): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity33")
class Controller33(private val service: Service33) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity33) = service.save(entity)
    @DeleteMapping("/{id}")
