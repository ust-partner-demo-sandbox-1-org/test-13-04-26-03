...
interface Entity32Repository : org.springframework.data.jpa.repository.JpaRepository<Entity32, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity32>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity32>
}
...
class Service32(private val repo: Entity32Repository) {
    ...
    fun findActive(): List<Entity32> = repo.findByField4(true)
    fun search(query: String): List<Entity32> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity32>): Map<String, List<Entity32>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity32): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity32")
class Controller32(private val service: Service32) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity32) = service.save(entity)
    @DeleteMapping("/{id}")
