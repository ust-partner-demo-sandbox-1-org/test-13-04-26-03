...
interface Entity37Repository : org.springframework.data.jpa.repository.JpaRepository<Entity37, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity37>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity37>
}
...
class Service37(private val repo: Entity37Repository) {
    ...
    fun findActive(): List<Entity37> = repo.findByField4(true)
    fun search(query: String): List<Entity37> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity37>): Map<String, List<Entity37>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity37): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity37")
class Controller37(private val service: Service37) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity37) = service.save(entity)
    @DeleteMapping("/{id}")
