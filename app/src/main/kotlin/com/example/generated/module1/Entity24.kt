...
interface Entity24Repository : org.springframework.data.jpa.repository.JpaRepository<Entity24, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity24>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity24>
}
...
class Service24(private val repo: Entity24Repository) {
    ...
    fun findActive(): List<Entity24> = repo.findByField4(true)
    fun search(query: String): List<Entity24> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity24>): Map<String, List<Entity24>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity24): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity24")
class Controller24(private val service: Service24) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity24) = service.save(entity)
    @DeleteMapping("/{id}")
