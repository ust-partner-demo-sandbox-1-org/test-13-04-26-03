...
interface Entity35Repository : org.springframework.data.jpa.repository.JpaRepository<Entity35, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity35>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity35>
}
...
class Service35(private val repo: Entity35Repository) {
    ...
    fun findActive(): List<Entity35> = repo.findByField4(true)
    fun search(query: String): List<Entity35> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity35>): Map<String, List<Entity35>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity35): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity35")
class Controller35(private val service: Service35) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity35) = service.save(entity)
    @DeleteMapping("/{id}")
