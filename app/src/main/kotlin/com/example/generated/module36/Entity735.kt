...
interface Entity735Repository : org.springframework.data.jpa.repository.JpaRepository<Entity735, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity735>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity735>
}
...
class Service735(private val repo: Entity735Repository) {
    ...
    fun findActive(): List<Entity735> = repo.findByField4(true)
    fun search(query: String): List<Entity735> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity735>): Map<String, List<Entity735>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity735): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity735")
class Controller735(private val service: Service735) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity735) = service.save(entity)
    @DeleteMapping("/{id}")
