...
interface Entity721Repository : org.springframework.data.jpa.repository.JpaRepository<Entity721, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity721>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity721>
}
...
class Service721(private val repo: Entity721Repository) {
    ...
    fun findActive(): List<Entity721> = repo.findByField4(true)
    fun search(query: String): List<Entity721> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity721>): Map<String, List<Entity721>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity721): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity721")
class Controller721(private val service: Service721) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity721) = service.save(entity)
    @DeleteMapping("/{id}")
