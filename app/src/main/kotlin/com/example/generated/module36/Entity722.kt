...
interface Entity722Repository : org.springframework.data.jpa.repository.JpaRepository<Entity722, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity722>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity722>
}
...
class Service722(private val repo: Entity722Repository) {
    ...
    fun findActive(): List<Entity722> = repo.findByField4(true)
    fun search(query: String): List<Entity722> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity722>): Map<String, List<Entity722>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity722): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity722")
class Controller722(private val service: Service722) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity722) = service.save(entity)
    @DeleteMapping("/{id}")
