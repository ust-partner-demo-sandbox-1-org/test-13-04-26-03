...
interface Entity727Repository : org.springframework.data.jpa.repository.JpaRepository<Entity727, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity727>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity727>
}
...
class Service727(private val repo: Entity727Repository) {
    ...
    fun findActive(): List<Entity727> = repo.findByField4(true)
    fun search(query: String): List<Entity727> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity727>): Map<String, List<Entity727>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity727): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity727")
class Controller727(private val service: Service727) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity727) = service.save(entity)
    @DeleteMapping("/{id}")
