...
interface Entity723Repository : org.springframework.data.jpa.repository.JpaRepository<Entity723, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity723>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity723>
}
...
class Service723(private val repo: Entity723Repository) {
    ...
    fun findActive(): List<Entity723> = repo.findByField4(true)
    fun search(query: String): List<Entity723> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity723>): Map<String, List<Entity723>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity723): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity723")
class Controller723(private val service: Service723) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity723) = service.save(entity)
    @DeleteMapping("/{id}")
