...
interface Entity733Repository : org.springframework.data.jpa.repository.JpaRepository<Entity733, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity733>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity733>
}
...
class Service733(private val repo: Entity733Repository) {
    ...
    fun findActive(): List<Entity733> = repo.findByField4(true)
    fun search(query: String): List<Entity733> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity733>): Map<String, List<Entity733>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity733): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity733")
class Controller733(private val service: Service733) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity733) = service.save(entity)
    @DeleteMapping("/{id}")
