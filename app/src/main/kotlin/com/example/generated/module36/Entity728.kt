...
interface Entity728Repository : org.springframework.data.jpa.repository.JpaRepository<Entity728, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity728>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity728>
}
...
class Service728(private val repo: Entity728Repository) {
    ...
    fun findActive(): List<Entity728> = repo.findByField4(true)
    fun search(query: String): List<Entity728> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity728>): Map<String, List<Entity728>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity728): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity728")
class Controller728(private val service: Service728) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity728) = service.save(entity)
    @DeleteMapping("/{id}")
