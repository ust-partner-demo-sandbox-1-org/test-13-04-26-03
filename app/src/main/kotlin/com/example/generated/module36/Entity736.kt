...
interface Entity736Repository : org.springframework.data.jpa.repository.JpaRepository<Entity736, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity736>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity736>
}
...
class Service736(private val repo: Entity736Repository) {
    ...
    fun findActive(): List<Entity736> = repo.findByField4(true)
    fun search(query: String): List<Entity736> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity736>): Map<String, List<Entity736>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity736): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity736")
class Controller736(private val service: Service736) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity736) = service.save(entity)
    @DeleteMapping("/{id}")
