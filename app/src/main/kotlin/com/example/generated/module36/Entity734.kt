...
interface Entity734Repository : org.springframework.data.jpa.repository.JpaRepository<Entity734, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity734>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity734>
}
...
class Service734(private val repo: Entity734Repository) {
    ...
    fun findActive(): List<Entity734> = repo.findByField4(true)
    fun search(query: String): List<Entity734> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity734>): Map<String, List<Entity734>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity734): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity734")
class Controller734(private val service: Service734) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity734) = service.save(entity)
    @DeleteMapping("/{id}")
