...
interface Entity726Repository : org.springframework.data.jpa.repository.JpaRepository<Entity726, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity726>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity726>
}
...
class Service726(private val repo: Entity726Repository) {
    ...
    fun findActive(): List<Entity726> = repo.findByField4(true)
    fun search(query: String): List<Entity726> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity726>): Map<String, List<Entity726>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity726): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity726")
class Controller726(private val service: Service726) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity726) = service.save(entity)
    @DeleteMapping("/{id}")
