...
interface Entity732Repository : org.springframework.data.jpa.repository.JpaRepository<Entity732, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity732>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity732>
}
...
class Service732(private val repo: Entity732Repository) {
    ...
    fun findActive(): List<Entity732> = repo.findByField4(true)
    fun search(query: String): List<Entity732> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity732>): Map<String, List<Entity732>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity732): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity732")
class Controller732(private val service: Service732) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity732) = service.save(entity)
    @DeleteMapping("/{id}")
