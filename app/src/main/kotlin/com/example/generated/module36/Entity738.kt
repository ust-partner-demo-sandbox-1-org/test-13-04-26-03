...
interface Entity738Repository : org.springframework.data.jpa.repository.JpaRepository<Entity738, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity738>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity738>
}
...
class Service738(private val repo: Entity738Repository) {
    ...
    fun findActive(): List<Entity738> = repo.findByField4(true)
    fun search(query: String): List<Entity738> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity738>): Map<String, List<Entity738>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity738): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity738")
class Controller738(private val service: Service738) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity738) = service.save(entity)
    @DeleteMapping("/{id}")
