...
interface Entity729Repository : org.springframework.data.jpa.repository.JpaRepository<Entity729, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity729>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity729>
}
...
class Service729(private val repo: Entity729Repository) {
    ...
    fun findActive(): List<Entity729> = repo.findByField4(true)
    fun search(query: String): List<Entity729> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity729>): Map<String, List<Entity729>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity729): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity729")
class Controller729(private val service: Service729) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity729) = service.save(entity)
    @DeleteMapping("/{id}")
