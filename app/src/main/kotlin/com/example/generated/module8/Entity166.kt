...
interface Entity166Repository : org.springframework.data.jpa.repository.JpaRepository<Entity166, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity166>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity166>
}
...
class Service166(private val repo: Entity166Repository) {
    ...
    fun findActive(): List<Entity166> = repo.findByField4(true)
    fun search(query: String): List<Entity166> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity166>): Map<String, List<Entity166>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity166): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity166")
class Controller166(private val service: Service166) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity166) = service.save(entity)
    @DeleteMapping("/{id}")
