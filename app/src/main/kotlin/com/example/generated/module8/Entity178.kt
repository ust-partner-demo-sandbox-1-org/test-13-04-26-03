...
interface Entity178Repository : org.springframework.data.jpa.repository.JpaRepository<Entity178, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity178>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity178>
}
...
class Service178(private val repo: Entity178Repository) {
    ...
    fun findActive(): List<Entity178> = repo.findByField4(true)
    fun search(query: String): List<Entity178> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity178>): Map<String, List<Entity178>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity178): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity178")
class Controller178(private val service: Service178) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity178) = service.save(entity)
    @DeleteMapping("/{id}")
