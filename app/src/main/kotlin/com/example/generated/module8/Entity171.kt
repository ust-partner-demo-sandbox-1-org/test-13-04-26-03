...
interface Entity171Repository : org.springframework.data.jpa.repository.JpaRepository<Entity171, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity171>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity171>
}
...
class Service171(private val repo: Entity171Repository) {
    ...
    fun findActive(): List<Entity171> = repo.findByField4(true)
    fun search(query: String): List<Entity171> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity171>): Map<String, List<Entity171>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity171): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity171")
class Controller171(private val service: Service171) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity171) = service.save(entity)
    @DeleteMapping("/{id}")
