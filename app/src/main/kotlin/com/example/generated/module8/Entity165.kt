...
interface Entity165Repository : org.springframework.data.jpa.repository.JpaRepository<Entity165, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity165>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity165>
}
...
class Service165(private val repo: Entity165Repository) {
    ...
    fun findActive(): List<Entity165> = repo.findByField4(true)
    fun search(query: String): List<Entity165> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity165>): Map<String, List<Entity165>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity165): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity165")
class Controller165(private val service: Service165) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity165) = service.save(entity)
    @DeleteMapping("/{id}")
