...
interface Entity163Repository : org.springframework.data.jpa.repository.JpaRepository<Entity163, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity163>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity163>
}
...
class Service163(private val repo: Entity163Repository) {
    ...
    fun findActive(): List<Entity163> = repo.findByField4(true)
    fun search(query: String): List<Entity163> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity163>): Map<String, List<Entity163>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity163): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity163")
class Controller163(private val service: Service163) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity163) = service.save(entity)
    @DeleteMapping("/{id}")
