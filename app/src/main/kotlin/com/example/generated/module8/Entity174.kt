...
interface Entity174Repository : org.springframework.data.jpa.repository.JpaRepository<Entity174, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity174>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity174>
}
...
class Service174(private val repo: Entity174Repository) {
    ...
    fun findActive(): List<Entity174> = repo.findByField4(true)
    fun search(query: String): List<Entity174> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity174>): Map<String, List<Entity174>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity174): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity174")
class Controller174(private val service: Service174) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity174) = service.save(entity)
    @DeleteMapping("/{id}")
