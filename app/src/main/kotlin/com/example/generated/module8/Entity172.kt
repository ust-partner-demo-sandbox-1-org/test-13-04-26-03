...
interface Entity172Repository : org.springframework.data.jpa.repository.JpaRepository<Entity172, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity172>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity172>
}
...
class Service172(private val repo: Entity172Repository) {
    ...
    fun findActive(): List<Entity172> = repo.findByField4(true)
    fun search(query: String): List<Entity172> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity172>): Map<String, List<Entity172>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity172): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity172")
class Controller172(private val service: Service172) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity172) = service.save(entity)
    @DeleteMapping("/{id}")
