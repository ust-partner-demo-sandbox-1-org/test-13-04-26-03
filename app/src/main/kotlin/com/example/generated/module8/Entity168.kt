...
interface Entity168Repository : org.springframework.data.jpa.repository.JpaRepository<Entity168, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity168>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity168>
}
...
class Service168(private val repo: Entity168Repository) {
    ...
    fun findActive(): List<Entity168> = repo.findByField4(true)
    fun search(query: String): List<Entity168> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity168>): Map<String, List<Entity168>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity168): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity168")
class Controller168(private val service: Service168) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity168) = service.save(entity)
    @DeleteMapping("/{id}")
