...
interface Entity173Repository : org.springframework.data.jpa.repository.JpaRepository<Entity173, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity173>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity173>
}
...
class Service173(private val repo: Entity173Repository) {
    ...
    fun findActive(): List<Entity173> = repo.findByField4(true)
    fun search(query: String): List<Entity173> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity173>): Map<String, List<Entity173>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity173): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity173")
class Controller173(private val service: Service173) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity173) = service.save(entity)
    @DeleteMapping("/{id}")
