...
interface Entity164Repository : org.springframework.data.jpa.repository.JpaRepository<Entity164, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity164>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity164>
}
...
class Service164(private val repo: Entity164Repository) {
    ...
    fun findActive(): List<Entity164> = repo.findByField4(true)
    fun search(query: String): List<Entity164> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity164>): Map<String, List<Entity164>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity164): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity164")
class Controller164(private val service: Service164) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity164) = service.save(entity)
    @DeleteMapping("/{id}")
