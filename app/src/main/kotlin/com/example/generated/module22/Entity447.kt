...
interface Entity447Repository : org.springframework.data.jpa.repository.JpaRepository<Entity447, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity447>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity447>
}
...
class Service447(private val repo: Entity447Repository) {
    ...
    fun findActive(): List<Entity447> = repo.findByField4(true)
    fun search(query: String): List<Entity447> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity447>): Map<String, List<Entity447>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity447): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity447")
class Controller447(private val service: Service447) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity447) = service.save(entity)
    @DeleteMapping("/{id}")
