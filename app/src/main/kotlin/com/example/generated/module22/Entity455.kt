...
interface Entity455Repository : org.springframework.data.jpa.repository.JpaRepository<Entity455, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity455>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity455>
}
...
class Service455(private val repo: Entity455Repository) {
    ...
    fun findActive(): List<Entity455> = repo.findByField4(true)
    fun search(query: String): List<Entity455> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity455>): Map<String, List<Entity455>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity455): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity455")
class Controller455(private val service: Service455) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity455) = service.save(entity)
    @DeleteMapping("/{id}")
