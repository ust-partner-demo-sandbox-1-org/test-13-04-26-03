...
interface Entity450Repository : org.springframework.data.jpa.repository.JpaRepository<Entity450, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity450>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity450>
}
...
class Service450(private val repo: Entity450Repository) {
    ...
    fun findActive(): List<Entity450> = repo.findByField4(true)
    fun search(query: String): List<Entity450> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity450>): Map<String, List<Entity450>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity450): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity450")
class Controller450(private val service: Service450) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity450) = service.save(entity)
    @DeleteMapping("/{id}")
