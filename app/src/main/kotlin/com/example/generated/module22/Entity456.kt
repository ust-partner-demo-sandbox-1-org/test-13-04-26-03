...
interface Entity456Repository : org.springframework.data.jpa.repository.JpaRepository<Entity456, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity456>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity456>
}
...
class Service456(private val repo: Entity456Repository) {
    ...
    fun findActive(): List<Entity456> = repo.findByField4(true)
    fun search(query: String): List<Entity456> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity456>): Map<String, List<Entity456>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity456): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity456")
class Controller456(private val service: Service456) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity456) = service.save(entity)
    @DeleteMapping("/{id}")
