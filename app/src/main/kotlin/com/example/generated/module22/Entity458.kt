...
interface Entity458Repository : org.springframework.data.jpa.repository.JpaRepository<Entity458, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity458>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity458>
}
...
class Service458(private val repo: Entity458Repository) {
    ...
    fun findActive(): List<Entity458> = repo.findByField4(true)
    fun search(query: String): List<Entity458> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity458>): Map<String, List<Entity458>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity458): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity458")
class Controller458(private val service: Service458) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity458) = service.save(entity)
    @DeleteMapping("/{id}")
