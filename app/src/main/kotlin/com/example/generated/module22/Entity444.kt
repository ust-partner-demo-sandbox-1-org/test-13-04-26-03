...
interface Entity444Repository : org.springframework.data.jpa.repository.JpaRepository<Entity444, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity444>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity444>
}
...
class Service444(private val repo: Entity444Repository) {
    ...
    fun findActive(): List<Entity444> = repo.findByField4(true)
    fun search(query: String): List<Entity444> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity444>): Map<String, List<Entity444>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity444): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity444")
class Controller444(private val service: Service444) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity444) = service.save(entity)
    @DeleteMapping("/{id}")
