...
interface Entity451Repository : org.springframework.data.jpa.repository.JpaRepository<Entity451, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity451>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity451>
}
...
class Service451(private val repo: Entity451Repository) {
    ...
    fun findActive(): List<Entity451> = repo.findByField4(true)
    fun search(query: String): List<Entity451> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity451>): Map<String, List<Entity451>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity451): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity451")
class Controller451(private val service: Service451) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity451) = service.save(entity)
    @DeleteMapping("/{id}")
