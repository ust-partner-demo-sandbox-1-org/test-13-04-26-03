...
interface Entity446Repository : org.springframework.data.jpa.repository.JpaRepository<Entity446, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity446>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity446>
}
...
class Service446(private val repo: Entity446Repository) {
    ...
    fun findActive(): List<Entity446> = repo.findByField4(true)
    fun search(query: String): List<Entity446> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity446>): Map<String, List<Entity446>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity446): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity446")
class Controller446(private val service: Service446) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity446) = service.save(entity)
    @DeleteMapping("/{id}")
