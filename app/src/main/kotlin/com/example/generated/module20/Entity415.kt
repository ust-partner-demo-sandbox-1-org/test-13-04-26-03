...
interface Entity415Repository : org.springframework.data.jpa.repository.JpaRepository<Entity415, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity415>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity415>
}
...
class Service415(private val repo: Entity415Repository) {
    ...
    fun findActive(): List<Entity415> = repo.findByField4(true)
    fun search(query: String): List<Entity415> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity415>): Map<String, List<Entity415>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity415): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity415")
class Controller415(private val service: Service415) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity415) = service.save(entity)
    @DeleteMapping("/{id}")
