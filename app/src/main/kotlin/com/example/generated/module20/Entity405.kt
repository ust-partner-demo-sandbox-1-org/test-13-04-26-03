...
interface Entity405Repository : org.springframework.data.jpa.repository.JpaRepository<Entity405, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity405>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity405>
}
...
class Service405(private val repo: Entity405Repository) {
    ...
    fun findActive(): List<Entity405> = repo.findByField4(true)
    fun search(query: String): List<Entity405> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity405>): Map<String, List<Entity405>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity405): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity405")
class Controller405(private val service: Service405) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity405) = service.save(entity)
    @DeleteMapping("/{id}")
