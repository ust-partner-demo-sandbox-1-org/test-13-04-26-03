...
interface Entity410Repository : org.springframework.data.jpa.repository.JpaRepository<Entity410, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity410>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity410>
}
...
class Service410(private val repo: Entity410Repository) {
    ...
    fun findActive(): List<Entity410> = repo.findByField4(true)
    fun search(query: String): List<Entity410> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity410>): Map<String, List<Entity410>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity410): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity410")
class Controller410(private val service: Service410) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity410) = service.save(entity)
    @DeleteMapping("/{id}")
