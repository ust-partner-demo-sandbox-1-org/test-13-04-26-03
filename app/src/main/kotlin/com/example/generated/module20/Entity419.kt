...
interface Entity419Repository : org.springframework.data.jpa.repository.JpaRepository<Entity419, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity419>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity419>
}
...
class Service419(private val repo: Entity419Repository) {
    ...
    fun findActive(): List<Entity419> = repo.findByField4(true)
    fun search(query: String): List<Entity419> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity419>): Map<String, List<Entity419>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity419): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity419")
class Controller419(private val service: Service419) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity419) = service.save(entity)
    @DeleteMapping("/{id}")
