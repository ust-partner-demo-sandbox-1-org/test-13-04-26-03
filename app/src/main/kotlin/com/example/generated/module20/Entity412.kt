...
interface Entity412Repository : org.springframework.data.jpa.repository.JpaRepository<Entity412, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity412>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity412>
}
...
class Service412(private val repo: Entity412Repository) {
    ...
    fun findActive(): List<Entity412> = repo.findByField4(true)
    fun search(query: String): List<Entity412> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity412>): Map<String, List<Entity412>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity412): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity412")
class Controller412(private val service: Service412) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity412) = service.save(entity)
    @DeleteMapping("/{id}")
