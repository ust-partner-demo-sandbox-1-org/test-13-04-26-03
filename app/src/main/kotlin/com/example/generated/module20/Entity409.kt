...
interface Entity409Repository : org.springframework.data.jpa.repository.JpaRepository<Entity409, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity409>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity409>
}
...
class Service409(private val repo: Entity409Repository) {
    ...
    fun findActive(): List<Entity409> = repo.findByField4(true)
    fun search(query: String): List<Entity409> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity409>): Map<String, List<Entity409>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity409): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity409")
class Controller409(private val service: Service409) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity409) = service.save(entity)
    @DeleteMapping("/{id}")
