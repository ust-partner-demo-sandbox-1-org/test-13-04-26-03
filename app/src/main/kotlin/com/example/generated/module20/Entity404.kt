...
interface Entity404Repository : org.springframework.data.jpa.repository.JpaRepository<Entity404, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity404>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity404>
}
...
class Service404(private val repo: Entity404Repository) {
    ...
    fun findActive(): List<Entity404> = repo.findByField4(true)
    fun search(query: String): List<Entity404> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity404>): Map<String, List<Entity404>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity404): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity404")
class Controller404(private val service: Service404) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity404) = service.save(entity)
    @DeleteMapping("/{id}")
