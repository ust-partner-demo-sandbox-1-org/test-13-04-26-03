...
interface Entity403Repository : org.springframework.data.jpa.repository.JpaRepository<Entity403, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity403>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity403>
}
...
class Service403(private val repo: Entity403Repository) {
    ...
    fun findActive(): List<Entity403> = repo.findByField4(true)
    fun search(query: String): List<Entity403> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity403>): Map<String, List<Entity403>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity403): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity403")
class Controller403(private val service: Service403) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity403) = service.save(entity)
    @DeleteMapping("/{id}")
