...
interface Entity401Repository : org.springframework.data.jpa.repository.JpaRepository<Entity401, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity401>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity401>
}
...
class Service401(private val repo: Entity401Repository) {
    ...
    fun findActive(): List<Entity401> = repo.findByField4(true)
    fun search(query: String): List<Entity401> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity401>): Map<String, List<Entity401>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity401): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity401")
class Controller401(private val service: Service401) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity401) = service.save(entity)
    @DeleteMapping("/{id}")
