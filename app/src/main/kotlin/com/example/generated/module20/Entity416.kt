...
interface Entity416Repository : org.springframework.data.jpa.repository.JpaRepository<Entity416, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity416>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity416>
}
...
class Service416(private val repo: Entity416Repository) {
    ...
    fun findActive(): List<Entity416> = repo.findByField4(true)
    fun search(query: String): List<Entity416> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity416>): Map<String, List<Entity416>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity416): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity416")
class Controller416(private val service: Service416) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity416) = service.save(entity)
    @DeleteMapping("/{id}")
