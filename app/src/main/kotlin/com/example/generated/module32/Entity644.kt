...
interface Entity644Repository : org.springframework.data.jpa.repository.JpaRepository<Entity644, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity644>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity644>
}
...
class Service644(private val repo: Entity644Repository) {
    ...
    fun findActive(): List<Entity644> = repo.findByField4(true)
    fun search(query: String): List<Entity644> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity644>): Map<String, List<Entity644>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity644): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity644")
class Controller644(private val service: Service644) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity644) = service.save(entity)
    @DeleteMapping("/{id}")
