...
interface Entity654Repository : org.springframework.data.jpa.repository.JpaRepository<Entity654, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity654>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity654>
}
...
class Service654(private val repo: Entity654Repository) {
    ...
    fun findActive(): List<Entity654> = repo.findByField4(true)
    fun search(query: String): List<Entity654> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity654>): Map<String, List<Entity654>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity654): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity654")
class Controller654(private val service: Service654) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity654) = service.save(entity)
    @DeleteMapping("/{id}")
