...
interface Entity648Repository : org.springframework.data.jpa.repository.JpaRepository<Entity648, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity648>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity648>
}
...
class Service648(private val repo: Entity648Repository) {
    ...
    fun findActive(): List<Entity648> = repo.findByField4(true)
    fun search(query: String): List<Entity648> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity648>): Map<String, List<Entity648>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity648): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity648")
class Controller648(private val service: Service648) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity648) = service.save(entity)
    @DeleteMapping("/{id}")
