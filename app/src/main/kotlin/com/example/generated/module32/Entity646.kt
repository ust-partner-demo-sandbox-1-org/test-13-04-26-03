...
interface Entity646Repository : org.springframework.data.jpa.repository.JpaRepository<Entity646, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity646>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity646>
}
...
class Service646(private val repo: Entity646Repository) {
    ...
    fun findActive(): List<Entity646> = repo.findByField4(true)
    fun search(query: String): List<Entity646> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity646>): Map<String, List<Entity646>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity646): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity646")
class Controller646(private val service: Service646) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity646) = service.save(entity)
    @DeleteMapping("/{id}")
