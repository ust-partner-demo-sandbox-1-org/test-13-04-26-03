...
interface Entity658Repository : org.springframework.data.jpa.repository.JpaRepository<Entity658, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity658>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity658>
}
...
class Service658(private val repo: Entity658Repository) {
    ...
    fun findActive(): List<Entity658> = repo.findByField4(true)
    fun search(query: String): List<Entity658> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity658>): Map<String, List<Entity658>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity658): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity658")
class Controller658(private val service: Service658) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity658) = service.save(entity)
    @DeleteMapping("/{id}")
