...
interface Entity656Repository : org.springframework.data.jpa.repository.JpaRepository<Entity656, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity656>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity656>
}
...
class Service656(private val repo: Entity656Repository) {
    ...
    fun findActive(): List<Entity656> = repo.findByField4(true)
    fun search(query: String): List<Entity656> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity656>): Map<String, List<Entity656>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity656): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity656")
class Controller656(private val service: Service656) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity656) = service.save(entity)
    @DeleteMapping("/{id}")
