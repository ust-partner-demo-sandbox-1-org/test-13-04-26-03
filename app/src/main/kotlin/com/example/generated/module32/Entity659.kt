...
interface Entity659Repository : org.springframework.data.jpa.repository.JpaRepository<Entity659, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity659>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity659>
}
...
class Service659(private val repo: Entity659Repository) {
    ...
    fun findActive(): List<Entity659> = repo.findByField4(true)
    fun search(query: String): List<Entity659> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity659>): Map<String, List<Entity659>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity659): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity659")
class Controller659(private val service: Service659) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity659) = service.save(entity)
    @DeleteMapping("/{id}")
