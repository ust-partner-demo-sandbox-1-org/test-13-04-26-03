...
interface Entity651Repository : org.springframework.data.jpa.repository.JpaRepository<Entity651, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity651>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity651>
}
...
class Service651(private val repo: Entity651Repository) {
    ...
    fun findActive(): List<Entity651> = repo.findByField4(true)
    fun search(query: String): List<Entity651> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity651>): Map<String, List<Entity651>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity651): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity651")
class Controller651(private val service: Service651) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity651) = service.save(entity)
    @DeleteMapping("/{id}")
