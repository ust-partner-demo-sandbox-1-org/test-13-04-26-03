...
interface Entity643Repository : org.springframework.data.jpa.repository.JpaRepository<Entity643, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity643>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity643>
}
...
class Service643(private val repo: Entity643Repository) {
    ...
    fun findActive(): List<Entity643> = repo.findByField4(true)
    fun search(query: String): List<Entity643> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity643>): Map<String, List<Entity643>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity643): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity643")
class Controller643(private val service: Service643) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity643) = service.save(entity)
    @DeleteMapping("/{id}")
