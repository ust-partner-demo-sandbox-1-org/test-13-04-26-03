...
interface Entity641Repository : org.springframework.data.jpa.repository.JpaRepository<Entity641, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity641>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity641>
}
...
class Service641(private val repo: Entity641Repository) {
    ...
    fun findActive(): List<Entity641> = repo.findByField4(true)
    fun search(query: String): List<Entity641> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity641>): Map<String, List<Entity641>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity641): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity641")
class Controller641(private val service: Service641) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity641) = service.save(entity)
    @DeleteMapping("/{id}")
