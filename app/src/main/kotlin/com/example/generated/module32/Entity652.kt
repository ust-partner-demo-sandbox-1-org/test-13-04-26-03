...
interface Entity652Repository : org.springframework.data.jpa.repository.JpaRepository<Entity652, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity652>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity652>
}
...
class Service652(private val repo: Entity652Repository) {
    ...
    fun findActive(): List<Entity652> = repo.findByField4(true)
    fun search(query: String): List<Entity652> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity652>): Map<String, List<Entity652>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity652): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity652")
class Controller652(private val service: Service652) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity652) = service.save(entity)
    @DeleteMapping("/{id}")
