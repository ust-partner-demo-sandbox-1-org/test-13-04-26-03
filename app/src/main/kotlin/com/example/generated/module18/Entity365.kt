...
interface Entity365Repository : org.springframework.data.jpa.repository.JpaRepository<Entity365, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity365>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity365>
}
...
class Service365(private val repo: Entity365Repository) {
    ...
    fun findActive(): List<Entity365> = repo.findByField4(true)
    fun search(query: String): List<Entity365> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity365>): Map<String, List<Entity365>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity365): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity365")
class Controller365(private val service: Service365) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity365) = service.save(entity)
    @DeleteMapping("/{id}")
