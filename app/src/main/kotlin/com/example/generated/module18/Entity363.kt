...
interface Entity363Repository : org.springframework.data.jpa.repository.JpaRepository<Entity363, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity363>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity363>
}
...
class Service363(private val repo: Entity363Repository) {
    ...
    fun findActive(): List<Entity363> = repo.findByField4(true)
    fun search(query: String): List<Entity363> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity363>): Map<String, List<Entity363>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity363): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity363")
class Controller363(private val service: Service363) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity363) = service.save(entity)
    @DeleteMapping("/{id}")
