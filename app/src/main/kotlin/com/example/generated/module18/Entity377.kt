...
interface Entity377Repository : org.springframework.data.jpa.repository.JpaRepository<Entity377, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity377>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity377>
}
...
class Service377(private val repo: Entity377Repository) {
    ...
    fun findActive(): List<Entity377> = repo.findByField4(true)
    fun search(query: String): List<Entity377> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity377>): Map<String, List<Entity377>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity377): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity377")
class Controller377(private val service: Service377) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity377) = service.save(entity)
    @DeleteMapping("/{id}")
