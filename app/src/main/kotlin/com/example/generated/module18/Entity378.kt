...
interface Entity378Repository : org.springframework.data.jpa.repository.JpaRepository<Entity378, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity378>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity378>
}
...
class Service378(private val repo: Entity378Repository) {
    ...
    fun findActive(): List<Entity378> = repo.findByField4(true)
    fun search(query: String): List<Entity378> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity378>): Map<String, List<Entity378>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity378): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity378")
class Controller378(private val service: Service378) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity378) = service.save(entity)
    @DeleteMapping("/{id}")
