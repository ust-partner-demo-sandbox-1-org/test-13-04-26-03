...
interface Entity362Repository : org.springframework.data.jpa.repository.JpaRepository<Entity362, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity362>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity362>
}
...
class Service362(private val repo: Entity362Repository) {
    ...
    fun findActive(): List<Entity362> = repo.findByField4(true)
    fun search(query: String): List<Entity362> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity362>): Map<String, List<Entity362>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity362): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity362")
class Controller362(private val service: Service362) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity362) = service.save(entity)
    @DeleteMapping("/{id}")
