...
interface Entity368Repository : org.springframework.data.jpa.repository.JpaRepository<Entity368, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity368>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity368>
}
...
class Service368(private val repo: Entity368Repository) {
    ...
    fun findActive(): List<Entity368> = repo.findByField4(true)
    fun search(query: String): List<Entity368> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity368>): Map<String, List<Entity368>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity368): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity368")
class Controller368(private val service: Service368) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity368) = service.save(entity)
    @DeleteMapping("/{id}")
