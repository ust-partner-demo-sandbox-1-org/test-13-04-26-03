...
interface Entity361Repository : org.springframework.data.jpa.repository.JpaRepository<Entity361, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity361>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity361>
}
...
class Service361(private val repo: Entity361Repository) {
    ...
    fun findActive(): List<Entity361> = repo.findByField4(true)
    fun search(query: String): List<Entity361> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity361>): Map<String, List<Entity361>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity361): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity361")
class Controller361(private val service: Service361) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity361) = service.save(entity)
    @DeleteMapping("/{id}")
