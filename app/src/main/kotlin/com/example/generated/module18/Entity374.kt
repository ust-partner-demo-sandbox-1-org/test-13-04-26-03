...
interface Entity374Repository : org.springframework.data.jpa.repository.JpaRepository<Entity374, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity374>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity374>
}
...
class Service374(private val repo: Entity374Repository) {
    ...
    fun findActive(): List<Entity374> = repo.findByField4(true)
    fun search(query: String): List<Entity374> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity374>): Map<String, List<Entity374>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity374): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity374")
class Controller374(private val service: Service374) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity374) = service.save(entity)
    @DeleteMapping("/{id}")
