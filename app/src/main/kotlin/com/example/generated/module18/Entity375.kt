...
interface Entity375Repository : org.springframework.data.jpa.repository.JpaRepository<Entity375, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity375>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity375>
}
...
class Service375(private val repo: Entity375Repository) {
    ...
    fun findActive(): List<Entity375> = repo.findByField4(true)
    fun search(query: String): List<Entity375> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity375>): Map<String, List<Entity375>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity375): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity375")
class Controller375(private val service: Service375) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity375) = service.save(entity)
    @DeleteMapping("/{id}")
