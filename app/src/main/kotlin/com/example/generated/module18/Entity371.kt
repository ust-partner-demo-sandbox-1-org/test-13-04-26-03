...
interface Entity371Repository : org.springframework.data.jpa.repository.JpaRepository<Entity371, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity371>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity371>
}
...
class Service371(private val repo: Entity371Repository) {
    ...
    fun findActive(): List<Entity371> = repo.findByField4(true)
    fun search(query: String): List<Entity371> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity371>): Map<String, List<Entity371>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity371): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity371")
class Controller371(private val service: Service371) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity371) = service.save(entity)
    @DeleteMapping("/{id}")
