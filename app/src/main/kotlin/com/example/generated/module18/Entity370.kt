...
interface Entity370Repository : org.springframework.data.jpa.repository.JpaRepository<Entity370, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity370>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity370>
}
...
class Service370(private val repo: Entity370Repository) {
    ...
    fun findActive(): List<Entity370> = repo.findByField4(true)
    fun search(query: String): List<Entity370> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity370>): Map<String, List<Entity370>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity370): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity370")
class Controller370(private val service: Service370) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity370) = service.save(entity)
    @DeleteMapping("/{id}")
