...
interface Entity494Repository : org.springframework.data.jpa.repository.JpaRepository<Entity494, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity494>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity494>
}
...
class Service494(private val repo: Entity494Repository) {
    ...
    fun findActive(): List<Entity494> = repo.findByField4(true)
    fun search(query: String): List<Entity494> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity494>): Map<String, List<Entity494>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity494): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity494")
class Controller494(private val service: Service494) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity494) = service.save(entity)
    @DeleteMapping("/{id}")
