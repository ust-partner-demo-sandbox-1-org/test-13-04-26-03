...
interface Entity499Repository : org.springframework.data.jpa.repository.JpaRepository<Entity499, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity499>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity499>
}
...
class Service499(private val repo: Entity499Repository) {
    ...
    fun findActive(): List<Entity499> = repo.findByField4(true)
    fun search(query: String): List<Entity499> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity499>): Map<String, List<Entity499>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity499): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity499")
class Controller499(private val service: Service499) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity499) = service.save(entity)
    @DeleteMapping("/{id}")
