...
interface Entity492Repository : org.springframework.data.jpa.repository.JpaRepository<Entity492, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity492>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity492>
}
...
class Service492(private val repo: Entity492Repository) {
    ...
    fun findActive(): List<Entity492> = repo.findByField4(true)
    fun search(query: String): List<Entity492> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity492>): Map<String, List<Entity492>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity492): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity492")
class Controller492(private val service: Service492) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity492) = service.save(entity)
    @DeleteMapping("/{id}")
