...
interface Entity482Repository : org.springframework.data.jpa.repository.JpaRepository<Entity482, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity482>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity482>
}
...
class Service482(private val repo: Entity482Repository) {
    ...
    fun findActive(): List<Entity482> = repo.findByField4(true)
    fun search(query: String): List<Entity482> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity482>): Map<String, List<Entity482>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity482): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity482")
class Controller482(private val service: Service482) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity482) = service.save(entity)
    @DeleteMapping("/{id}")
