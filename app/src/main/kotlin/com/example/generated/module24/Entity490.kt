...
interface Entity490Repository : org.springframework.data.jpa.repository.JpaRepository<Entity490, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity490>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity490>
}
...
class Service490(private val repo: Entity490Repository) {
    ...
    fun findActive(): List<Entity490> = repo.findByField4(true)
    fun search(query: String): List<Entity490> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity490>): Map<String, List<Entity490>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity490): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity490")
class Controller490(private val service: Service490) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity490) = service.save(entity)
    @DeleteMapping("/{id}")
