...
interface Entity493Repository : org.springframework.data.jpa.repository.JpaRepository<Entity493, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity493>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity493>
}
...
class Service493(private val repo: Entity493Repository) {
    ...
    fun findActive(): List<Entity493> = repo.findByField4(true)
    fun search(query: String): List<Entity493> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity493>): Map<String, List<Entity493>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity493): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity493")
class Controller493(private val service: Service493) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity493) = service.save(entity)
    @DeleteMapping("/{id}")
