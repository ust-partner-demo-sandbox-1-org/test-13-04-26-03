...
interface Entity485Repository : org.springframework.data.jpa.repository.JpaRepository<Entity485, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity485>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity485>
}
...
class Service485(private val repo: Entity485Repository) {
    ...
    fun findActive(): List<Entity485> = repo.findByField4(true)
    fun search(query: String): List<Entity485> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity485>): Map<String, List<Entity485>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity485): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity485")
class Controller485(private val service: Service485) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity485) = service.save(entity)
    @DeleteMapping("/{id}")
