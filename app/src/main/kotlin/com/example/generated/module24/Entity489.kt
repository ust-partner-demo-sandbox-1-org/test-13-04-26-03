...
interface Entity489Repository : org.springframework.data.jpa.repository.JpaRepository<Entity489, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity489>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity489>
}
...
class Service489(private val repo: Entity489Repository) {
    ...
    fun findActive(): List<Entity489> = repo.findByField4(true)
    fun search(query: String): List<Entity489> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity489>): Map<String, List<Entity489>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity489): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity489")
class Controller489(private val service: Service489) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity489) = service.save(entity)
    @DeleteMapping("/{id}")
