...
interface Entity488Repository : org.springframework.data.jpa.repository.JpaRepository<Entity488, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity488>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity488>
}
...
class Service488(private val repo: Entity488Repository) {
    ...
    fun findActive(): List<Entity488> = repo.findByField4(true)
    fun search(query: String): List<Entity488> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity488>): Map<String, List<Entity488>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity488): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity488")
class Controller488(private val service: Service488) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity488) = service.save(entity)
    @DeleteMapping("/{id}")
