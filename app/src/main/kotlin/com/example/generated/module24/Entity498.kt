...
interface Entity498Repository : org.springframework.data.jpa.repository.JpaRepository<Entity498, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity498>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity498>
}
...
class Service498(private val repo: Entity498Repository) {
    ...
    fun findActive(): List<Entity498> = repo.findByField4(true)
    fun search(query: String): List<Entity498> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity498>): Map<String, List<Entity498>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity498): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity498")
class Controller498(private val service: Service498) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity498) = service.save(entity)
    @DeleteMapping("/{id}")
