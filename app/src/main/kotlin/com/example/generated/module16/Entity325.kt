...
interface Entity325Repository : org.springframework.data.jpa.repository.JpaRepository<Entity325, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity325>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity325>
}
...
class Service325(private val repo: Entity325Repository) {
    ...
    fun findActive(): List<Entity325> = repo.findByField4(true)
    fun search(query: String): List<Entity325> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity325>): Map<String, List<Entity325>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity325): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity325")
class Controller325(private val service: Service325) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity325) = service.save(entity)
    @DeleteMapping("/{id}")
