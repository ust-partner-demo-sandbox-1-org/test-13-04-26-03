...
interface Entity334Repository : org.springframework.data.jpa.repository.JpaRepository<Entity334, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity334>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity334>
}
...
class Service334(private val repo: Entity334Repository) {
    ...
    fun findActive(): List<Entity334> = repo.findByField4(true)
    fun search(query: String): List<Entity334> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity334>): Map<String, List<Entity334>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity334): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity334")
class Controller334(private val service: Service334) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity334) = service.save(entity)
    @DeleteMapping("/{id}")
