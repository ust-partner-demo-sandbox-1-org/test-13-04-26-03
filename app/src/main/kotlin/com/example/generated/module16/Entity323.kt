...
interface Entity323Repository : org.springframework.data.jpa.repository.JpaRepository<Entity323, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity323>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity323>
}
...
class Service323(private val repo: Entity323Repository) {
    ...
    fun findActive(): List<Entity323> = repo.findByField4(true)
    fun search(query: String): List<Entity323> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity323>): Map<String, List<Entity323>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity323): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity323")
class Controller323(private val service: Service323) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity323) = service.save(entity)
    @DeleteMapping("/{id}")
