...
interface Entity321Repository : org.springframework.data.jpa.repository.JpaRepository<Entity321, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity321>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity321>
}
...
class Service321(private val repo: Entity321Repository) {
    ...
    fun findActive(): List<Entity321> = repo.findByField4(true)
    fun search(query: String): List<Entity321> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity321>): Map<String, List<Entity321>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity321): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity321")
class Controller321(private val service: Service321) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity321) = service.save(entity)
    @DeleteMapping("/{id}")
