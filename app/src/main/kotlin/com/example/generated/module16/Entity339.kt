...
interface Entity339Repository : org.springframework.data.jpa.repository.JpaRepository<Entity339, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity339>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity339>
}
...
class Service339(private val repo: Entity339Repository) {
    ...
    fun findActive(): List<Entity339> = repo.findByField4(true)
    fun search(query: String): List<Entity339> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity339>): Map<String, List<Entity339>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity339): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity339")
class Controller339(private val service: Service339) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity339) = service.save(entity)
    @DeleteMapping("/{id}")
