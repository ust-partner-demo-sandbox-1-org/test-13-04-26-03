...
interface Entity331Repository : org.springframework.data.jpa.repository.JpaRepository<Entity331, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity331>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity331>
}
...
class Service331(private val repo: Entity331Repository) {
    ...
    fun findActive(): List<Entity331> = repo.findByField4(true)
    fun search(query: String): List<Entity331> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity331>): Map<String, List<Entity331>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity331): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity331")
class Controller331(private val service: Service331) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity331) = service.save(entity)
    @DeleteMapping("/{id}")
