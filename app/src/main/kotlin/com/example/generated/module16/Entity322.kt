...
interface Entity322Repository : org.springframework.data.jpa.repository.JpaRepository<Entity322, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity322>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity322>
}
...
class Service322(private val repo: Entity322Repository) {
    ...
    fun findActive(): List<Entity322> = repo.findByField4(true)
    fun search(query: String): List<Entity322> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity322>): Map<String, List<Entity322>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity322): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity322")
class Controller322(private val service: Service322) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity322) = service.save(entity)
    @DeleteMapping("/{id}")
