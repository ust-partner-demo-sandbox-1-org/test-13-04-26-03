...
interface Entity327Repository : org.springframework.data.jpa.repository.JpaRepository<Entity327, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity327>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity327>
}
...
class Service327(private val repo: Entity327Repository) {
    ...
    fun findActive(): List<Entity327> = repo.findByField4(true)
    fun search(query: String): List<Entity327> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity327>): Map<String, List<Entity327>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity327): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity327")
class Controller327(private val service: Service327) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity327) = service.save(entity)
    @DeleteMapping("/{id}")
