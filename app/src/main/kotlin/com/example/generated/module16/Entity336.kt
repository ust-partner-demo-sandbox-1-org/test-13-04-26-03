...
interface Entity336Repository : org.springframework.data.jpa.repository.JpaRepository<Entity336, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity336>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity336>
}
...
class Service336(private val repo: Entity336Repository) {
    ...
    fun findActive(): List<Entity336> = repo.findByField4(true)
    fun search(query: String): List<Entity336> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity336>): Map<String, List<Entity336>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity336): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity336")
class Controller336(private val service: Service336) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity336) = service.save(entity)
    @DeleteMapping("/{id}")
