...
interface Entity337Repository : org.springframework.data.jpa.repository.JpaRepository<Entity337, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity337>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity337>
}
...
class Service337(private val repo: Entity337Repository) {
    ...
    fun findActive(): List<Entity337> = repo.findByField4(true)
    fun search(query: String): List<Entity337> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity337>): Map<String, List<Entity337>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity337): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity337")
class Controller337(private val service: Service337) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity337) = service.save(entity)
    @DeleteMapping("/{id}")
