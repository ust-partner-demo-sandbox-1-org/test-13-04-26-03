...
interface Entity66Repository : org.springframework.data.jpa.repository.JpaRepository<Entity66, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity66>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity66>
}
...
class Service66(private val repo: Entity66Repository) {
    ...
    fun findActive(): List<Entity66> = repo.findByField4(true)
    fun search(query: String): List<Entity66> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity66>): Map<String, List<Entity66>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity66): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity66")
class Controller66(private val service: Service66) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity66) = service.save(entity)
    @DeleteMapping("/{id}")
