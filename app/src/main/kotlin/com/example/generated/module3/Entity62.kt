...
interface Entity62Repository : org.springframework.data.jpa.repository.JpaRepository<Entity62, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity62>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity62>
}
...
class Service62(private val repo: Entity62Repository) {
    ...
    fun findActive(): List<Entity62> = repo.findByField4(true)
    fun search(query: String): List<Entity62> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity62>): Map<String, List<Entity62>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity62): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity62")
class Controller62(private val service: Service62) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity62) = service.save(entity)
    @DeleteMapping("/{id}")
