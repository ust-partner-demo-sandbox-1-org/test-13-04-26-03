...
interface Entity75Repository : org.springframework.data.jpa.repository.JpaRepository<Entity75, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity75>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity75>
}
...
class Service75(private val repo: Entity75Repository) {
    ...
    fun findActive(): List<Entity75> = repo.findByField4(true)
    fun search(query: String): List<Entity75> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity75>): Map<String, List<Entity75>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity75): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity75")
class Controller75(private val service: Service75) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity75) = service.save(entity)
    @DeleteMapping("/{id}")
