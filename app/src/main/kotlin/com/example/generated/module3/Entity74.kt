...
interface Entity74Repository : org.springframework.data.jpa.repository.JpaRepository<Entity74, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity74>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity74>
}
...
class Service74(private val repo: Entity74Repository) {
    ...
    fun findActive(): List<Entity74> = repo.findByField4(true)
    fun search(query: String): List<Entity74> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity74>): Map<String, List<Entity74>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity74): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity74")
class Controller74(private val service: Service74) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity74) = service.save(entity)
    @DeleteMapping("/{id}")
