...
interface Entity70Repository : org.springframework.data.jpa.repository.JpaRepository<Entity70, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity70>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity70>
}
...
class Service70(private val repo: Entity70Repository) {
    ...
    fun findActive(): List<Entity70> = repo.findByField4(true)
    fun search(query: String): List<Entity70> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity70>): Map<String, List<Entity70>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity70): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity70")
class Controller70(private val service: Service70) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity70) = service.save(entity)
    @DeleteMapping("/{id}")
