...
interface Entity72Repository : org.springframework.data.jpa.repository.JpaRepository<Entity72, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity72>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity72>
}
...
class Service72(private val repo: Entity72Repository) {
    ...
    fun findActive(): List<Entity72> = repo.findByField4(true)
    fun search(query: String): List<Entity72> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity72>): Map<String, List<Entity72>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity72): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity72")
class Controller72(private val service: Service72) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity72) = service.save(entity)
    @DeleteMapping("/{id}")
