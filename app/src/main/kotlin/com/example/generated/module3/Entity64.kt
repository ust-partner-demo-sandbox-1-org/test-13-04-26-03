...
interface Entity64Repository : org.springframework.data.jpa.repository.JpaRepository<Entity64, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity64>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity64>
}
...
class Service64(private val repo: Entity64Repository) {
    ...
    fun findActive(): List<Entity64> = repo.findByField4(true)
    fun search(query: String): List<Entity64> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity64>): Map<String, List<Entity64>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity64): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity64")
class Controller64(private val service: Service64) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity64) = service.save(entity)
    @DeleteMapping("/{id}")
