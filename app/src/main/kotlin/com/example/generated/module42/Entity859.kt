...
interface Entity859Repository : org.springframework.data.jpa.repository.JpaRepository<Entity859, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity859>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity859>
}
...
class Service859(private val repo: Entity859Repository) {
    ...
    fun findActive(): List<Entity859> = repo.findByField4(true)
    fun search(query: String): List<Entity859> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity859>): Map<String, List<Entity859>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity859): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity859")
class Controller859(private val service: Service859) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity859) = service.save(entity)
    @DeleteMapping("/{id}")
