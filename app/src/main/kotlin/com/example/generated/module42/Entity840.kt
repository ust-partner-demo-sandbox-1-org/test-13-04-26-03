...
interface Entity840Repository : org.springframework.data.jpa.repository.JpaRepository<Entity840, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity840>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity840>
}
...
class Service840(private val repo: Entity840Repository) {
    ...
    fun findActive(): List<Entity840> = repo.findByField4(true)
    fun search(query: String): List<Entity840> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity840>): Map<String, List<Entity840>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity840): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity840")
class Controller840(private val service: Service840) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity840) = service.save(entity)
    @DeleteMapping("/{id}")
