...
interface Entity843Repository : org.springframework.data.jpa.repository.JpaRepository<Entity843, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity843>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity843>
}
...
class Service843(private val repo: Entity843Repository) {
    ...
    fun findActive(): List<Entity843> = repo.findByField4(true)
    fun search(query: String): List<Entity843> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity843>): Map<String, List<Entity843>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity843): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity843")
class Controller843(private val service: Service843) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity843) = service.save(entity)
    @DeleteMapping("/{id}")
