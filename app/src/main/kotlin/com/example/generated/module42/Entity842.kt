...
interface Entity842Repository : org.springframework.data.jpa.repository.JpaRepository<Entity842, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity842>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity842>
}
...
class Service842(private val repo: Entity842Repository) {
    ...
    fun findActive(): List<Entity842> = repo.findByField4(true)
    fun search(query: String): List<Entity842> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity842>): Map<String, List<Entity842>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity842): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity842")
class Controller842(private val service: Service842) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity842) = service.save(entity)
    @DeleteMapping("/{id}")
