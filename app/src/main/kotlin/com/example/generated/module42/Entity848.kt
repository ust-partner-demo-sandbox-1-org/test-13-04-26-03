...
interface Entity848Repository : org.springframework.data.jpa.repository.JpaRepository<Entity848, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity848>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity848>
}
...
class Service848(private val repo: Entity848Repository) {
    ...
    fun findActive(): List<Entity848> = repo.findByField4(true)
    fun search(query: String): List<Entity848> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity848>): Map<String, List<Entity848>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity848): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity848")
class Controller848(private val service: Service848) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity848) = service.save(entity)
    @DeleteMapping("/{id}")
