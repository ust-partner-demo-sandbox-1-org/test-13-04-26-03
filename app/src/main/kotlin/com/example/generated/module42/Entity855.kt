...
interface Entity855Repository : org.springframework.data.jpa.repository.JpaRepository<Entity855, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity855>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity855>
}
...
class Service855(private val repo: Entity855Repository) {
    ...
    fun findActive(): List<Entity855> = repo.findByField4(true)
    fun search(query: String): List<Entity855> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity855>): Map<String, List<Entity855>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity855): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity855")
class Controller855(private val service: Service855) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity855) = service.save(entity)
    @DeleteMapping("/{id}")
