...
interface Entity858Repository : org.springframework.data.jpa.repository.JpaRepository<Entity858, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity858>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity858>
}
...
class Service858(private val repo: Entity858Repository) {
    ...
    fun findActive(): List<Entity858> = repo.findByField4(true)
    fun search(query: String): List<Entity858> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity858>): Map<String, List<Entity858>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity858): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity858")
class Controller858(private val service: Service858) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity858) = service.save(entity)
    @DeleteMapping("/{id}")
