...
interface Entity845Repository : org.springframework.data.jpa.repository.JpaRepository<Entity845, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity845>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity845>
}
...
class Service845(private val repo: Entity845Repository) {
    ...
    fun findActive(): List<Entity845> = repo.findByField4(true)
    fun search(query: String): List<Entity845> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity845>): Map<String, List<Entity845>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity845): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity845")
class Controller845(private val service: Service845) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity845) = service.save(entity)
    @DeleteMapping("/{id}")
