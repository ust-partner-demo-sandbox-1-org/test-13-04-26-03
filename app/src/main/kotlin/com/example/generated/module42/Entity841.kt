...
interface Entity841Repository : org.springframework.data.jpa.repository.JpaRepository<Entity841, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity841>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity841>
}
...
class Service841(private val repo: Entity841Repository) {
    ...
    fun findActive(): List<Entity841> = repo.findByField4(true)
    fun search(query: String): List<Entity841> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity841>): Map<String, List<Entity841>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity841): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity841")
class Controller841(private val service: Service841) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity841) = service.save(entity)
    @DeleteMapping("/{id}")
