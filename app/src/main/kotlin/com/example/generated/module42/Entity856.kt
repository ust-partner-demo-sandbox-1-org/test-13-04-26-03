...
interface Entity856Repository : org.springframework.data.jpa.repository.JpaRepository<Entity856, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity856>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity856>
}
...
class Service856(private val repo: Entity856Repository) {
    ...
    fun findActive(): List<Entity856> = repo.findByField4(true)
    fun search(query: String): List<Entity856> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity856>): Map<String, List<Entity856>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity856): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity856")
class Controller856(private val service: Service856) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity856) = service.save(entity)
    @DeleteMapping("/{id}")
