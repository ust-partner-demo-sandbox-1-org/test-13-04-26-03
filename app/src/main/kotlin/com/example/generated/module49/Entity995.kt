...
interface Entity995Repository : org.springframework.data.jpa.repository.JpaRepository<Entity995, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity995>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity995>
}
...
class Service995(private val repo: Entity995Repository) {
    ...
    fun findActive(): List<Entity995> = repo.findByField4(true)
    fun search(query: String): List<Entity995> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity995>): Map<String, List<Entity995>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity995): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity995")
class Controller995(private val service: Service995) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity995) = service.save(entity)
    @DeleteMapping("/{id}")
