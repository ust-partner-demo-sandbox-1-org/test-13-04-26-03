...
interface Entity990Repository : org.springframework.data.jpa.repository.JpaRepository<Entity990, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity990>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity990>
}
...
class Service990(private val repo: Entity990Repository) {
    ...
    fun findActive(): List<Entity990> = repo.findByField4(true)
    fun search(query: String): List<Entity990> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity990>): Map<String, List<Entity990>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity990): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity990")
class Controller990(private val service: Service990) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity990) = service.save(entity)
    @DeleteMapping("/{id}")
