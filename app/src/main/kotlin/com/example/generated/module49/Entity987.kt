...
interface Entity987Repository : org.springframework.data.jpa.repository.JpaRepository<Entity987, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity987>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity987>
}
...
class Service987(private val repo: Entity987Repository) {
    ...
    fun findActive(): List<Entity987> = repo.findByField4(true)
    fun search(query: String): List<Entity987> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity987>): Map<String, List<Entity987>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity987): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity987")
class Controller987(private val service: Service987) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity987) = service.save(entity)
    @DeleteMapping("/{id}")
