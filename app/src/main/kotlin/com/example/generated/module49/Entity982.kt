...
interface Entity982Repository : org.springframework.data.jpa.repository.JpaRepository<Entity982, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity982>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity982>
}
...
class Service982(private val repo: Entity982Repository) {
    ...
    fun findActive(): List<Entity982> = repo.findByField4(true)
    fun search(query: String): List<Entity982> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity982>): Map<String, List<Entity982>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity982): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity982")
class Controller982(private val service: Service982) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity982) = service.save(entity)
    @DeleteMapping("/{id}")
