...
interface Entity997Repository : org.springframework.data.jpa.repository.JpaRepository<Entity997, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity997>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity997>
}
...
class Service997(private val repo: Entity997Repository) {
    ...
    fun findActive(): List<Entity997> = repo.findByField4(true)
    fun search(query: String): List<Entity997> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity997>): Map<String, List<Entity997>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity997): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity997")
class Controller997(private val service: Service997) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity997) = service.save(entity)
    @DeleteMapping("/{id}")
