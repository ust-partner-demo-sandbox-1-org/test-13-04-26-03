...
interface Entity991Repository : org.springframework.data.jpa.repository.JpaRepository<Entity991, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity991>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity991>
}
...
class Service991(private val repo: Entity991Repository) {
    ...
    fun findActive(): List<Entity991> = repo.findByField4(true)
    fun search(query: String): List<Entity991> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity991>): Map<String, List<Entity991>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity991): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity991")
class Controller991(private val service: Service991) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity991) = service.save(entity)
    @DeleteMapping("/{id}")
