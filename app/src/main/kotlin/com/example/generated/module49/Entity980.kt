...
interface Entity980Repository : org.springframework.data.jpa.repository.JpaRepository<Entity980, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity980>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity980>
}
...
class Service980(private val repo: Entity980Repository) {
    ...
    fun findActive(): List<Entity980> = repo.findByField4(true)
    fun search(query: String): List<Entity980> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity980>): Map<String, List<Entity980>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity980): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity980")
class Controller980(private val service: Service980) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity980) = service.save(entity)
    @DeleteMapping("/{id}")
