...
interface Entity996Repository : org.springframework.data.jpa.repository.JpaRepository<Entity996, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity996>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity996>
}
...
class Service996(private val repo: Entity996Repository) {
    ...
    fun findActive(): List<Entity996> = repo.findByField4(true)
    fun search(query: String): List<Entity996> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity996>): Map<String, List<Entity996>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity996): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity996")
class Controller996(private val service: Service996) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity996) = service.save(entity)
    @DeleteMapping("/{id}")
