...
interface Entity986Repository : org.springframework.data.jpa.repository.JpaRepository<Entity986, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity986>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity986>
}
...
class Service986(private val repo: Entity986Repository) {
    ...
    fun findActive(): List<Entity986> = repo.findByField4(true)
    fun search(query: String): List<Entity986> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity986>): Map<String, List<Entity986>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity986): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity986")
class Controller986(private val service: Service986) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity986) = service.save(entity)
    @DeleteMapping("/{id}")
