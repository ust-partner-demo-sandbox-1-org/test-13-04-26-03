...
interface Entity988Repository : org.springframework.data.jpa.repository.JpaRepository<Entity988, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity988>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity988>
}
...
class Service988(private val repo: Entity988Repository) {
    ...
    fun findActive(): List<Entity988> = repo.findByField4(true)
    fun search(query: String): List<Entity988> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity988>): Map<String, List<Entity988>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity988): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity988")
class Controller988(private val service: Service988) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity988) = service.save(entity)
    @DeleteMapping("/{id}")
