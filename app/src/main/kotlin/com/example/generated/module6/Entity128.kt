...
interface Entity128Repository : org.springframework.data.jpa.repository.JpaRepository<Entity128, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity128>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity128>
}
...
class Service128(private val repo: Entity128Repository) {
    ...
    fun findActive(): List<Entity128> = repo.findByField4(true)
    fun search(query: String): List<Entity128> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity128>): Map<String, List<Entity128>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity128): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity128")
class Controller128(private val service: Service128) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity128) = service.save(entity)
    @DeleteMapping("/{id}")
