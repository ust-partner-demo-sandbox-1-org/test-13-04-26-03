...
interface Entity127Repository : org.springframework.data.jpa.repository.JpaRepository<Entity127, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity127>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity127>
}
...
class Service127(private val repo: Entity127Repository) {
    ...
    fun findActive(): List<Entity127> = repo.findByField4(true)
    fun search(query: String): List<Entity127> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity127>): Map<String, List<Entity127>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity127): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity127")
class Controller127(private val service: Service127) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity127) = service.save(entity)
    @DeleteMapping("/{id}")
