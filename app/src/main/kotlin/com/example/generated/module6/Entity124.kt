...
interface Entity124Repository : org.springframework.data.jpa.repository.JpaRepository<Entity124, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity124>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity124>
}
...
class Service124(private val repo: Entity124Repository) {
    ...
    fun findActive(): List<Entity124> = repo.findByField4(true)
    fun search(query: String): List<Entity124> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity124>): Map<String, List<Entity124>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity124): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity124")
class Controller124(private val service: Service124) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity124) = service.save(entity)
    @DeleteMapping("/{id}")
