...
interface Entity138Repository : org.springframework.data.jpa.repository.JpaRepository<Entity138, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity138>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity138>
}
...
class Service138(private val repo: Entity138Repository) {
    ...
    fun findActive(): List<Entity138> = repo.findByField4(true)
    fun search(query: String): List<Entity138> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity138>): Map<String, List<Entity138>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity138): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity138")
class Controller138(private val service: Service138) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity138) = service.save(entity)
    @DeleteMapping("/{id}")
