...
interface Entity131Repository : org.springframework.data.jpa.repository.JpaRepository<Entity131, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity131>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity131>
}
...
class Service131(private val repo: Entity131Repository) {
    ...
    fun findActive(): List<Entity131> = repo.findByField4(true)
    fun search(query: String): List<Entity131> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity131>): Map<String, List<Entity131>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity131): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity131")
class Controller131(private val service: Service131) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity131) = service.save(entity)
    @DeleteMapping("/{id}")
