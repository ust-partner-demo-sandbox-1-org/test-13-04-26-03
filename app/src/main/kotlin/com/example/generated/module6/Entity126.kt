...
interface Entity126Repository : org.springframework.data.jpa.repository.JpaRepository<Entity126, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity126>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity126>
}
...
class Service126(private val repo: Entity126Repository) {
    ...
    fun findActive(): List<Entity126> = repo.findByField4(true)
    fun search(query: String): List<Entity126> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity126>): Map<String, List<Entity126>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity126): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity126")
class Controller126(private val service: Service126) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity126) = service.save(entity)
    @DeleteMapping("/{id}")
