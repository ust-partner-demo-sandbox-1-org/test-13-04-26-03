...
interface Entity129Repository : org.springframework.data.jpa.repository.JpaRepository<Entity129, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity129>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity129>
}
...
class Service129(private val repo: Entity129Repository) {
    ...
    fun findActive(): List<Entity129> = repo.findByField4(true)
    fun search(query: String): List<Entity129> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity129>): Map<String, List<Entity129>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity129): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity129")
class Controller129(private val service: Service129) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity129) = service.save(entity)
    @DeleteMapping("/{id}")
