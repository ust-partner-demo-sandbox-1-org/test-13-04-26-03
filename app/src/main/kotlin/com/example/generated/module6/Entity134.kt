...
interface Entity134Repository : org.springframework.data.jpa.repository.JpaRepository<Entity134, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity134>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity134>
}
...
class Service134(private val repo: Entity134Repository) {
    ...
    fun findActive(): List<Entity134> = repo.findByField4(true)
    fun search(query: String): List<Entity134> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity134>): Map<String, List<Entity134>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity134): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity134")
class Controller134(private val service: Service134) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity134) = service.save(entity)
    @DeleteMapping("/{id}")
