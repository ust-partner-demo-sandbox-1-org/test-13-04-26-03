...
interface Entity130Repository : org.springframework.data.jpa.repository.JpaRepository<Entity130, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity130>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity130>
}
...
class Service130(private val repo: Entity130Repository) {
    ...
    fun findActive(): List<Entity130> = repo.findByField4(true)
    fun search(query: String): List<Entity130> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity130>): Map<String, List<Entity130>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity130): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity130")
class Controller130(private val service: Service130) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity130) = service.save(entity)
    @DeleteMapping("/{id}")
