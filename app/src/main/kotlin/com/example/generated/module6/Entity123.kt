...
interface Entity123Repository : org.springframework.data.jpa.repository.JpaRepository<Entity123, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity123>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity123>
}
...
class Service123(private val repo: Entity123Repository) {
    ...
    fun findActive(): List<Entity123> = repo.findByField4(true)
    fun search(query: String): List<Entity123> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity123>): Map<String, List<Entity123>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity123): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity123")
class Controller123(private val service: Service123) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity123) = service.save(entity)
    @DeleteMapping("/{id}")
