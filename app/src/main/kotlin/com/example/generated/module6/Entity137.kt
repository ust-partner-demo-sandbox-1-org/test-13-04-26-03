...
interface Entity137Repository : org.springframework.data.jpa.repository.JpaRepository<Entity137, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity137>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity137>
}
...
class Service137(private val repo: Entity137Repository) {
    ...
    fun findActive(): List<Entity137> = repo.findByField4(true)
    fun search(query: String): List<Entity137> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity137>): Map<String, List<Entity137>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity137): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity137")
class Controller137(private val service: Service137) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity137) = service.save(entity)
    @DeleteMapping("/{id}")
