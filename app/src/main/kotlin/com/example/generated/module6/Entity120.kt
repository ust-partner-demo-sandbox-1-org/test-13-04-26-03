...
interface Entity120Repository : org.springframework.data.jpa.repository.JpaRepository<Entity120, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity120>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity120>
}
...
class Service120(private val repo: Entity120Repository) {
    ...
    fun findActive(): List<Entity120> = repo.findByField4(true)
    fun search(query: String): List<Entity120> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity120>): Map<String, List<Entity120>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity120): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity120")
class Controller120(private val service: Service120) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity120) = service.save(entity)
    @DeleteMapping("/{id}")
