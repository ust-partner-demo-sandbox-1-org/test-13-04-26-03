...
interface Entity255Repository : org.springframework.data.jpa.repository.JpaRepository<Entity255, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity255>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity255>
}
...
class Service255(private val repo: Entity255Repository) {
    ...
    fun findActive(): List<Entity255> = repo.findByField4(true)
    fun search(query: String): List<Entity255> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity255>): Map<String, List<Entity255>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity255): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity255")
class Controller255(private val service: Service255) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity255) = service.save(entity)
    @DeleteMapping("/{id}")
