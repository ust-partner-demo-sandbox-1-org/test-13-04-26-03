...
interface Entity248Repository : org.springframework.data.jpa.repository.JpaRepository<Entity248, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity248>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity248>
}
...
class Service248(private val repo: Entity248Repository) {
    ...
    fun findActive(): List<Entity248> = repo.findByField4(true)
    fun search(query: String): List<Entity248> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity248>): Map<String, List<Entity248>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity248): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity248")
class Controller248(private val service: Service248) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity248) = service.save(entity)
    @DeleteMapping("/{id}")
