...
interface Entity251Repository : org.springframework.data.jpa.repository.JpaRepository<Entity251, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity251>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity251>
}
...
class Service251(private val repo: Entity251Repository) {
    ...
    fun findActive(): List<Entity251> = repo.findByField4(true)
    fun search(query: String): List<Entity251> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity251>): Map<String, List<Entity251>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity251): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity251")
class Controller251(private val service: Service251) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity251) = service.save(entity)
    @DeleteMapping("/{id}")
