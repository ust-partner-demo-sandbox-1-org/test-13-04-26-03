...
interface Entity253Repository : org.springframework.data.jpa.repository.JpaRepository<Entity253, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity253>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity253>
}
...
class Service253(private val repo: Entity253Repository) {
    ...
    fun findActive(): List<Entity253> = repo.findByField4(true)
    fun search(query: String): List<Entity253> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity253>): Map<String, List<Entity253>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity253): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity253")
class Controller253(private val service: Service253) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity253) = service.save(entity)
    @DeleteMapping("/{id}")
