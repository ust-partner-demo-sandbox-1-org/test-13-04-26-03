...
interface Entity252Repository : org.springframework.data.jpa.repository.JpaRepository<Entity252, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity252>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity252>
}
...
class Service252(private val repo: Entity252Repository) {
    ...
    fun findActive(): List<Entity252> = repo.findByField4(true)
    fun search(query: String): List<Entity252> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity252>): Map<String, List<Entity252>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity252): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity252")
class Controller252(private val service: Service252) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity252) = service.save(entity)
    @DeleteMapping("/{id}")
