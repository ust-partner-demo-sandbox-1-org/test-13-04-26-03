...
interface Entity257Repository : org.springframework.data.jpa.repository.JpaRepository<Entity257, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity257>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity257>
}
...
class Service257(private val repo: Entity257Repository) {
    ...
    fun findActive(): List<Entity257> = repo.findByField4(true)
    fun search(query: String): List<Entity257> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity257>): Map<String, List<Entity257>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity257): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity257")
class Controller257(private val service: Service257) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity257) = service.save(entity)
    @DeleteMapping("/{id}")
