...
interface Entity241Repository : org.springframework.data.jpa.repository.JpaRepository<Entity241, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity241>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity241>
}
...
class Service241(private val repo: Entity241Repository) {
    ...
    fun findActive(): List<Entity241> = repo.findByField4(true)
    fun search(query: String): List<Entity241> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity241>): Map<String, List<Entity241>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity241): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity241")
class Controller241(private val service: Service241) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity241) = service.save(entity)
    @DeleteMapping("/{id}")
