...
interface Entity259Repository : org.springframework.data.jpa.repository.JpaRepository<Entity259, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity259>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity259>
}
...
class Service259(private val repo: Entity259Repository) {
    ...
    fun findActive(): List<Entity259> = repo.findByField4(true)
    fun search(query: String): List<Entity259> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity259>): Map<String, List<Entity259>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity259): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity259")
class Controller259(private val service: Service259) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity259) = service.save(entity)
    @DeleteMapping("/{id}")
