...
interface Entity244Repository : org.springframework.data.jpa.repository.JpaRepository<Entity244, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity244>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity244>
}
...
class Service244(private val repo: Entity244Repository) {
    ...
    fun findActive(): List<Entity244> = repo.findByField4(true)
    fun search(query: String): List<Entity244> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity244>): Map<String, List<Entity244>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity244): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity244")
class Controller244(private val service: Service244) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity244) = service.save(entity)
    @DeleteMapping("/{id}")
