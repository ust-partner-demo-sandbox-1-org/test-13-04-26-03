...
interface Entity96Repository : org.springframework.data.jpa.repository.JpaRepository<Entity96, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity96>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity96>
}
...
class Service96(private val repo: Entity96Repository) {
    ...
    fun findActive(): List<Entity96> = repo.findByField4(true)
    fun search(query: String): List<Entity96> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity96>): Map<String, List<Entity96>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity96): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity96")
class Controller96(private val service: Service96) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity96) = service.save(entity)
    @DeleteMapping("/{id}")
