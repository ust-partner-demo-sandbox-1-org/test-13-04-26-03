...
interface Entity93Repository : org.springframework.data.jpa.repository.JpaRepository<Entity93, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity93>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity93>
}
...
class Service93(private val repo: Entity93Repository) {
    ...
    fun findActive(): List<Entity93> = repo.findByField4(true)
    fun search(query: String): List<Entity93> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity93>): Map<String, List<Entity93>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity93): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity93")
class Controller93(private val service: Service93) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity93) = service.save(entity)
    @DeleteMapping("/{id}")
