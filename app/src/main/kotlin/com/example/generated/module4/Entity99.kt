...
interface Entity99Repository : org.springframework.data.jpa.repository.JpaRepository<Entity99, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity99>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity99>
}
...
class Service99(private val repo: Entity99Repository) {
    ...
    fun findActive(): List<Entity99> = repo.findByField4(true)
    fun search(query: String): List<Entity99> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity99>): Map<String, List<Entity99>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity99): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity99")
class Controller99(private val service: Service99) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity99) = service.save(entity)
    @DeleteMapping("/{id}")
