...
interface Entity97Repository : org.springframework.data.jpa.repository.JpaRepository<Entity97, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity97>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity97>
}
...
class Service97(private val repo: Entity97Repository) {
    ...
    fun findActive(): List<Entity97> = repo.findByField4(true)
    fun search(query: String): List<Entity97> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity97>): Map<String, List<Entity97>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity97): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity97")
class Controller97(private val service: Service97) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity97) = service.save(entity)
    @DeleteMapping("/{id}")
