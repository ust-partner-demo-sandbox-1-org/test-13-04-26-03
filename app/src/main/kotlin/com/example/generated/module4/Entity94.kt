...
interface Entity94Repository : org.springframework.data.jpa.repository.JpaRepository<Entity94, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity94>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity94>
}
...
class Service94(private val repo: Entity94Repository) {
    ...
    fun findActive(): List<Entity94> = repo.findByField4(true)
    fun search(query: String): List<Entity94> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity94>): Map<String, List<Entity94>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity94): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity94")
class Controller94(private val service: Service94) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity94) = service.save(entity)
    @DeleteMapping("/{id}")
