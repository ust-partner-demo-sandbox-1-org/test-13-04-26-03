...
interface Entity82Repository : org.springframework.data.jpa.repository.JpaRepository<Entity82, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity82>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity82>
}
...
class Service82(private val repo: Entity82Repository) {
    ...
    fun findActive(): List<Entity82> = repo.findByField4(true)
    fun search(query: String): List<Entity82> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity82>): Map<String, List<Entity82>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity82): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity82")
class Controller82(private val service: Service82) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity82) = service.save(entity)
    @DeleteMapping("/{id}")
