...
interface Entity80Repository : org.springframework.data.jpa.repository.JpaRepository<Entity80, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity80>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity80>
}
...
class Service80(private val repo: Entity80Repository) {
    ...
    fun findActive(): List<Entity80> = repo.findByField4(true)
    fun search(query: String): List<Entity80> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity80>): Map<String, List<Entity80>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity80): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity80")
class Controller80(private val service: Service80) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity80) = service.save(entity)
    @DeleteMapping("/{id}")
