...
interface Entity90Repository : org.springframework.data.jpa.repository.JpaRepository<Entity90, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity90>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity90>
}
...
class Service90(private val repo: Entity90Repository) {
    ...
    fun findActive(): List<Entity90> = repo.findByField4(true)
    fun search(query: String): List<Entity90> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity90>): Map<String, List<Entity90>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity90): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity90")
class Controller90(private val service: Service90) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity90) = service.save(entity)
    @DeleteMapping("/{id}")
