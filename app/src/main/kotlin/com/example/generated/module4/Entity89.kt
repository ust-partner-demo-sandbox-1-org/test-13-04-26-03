...
interface Entity89Repository : org.springframework.data.jpa.repository.JpaRepository<Entity89, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity89>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity89>
}
...
class Service89(private val repo: Entity89Repository) {
    ...
    fun findActive(): List<Entity89> = repo.findByField4(true)
    fun search(query: String): List<Entity89> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity89>): Map<String, List<Entity89>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity89): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity89")
class Controller89(private val service: Service89) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity89) = service.save(entity)
    @DeleteMapping("/{id}")
