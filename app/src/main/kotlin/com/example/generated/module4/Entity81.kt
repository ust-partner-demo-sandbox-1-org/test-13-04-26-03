...
interface Entity81Repository : org.springframework.data.jpa.repository.JpaRepository<Entity81, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity81>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity81>
}
...
class Service81(private val repo: Entity81Repository) {
    ...
    fun findActive(): List<Entity81> = repo.findByField4(true)
    fun search(query: String): List<Entity81> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity81>): Map<String, List<Entity81>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity81): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity81")
class Controller81(private val service: Service81) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity81) = service.save(entity)
    @DeleteMapping("/{id}")
