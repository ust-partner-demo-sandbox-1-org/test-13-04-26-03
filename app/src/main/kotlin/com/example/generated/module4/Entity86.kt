...
interface Entity86Repository : org.springframework.data.jpa.repository.JpaRepository<Entity86, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity86>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity86>
}
...
class Service86(private val repo: Entity86Repository) {
    ...
    fun findActive(): List<Entity86> = repo.findByField4(true)
    fun search(query: String): List<Entity86> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity86>): Map<String, List<Entity86>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity86): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity86")
class Controller86(private val service: Service86) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity86) = service.save(entity)
    @DeleteMapping("/{id}")
