...
interface Entity352Repository : org.springframework.data.jpa.repository.JpaRepository<Entity352, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity352>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity352>
}
...
class Service352(private val repo: Entity352Repository) {
    ...
    fun findActive(): List<Entity352> = repo.findByField4(true)
    fun search(query: String): List<Entity352> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity352>): Map<String, List<Entity352>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity352): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity352")
class Controller352(private val service: Service352) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity352) = service.save(entity)
    @DeleteMapping("/{id}")
