...
interface Entity356Repository : org.springframework.data.jpa.repository.JpaRepository<Entity356, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity356>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity356>
}
...
class Service356(private val repo: Entity356Repository) {
    ...
    fun findActive(): List<Entity356> = repo.findByField4(true)
    fun search(query: String): List<Entity356> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity356>): Map<String, List<Entity356>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity356): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity356")
class Controller356(private val service: Service356) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity356) = service.save(entity)
    @DeleteMapping("/{id}")
