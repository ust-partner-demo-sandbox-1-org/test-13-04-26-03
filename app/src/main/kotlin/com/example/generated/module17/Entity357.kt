...
interface Entity357Repository : org.springframework.data.jpa.repository.JpaRepository<Entity357, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity357>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity357>
}
...
class Service357(private val repo: Entity357Repository) {
    ...
    fun findActive(): List<Entity357> = repo.findByField4(true)
    fun search(query: String): List<Entity357> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity357>): Map<String, List<Entity357>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity357): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity357")
class Controller357(private val service: Service357) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity357) = service.save(entity)
    @DeleteMapping("/{id}")
