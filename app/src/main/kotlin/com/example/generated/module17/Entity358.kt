...
interface Entity358Repository : org.springframework.data.jpa.repository.JpaRepository<Entity358, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity358>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity358>
}
...
class Service358(private val repo: Entity358Repository) {
    ...
    fun findActive(): List<Entity358> = repo.findByField4(true)
    fun search(query: String): List<Entity358> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity358>): Map<String, List<Entity358>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity358): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity358")
class Controller358(private val service: Service358) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity358) = service.save(entity)
    @DeleteMapping("/{id}")
