...
interface Entity347Repository : org.springframework.data.jpa.repository.JpaRepository<Entity347, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity347>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity347>
}
...
class Service347(private val repo: Entity347Repository) {
    ...
    fun findActive(): List<Entity347> = repo.findByField4(true)
    fun search(query: String): List<Entity347> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity347>): Map<String, List<Entity347>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity347): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity347")
class Controller347(private val service: Service347) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity347) = service.save(entity)
    @DeleteMapping("/{id}")
