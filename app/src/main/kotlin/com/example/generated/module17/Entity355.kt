...
interface Entity355Repository : org.springframework.data.jpa.repository.JpaRepository<Entity355, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity355>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity355>
}
...
class Service355(private val repo: Entity355Repository) {
    ...
    fun findActive(): List<Entity355> = repo.findByField4(true)
    fun search(query: String): List<Entity355> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity355>): Map<String, List<Entity355>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity355): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity355")
class Controller355(private val service: Service355) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity355) = service.save(entity)
    @DeleteMapping("/{id}")
