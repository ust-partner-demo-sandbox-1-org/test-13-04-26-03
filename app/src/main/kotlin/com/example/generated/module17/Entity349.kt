...
interface Entity349Repository : org.springframework.data.jpa.repository.JpaRepository<Entity349, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity349>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity349>
}
...
class Service349(private val repo: Entity349Repository) {
    ...
    fun findActive(): List<Entity349> = repo.findByField4(true)
    fun search(query: String): List<Entity349> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity349>): Map<String, List<Entity349>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity349): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity349")
class Controller349(private val service: Service349) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity349) = service.save(entity)
    @DeleteMapping("/{id}")
