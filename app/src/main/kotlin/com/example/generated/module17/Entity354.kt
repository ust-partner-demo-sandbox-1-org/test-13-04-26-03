...
interface Entity354Repository : org.springframework.data.jpa.repository.JpaRepository<Entity354, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity354>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity354>
}
...
class Service354(private val repo: Entity354Repository) {
    ...
    fun findActive(): List<Entity354> = repo.findByField4(true)
    fun search(query: String): List<Entity354> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity354>): Map<String, List<Entity354>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity354): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity354")
class Controller354(private val service: Service354) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity354) = service.save(entity)
    @DeleteMapping("/{id}")
