...
interface Entity340Repository : org.springframework.data.jpa.repository.JpaRepository<Entity340, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity340>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity340>
}
...
class Service340(private val repo: Entity340Repository) {
    ...
    fun findActive(): List<Entity340> = repo.findByField4(true)
    fun search(query: String): List<Entity340> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity340>): Map<String, List<Entity340>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity340): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity340")
class Controller340(private val service: Service340) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity340) = service.save(entity)
    @DeleteMapping("/{id}")
