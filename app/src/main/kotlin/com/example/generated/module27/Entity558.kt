...
interface Entity558Repository : org.springframework.data.jpa.repository.JpaRepository<Entity558, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity558>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity558>
}
...
class Service558(private val repo: Entity558Repository) {
    ...
    fun findActive(): List<Entity558> = repo.findByField4(true)
    fun search(query: String): List<Entity558> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity558>): Map<String, List<Entity558>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity558): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity558")
class Controller558(private val service: Service558) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity558) = service.save(entity)
    @DeleteMapping("/{id}")
