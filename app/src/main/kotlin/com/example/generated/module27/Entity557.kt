...
interface Entity557Repository : org.springframework.data.jpa.repository.JpaRepository<Entity557, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity557>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity557>
}
...
class Service557(private val repo: Entity557Repository) {
    ...
    fun findActive(): List<Entity557> = repo.findByField4(true)
    fun search(query: String): List<Entity557> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity557>): Map<String, List<Entity557>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity557): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity557")
class Controller557(private val service: Service557) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity557) = service.save(entity)
    @DeleteMapping("/{id}")
