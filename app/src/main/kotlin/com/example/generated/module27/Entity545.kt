...
interface Entity545Repository : org.springframework.data.jpa.repository.JpaRepository<Entity545, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity545>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity545>
}
...
class Service545(private val repo: Entity545Repository) {
    ...
    fun findActive(): List<Entity545> = repo.findByField4(true)
    fun search(query: String): List<Entity545> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity545>): Map<String, List<Entity545>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity545): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity545")
class Controller545(private val service: Service545) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity545) = service.save(entity)
    @DeleteMapping("/{id}")
