...
interface Entity547Repository : org.springframework.data.jpa.repository.JpaRepository<Entity547, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity547>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity547>
}
...
class Service547(private val repo: Entity547Repository) {
    ...
    fun findActive(): List<Entity547> = repo.findByField4(true)
    fun search(query: String): List<Entity547> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity547>): Map<String, List<Entity547>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity547): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity547")
class Controller547(private val service: Service547) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity547) = service.save(entity)
    @DeleteMapping("/{id}")
