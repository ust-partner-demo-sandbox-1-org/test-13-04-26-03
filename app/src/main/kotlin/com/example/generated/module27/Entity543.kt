...
interface Entity543Repository : org.springframework.data.jpa.repository.JpaRepository<Entity543, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity543>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity543>
}
...
class Service543(private val repo: Entity543Repository) {
    ...
    fun findActive(): List<Entity543> = repo.findByField4(true)
    fun search(query: String): List<Entity543> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity543>): Map<String, List<Entity543>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity543): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity543")
class Controller543(private val service: Service543) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity543) = service.save(entity)
    @DeleteMapping("/{id}")
