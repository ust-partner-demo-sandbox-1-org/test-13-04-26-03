...
interface Entity559Repository : org.springframework.data.jpa.repository.JpaRepository<Entity559, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity559>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity559>
}
...
class Service559(private val repo: Entity559Repository) {
    ...
    fun findActive(): List<Entity559> = repo.findByField4(true)
    fun search(query: String): List<Entity559> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity559>): Map<String, List<Entity559>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity559): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity559")
class Controller559(private val service: Service559) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity559) = service.save(entity)
    @DeleteMapping("/{id}")
