...
interface Entity544Repository : org.springframework.data.jpa.repository.JpaRepository<Entity544, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity544>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity544>
}
...
class Service544(private val repo: Entity544Repository) {
    ...
    fun findActive(): List<Entity544> = repo.findByField4(true)
    fun search(query: String): List<Entity544> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity544>): Map<String, List<Entity544>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity544): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity544")
class Controller544(private val service: Service544) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity544) = service.save(entity)
    @DeleteMapping("/{id}")
