...
interface Entity548Repository : org.springframework.data.jpa.repository.JpaRepository<Entity548, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity548>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity548>
}
...
class Service548(private val repo: Entity548Repository) {
    ...
    fun findActive(): List<Entity548> = repo.findByField4(true)
    fun search(query: String): List<Entity548> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity548>): Map<String, List<Entity548>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity548): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity548")
class Controller548(private val service: Service548) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity548) = service.save(entity)
    @DeleteMapping("/{id}")
