...
interface Entity554Repository : org.springframework.data.jpa.repository.JpaRepository<Entity554, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity554>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity554>
}
...
class Service554(private val repo: Entity554Repository) {
    ...
    fun findActive(): List<Entity554> = repo.findByField4(true)
    fun search(query: String): List<Entity554> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity554>): Map<String, List<Entity554>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity554): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity554")
class Controller554(private val service: Service554) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity554) = service.save(entity)
    @DeleteMapping("/{id}")
