...
interface Entity150Repository : org.springframework.data.jpa.repository.JpaRepository<Entity150, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity150>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity150>
}
...
class Service150(private val repo: Entity150Repository) {
    ...
    fun findActive(): List<Entity150> = repo.findByField4(true)
    fun search(query: String): List<Entity150> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity150>): Map<String, List<Entity150>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity150): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity150")
class Controller150(private val service: Service150) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity150) = service.save(entity)
    @DeleteMapping("/{id}")
