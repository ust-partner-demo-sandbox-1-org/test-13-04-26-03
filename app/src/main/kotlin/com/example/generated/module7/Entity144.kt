...
interface Entity144Repository : org.springframework.data.jpa.repository.JpaRepository<Entity144, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity144>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity144>
}
...
class Service144(private val repo: Entity144Repository) {
    ...
    fun findActive(): List<Entity144> = repo.findByField4(true)
    fun search(query: String): List<Entity144> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity144>): Map<String, List<Entity144>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity144): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity144")
class Controller144(private val service: Service144) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity144) = service.save(entity)
    @DeleteMapping("/{id}")
