...
interface Entity141Repository : org.springframework.data.jpa.repository.JpaRepository<Entity141, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity141>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity141>
}
...
class Service141(private val repo: Entity141Repository) {
    ...
    fun findActive(): List<Entity141> = repo.findByField4(true)
    fun search(query: String): List<Entity141> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity141>): Map<String, List<Entity141>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity141): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity141")
class Controller141(private val service: Service141) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity141) = service.save(entity)
    @DeleteMapping("/{id}")
