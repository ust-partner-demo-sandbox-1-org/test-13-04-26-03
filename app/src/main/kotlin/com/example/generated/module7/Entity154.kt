...
interface Entity154Repository : org.springframework.data.jpa.repository.JpaRepository<Entity154, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity154>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity154>
}
...
class Service154(private val repo: Entity154Repository) {
    ...
    fun findActive(): List<Entity154> = repo.findByField4(true)
    fun search(query: String): List<Entity154> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity154>): Map<String, List<Entity154>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity154): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity154")
class Controller154(private val service: Service154) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity154) = service.save(entity)
    @DeleteMapping("/{id}")
