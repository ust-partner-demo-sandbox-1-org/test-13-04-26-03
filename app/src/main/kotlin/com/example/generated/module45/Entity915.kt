...
interface Entity915Repository : org.springframework.data.jpa.repository.JpaRepository<Entity915, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity915>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity915>
}
...
class Service915(private val repo: Entity915Repository) {
    ...
    fun findActive(): List<Entity915> = repo.findByField4(true)
    fun search(query: String): List<Entity915> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity915>): Map<String, List<Entity915>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity915): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity915")
class Controller915(private val service: Service915) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity915) = service.save(entity)
    @DeleteMapping("/{id}")
