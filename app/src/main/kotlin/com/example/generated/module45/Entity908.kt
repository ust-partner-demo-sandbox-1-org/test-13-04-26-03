...
interface Entity908Repository : org.springframework.data.jpa.repository.JpaRepository<Entity908, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity908>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity908>
}
...
class Service908(private val repo: Entity908Repository) {
    ...
    fun findActive(): List<Entity908> = repo.findByField4(true)
    fun search(query: String): List<Entity908> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity908>): Map<String, List<Entity908>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity908): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity908")
class Controller908(private val service: Service908) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity908) = service.save(entity)
    @DeleteMapping("/{id}")
