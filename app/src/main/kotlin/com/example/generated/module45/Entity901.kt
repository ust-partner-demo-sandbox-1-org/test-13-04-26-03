...
interface Entity901Repository : org.springframework.data.jpa.repository.JpaRepository<Entity901, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity901>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity901>
}
...
class Service901(private val repo: Entity901Repository) {
    ...
    fun findActive(): List<Entity901> = repo.findByField4(true)
    fun search(query: String): List<Entity901> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity901>): Map<String, List<Entity901>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity901): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity901")
class Controller901(private val service: Service901) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity901) = service.save(entity)
    @DeleteMapping("/{id}")
