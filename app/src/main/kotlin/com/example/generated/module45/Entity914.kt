...
interface Entity914Repository : org.springframework.data.jpa.repository.JpaRepository<Entity914, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity914>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity914>
}
...
class Service914(private val repo: Entity914Repository) {
    ...
    fun findActive(): List<Entity914> = repo.findByField4(true)
    fun search(query: String): List<Entity914> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity914>): Map<String, List<Entity914>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity914): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity914")
class Controller914(private val service: Service914) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity914) = service.save(entity)
    @DeleteMapping("/{id}")
