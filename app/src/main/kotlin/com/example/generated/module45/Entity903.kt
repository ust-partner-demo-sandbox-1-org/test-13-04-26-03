...
interface Entity903Repository : org.springframework.data.jpa.repository.JpaRepository<Entity903, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity903>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity903>
}
...
class Service903(private val repo: Entity903Repository) {
    ...
    fun findActive(): List<Entity903> = repo.findByField4(true)
    fun search(query: String): List<Entity903> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity903>): Map<String, List<Entity903>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity903): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity903")
class Controller903(private val service: Service903) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity903) = service.save(entity)
    @DeleteMapping("/{id}")
