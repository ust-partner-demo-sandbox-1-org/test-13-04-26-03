...
interface Entity917Repository : org.springframework.data.jpa.repository.JpaRepository<Entity917, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity917>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity917>
}
...
class Service917(private val repo: Entity917Repository) {
    ...
    fun findActive(): List<Entity917> = repo.findByField4(true)
    fun search(query: String): List<Entity917> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity917>): Map<String, List<Entity917>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity917): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity917")
class Controller917(private val service: Service917) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity917) = service.save(entity)
    @DeleteMapping("/{id}")
