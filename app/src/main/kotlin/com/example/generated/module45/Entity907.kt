...
interface Entity907Repository : org.springframework.data.jpa.repository.JpaRepository<Entity907, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity907>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity907>
}
...
class Service907(private val repo: Entity907Repository) {
    ...
    fun findActive(): List<Entity907> = repo.findByField4(true)
    fun search(query: String): List<Entity907> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity907>): Map<String, List<Entity907>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity907): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity907")
class Controller907(private val service: Service907) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity907) = service.save(entity)
    @DeleteMapping("/{id}")
