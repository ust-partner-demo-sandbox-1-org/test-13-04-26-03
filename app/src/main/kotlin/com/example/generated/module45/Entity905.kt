...
interface Entity905Repository : org.springframework.data.jpa.repository.JpaRepository<Entity905, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity905>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity905>
}
...
class Service905(private val repo: Entity905Repository) {
    ...
    fun findActive(): List<Entity905> = repo.findByField4(true)
    fun search(query: String): List<Entity905> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity905>): Map<String, List<Entity905>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity905): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity905")
class Controller905(private val service: Service905) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity905) = service.save(entity)
    @DeleteMapping("/{id}")
