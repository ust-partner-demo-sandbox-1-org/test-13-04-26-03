...
interface Entity10Repository : org.springframework.data.jpa.repository.JpaRepository<Entity10, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity10>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity10>
}
...
class Service10(private val repo: Entity10Repository) {
    ...
    fun findActive(): List<Entity10> = repo.findByField4(true)
    fun search(query: String): List<Entity10> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity10>): Map<String, List<Entity10>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity10): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity10")
class Controller10(private val service: Service10) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity10) = service.save(entity)
    @DeleteMapping("/{id}")
