...
interface Entity4Repository : org.springframework.data.jpa.repository.JpaRepository<Entity4, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity4>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity4>
}
...
class Service4(private val repo: Entity4Repository) {
    ...
    fun findActive(): List<Entity4> = repo.findByField4(true)
    fun search(query: String): List<Entity4> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity4>): Map<String, List<Entity4>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity4): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity4")
class Controller4(private val service: Service4) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity4) = service.save(entity)
    @DeleteMapping("/{id}")
