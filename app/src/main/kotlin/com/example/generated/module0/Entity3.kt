...
interface Entity3Repository : org.springframework.data.jpa.repository.JpaRepository<Entity3, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity3>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity3>
}
...
class Service3(private val repo: Entity3Repository) {
    ...
    fun findActive(): List<Entity3> = repo.findByField4(true)
    fun search(query: String): List<Entity3> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity3>): Map<String, List<Entity3>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity3): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity3")
class Controller3(private val service: Service3) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity3) = service.save(entity)
    @DeleteMapping("/{id}")
