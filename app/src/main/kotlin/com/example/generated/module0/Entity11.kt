...
interface Entity11Repository : org.springframework.data.jpa.repository.JpaRepository<Entity11, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity11>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity11>
}
...
class Service11(private val repo: Entity11Repository) {
    ...
    fun findActive(): List<Entity11> = repo.findByField4(true)
    fun search(query: String): List<Entity11> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity11>): Map<String, List<Entity11>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity11): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity11")
class Controller11(private val service: Service11) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity11) = service.save(entity)
    @DeleteMapping("/{id}")
