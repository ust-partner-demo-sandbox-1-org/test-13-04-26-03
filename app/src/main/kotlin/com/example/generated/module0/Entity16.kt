...
interface Entity16Repository : org.springframework.data.jpa.repository.JpaRepository<Entity16, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity16>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity16>
}
...
class Service16(private val repo: Entity16Repository) {
    ...
    fun findActive(): List<Entity16> = repo.findByField4(true)
    fun search(query: String): List<Entity16> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity16>): Map<String, List<Entity16>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity16): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity16")
class Controller16(private val service: Service16) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity16) = service.save(entity)
    @DeleteMapping("/{id}")
