...
interface Entity17Repository : org.springframework.data.jpa.repository.JpaRepository<Entity17, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity17>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity17>
}
...
class Service17(private val repo: Entity17Repository) {
    ...
    fun findActive(): List<Entity17> = repo.findByField4(true)
    fun search(query: String): List<Entity17> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity17>): Map<String, List<Entity17>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity17): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity17")
class Controller17(private val service: Service17) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity17) = service.save(entity)
    @DeleteMapping("/{id}")
