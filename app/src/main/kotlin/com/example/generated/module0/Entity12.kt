...
interface Entity12Repository : org.springframework.data.jpa.repository.JpaRepository<Entity12, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity12>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity12>
}
...
class Service12(private val repo: Entity12Repository) {
    ...
    fun findActive(): List<Entity12> = repo.findByField4(true)
    fun search(query: String): List<Entity12> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity12>): Map<String, List<Entity12>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity12): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity12")
class Controller12(private val service: Service12) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity12) = service.save(entity)
    @DeleteMapping("/{id}")
