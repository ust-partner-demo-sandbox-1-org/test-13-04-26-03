...
interface Entity1Repository : org.springframework.data.jpa.repository.JpaRepository<Entity1, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity1>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity1>
}
...
class Service1(private val repo: Entity1Repository) {
    ...
    fun findActive(): List<Entity1> = repo.findByField4(true)
    fun search(query: String): List<Entity1> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity1>): Map<String, List<Entity1>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity1): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity1")
class Controller1(private val service: Service1) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity1) = service.save(entity)
    @DeleteMapping("/{id}")
