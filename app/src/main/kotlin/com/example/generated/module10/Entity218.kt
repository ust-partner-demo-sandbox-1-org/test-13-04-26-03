...
interface Entity218Repository : org.springframework.data.jpa.repository.JpaRepository<Entity218, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity218>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity218>
}
...
class Service218(private val repo: Entity218Repository) {
    ...
    fun findActive(): List<Entity218> = repo.findByField4(true)
    fun search(query: String): List<Entity218> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity218>): Map<String, List<Entity218>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity218): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity218")
class Controller218(private val service: Service218) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity218) = service.save(entity)
    @DeleteMapping("/{id}")
