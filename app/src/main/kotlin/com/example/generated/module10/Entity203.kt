...
interface Entity203Repository : org.springframework.data.jpa.repository.JpaRepository<Entity203, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity203>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity203>
}
...
class Service203(private val repo: Entity203Repository) {
    ...
    fun findActive(): List<Entity203> = repo.findByField4(true)
    fun search(query: String): List<Entity203> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity203>): Map<String, List<Entity203>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity203): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity203")
class Controller203(private val service: Service203) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity203) = service.save(entity)
    @DeleteMapping("/{id}")
