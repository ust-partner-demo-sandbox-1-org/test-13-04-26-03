...
interface Entity206Repository : org.springframework.data.jpa.repository.JpaRepository<Entity206, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity206>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity206>
}
...
class Service206(private val repo: Entity206Repository) {
    ...
    fun findActive(): List<Entity206> = repo.findByField4(true)
    fun search(query: String): List<Entity206> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity206>): Map<String, List<Entity206>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity206): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity206")
class Controller206(private val service: Service206) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity206) = service.save(entity)
    @DeleteMapping("/{id}")
