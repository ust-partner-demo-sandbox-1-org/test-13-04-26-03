...
interface Entity209Repository : org.springframework.data.jpa.repository.JpaRepository<Entity209, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity209>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity209>
}
...
class Service209(private val repo: Entity209Repository) {
    ...
    fun findActive(): List<Entity209> = repo.findByField4(true)
    fun search(query: String): List<Entity209> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity209>): Map<String, List<Entity209>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity209): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity209")
class Controller209(private val service: Service209) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity209) = service.save(entity)
    @DeleteMapping("/{id}")
