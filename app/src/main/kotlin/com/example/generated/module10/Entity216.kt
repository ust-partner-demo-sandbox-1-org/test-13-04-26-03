...
interface Entity216Repository : org.springframework.data.jpa.repository.JpaRepository<Entity216, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity216>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity216>
}
...
class Service216(private val repo: Entity216Repository) {
    ...
    fun findActive(): List<Entity216> = repo.findByField4(true)
    fun search(query: String): List<Entity216> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity216>): Map<String, List<Entity216>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity216): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity216")
class Controller216(private val service: Service216) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity216) = service.save(entity)
    @DeleteMapping("/{id}")
