...
interface Entity215Repository : org.springframework.data.jpa.repository.JpaRepository<Entity215, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity215>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity215>
}
...
class Service215(private val repo: Entity215Repository) {
    ...
    fun findActive(): List<Entity215> = repo.findByField4(true)
    fun search(query: String): List<Entity215> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity215>): Map<String, List<Entity215>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity215): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity215")
class Controller215(private val service: Service215) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity215) = service.save(entity)
    @DeleteMapping("/{id}")
