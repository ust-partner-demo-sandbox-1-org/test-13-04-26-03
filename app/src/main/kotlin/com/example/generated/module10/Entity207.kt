...
interface Entity207Repository : org.springframework.data.jpa.repository.JpaRepository<Entity207, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity207>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity207>
}
...
class Service207(private val repo: Entity207Repository) {
    ...
    fun findActive(): List<Entity207> = repo.findByField4(true)
    fun search(query: String): List<Entity207> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity207>): Map<String, List<Entity207>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity207): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity207")
class Controller207(private val service: Service207) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity207) = service.save(entity)
    @DeleteMapping("/{id}")
