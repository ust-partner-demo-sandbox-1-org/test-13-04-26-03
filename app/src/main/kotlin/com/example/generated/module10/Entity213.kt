...
interface Entity213Repository : org.springframework.data.jpa.repository.JpaRepository<Entity213, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity213>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity213>
}
...
class Service213(private val repo: Entity213Repository) {
    ...
    fun findActive(): List<Entity213> = repo.findByField4(true)
    fun search(query: String): List<Entity213> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity213>): Map<String, List<Entity213>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity213): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity213")
class Controller213(private val service: Service213) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity213) = service.save(entity)
    @DeleteMapping("/{id}")
