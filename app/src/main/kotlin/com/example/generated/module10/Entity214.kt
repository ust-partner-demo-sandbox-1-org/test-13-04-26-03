...
interface Entity214Repository : org.springframework.data.jpa.repository.JpaRepository<Entity214, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity214>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity214>
}
...
class Service214(private val repo: Entity214Repository) {
    ...
    fun findActive(): List<Entity214> = repo.findByField4(true)
    fun search(query: String): List<Entity214> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity214>): Map<String, List<Entity214>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity214): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity214")
class Controller214(private val service: Service214) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity214) = service.save(entity)
    @DeleteMapping("/{id}")
