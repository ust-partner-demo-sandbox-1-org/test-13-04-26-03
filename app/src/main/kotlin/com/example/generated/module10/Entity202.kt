...
interface Entity202Repository : org.springframework.data.jpa.repository.JpaRepository<Entity202, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity202>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity202>
}
...
class Service202(private val repo: Entity202Repository) {
    ...
    fun findActive(): List<Entity202> = repo.findByField4(true)
    fun search(query: String): List<Entity202> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity202>): Map<String, List<Entity202>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity202): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity202")
class Controller202(private val service: Service202) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity202) = service.save(entity)
    @DeleteMapping("/{id}")
