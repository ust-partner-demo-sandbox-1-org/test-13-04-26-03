...
interface Entity874Repository : org.springframework.data.jpa.repository.JpaRepository<Entity874, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity874>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity874>
}
...
class Service874(private val repo: Entity874Repository) {
    ...
    fun findActive(): List<Entity874> = repo.findByField4(true)
    fun search(query: String): List<Entity874> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity874>): Map<String, List<Entity874>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity874): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity874")
class Controller874(private val service: Service874) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity874) = service.save(entity)
    @DeleteMapping("/{id}")
