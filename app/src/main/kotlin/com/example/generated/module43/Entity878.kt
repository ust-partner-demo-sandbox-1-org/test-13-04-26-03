...
interface Entity878Repository : org.springframework.data.jpa.repository.JpaRepository<Entity878, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity878>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity878>
}
...
class Service878(private val repo: Entity878Repository) {
    ...
    fun findActive(): List<Entity878> = repo.findByField4(true)
    fun search(query: String): List<Entity878> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity878>): Map<String, List<Entity878>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity878): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity878")
class Controller878(private val service: Service878) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity878) = service.save(entity)
    @DeleteMapping("/{id}")
