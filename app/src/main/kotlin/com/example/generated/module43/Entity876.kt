...
interface Entity876Repository : org.springframework.data.jpa.repository.JpaRepository<Entity876, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity876>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity876>
}
...
class Service876(private val repo: Entity876Repository) {
    ...
    fun findActive(): List<Entity876> = repo.findByField4(true)
    fun search(query: String): List<Entity876> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity876>): Map<String, List<Entity876>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity876): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity876")
class Controller876(private val service: Service876) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity876) = service.save(entity)
    @DeleteMapping("/{id}")
