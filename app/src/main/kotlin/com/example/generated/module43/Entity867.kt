...
interface Entity867Repository : org.springframework.data.jpa.repository.JpaRepository<Entity867, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity867>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity867>
}
...
class Service867(private val repo: Entity867Repository) {
    ...
    fun findActive(): List<Entity867> = repo.findByField4(true)
    fun search(query: String): List<Entity867> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity867>): Map<String, List<Entity867>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity867): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity867")
class Controller867(private val service: Service867) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity867) = service.save(entity)
    @DeleteMapping("/{id}")
