...
interface Entity870Repository : org.springframework.data.jpa.repository.JpaRepository<Entity870, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity870>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity870>
}
...
class Service870(private val repo: Entity870Repository) {
    ...
    fun findActive(): List<Entity870> = repo.findByField4(true)
    fun search(query: String): List<Entity870> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity870>): Map<String, List<Entity870>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity870): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity870")
class Controller870(private val service: Service870) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity870) = service.save(entity)
    @DeleteMapping("/{id}")
