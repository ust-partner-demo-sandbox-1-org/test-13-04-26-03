...
interface Entity861Repository : org.springframework.data.jpa.repository.JpaRepository<Entity861, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity861>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity861>
}
...
class Service861(private val repo: Entity861Repository) {
    ...
    fun findActive(): List<Entity861> = repo.findByField4(true)
    fun search(query: String): List<Entity861> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity861>): Map<String, List<Entity861>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity861): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity861")
class Controller861(private val service: Service861) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity861) = service.save(entity)
    @DeleteMapping("/{id}")
