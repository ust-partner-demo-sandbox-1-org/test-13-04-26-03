...
interface Entity877Repository : org.springframework.data.jpa.repository.JpaRepository<Entity877, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity877>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity877>
}
...
class Service877(private val repo: Entity877Repository) {
    ...
    fun findActive(): List<Entity877> = repo.findByField4(true)
    fun search(query: String): List<Entity877> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity877>): Map<String, List<Entity877>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity877): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity877")
class Controller877(private val service: Service877) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity877) = service.save(entity)
    @DeleteMapping("/{id}")
