...
interface Entity879Repository : org.springframework.data.jpa.repository.JpaRepository<Entity879, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity879>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity879>
}
...
class Service879(private val repo: Entity879Repository) {
    ...
    fun findActive(): List<Entity879> = repo.findByField4(true)
    fun search(query: String): List<Entity879> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity879>): Map<String, List<Entity879>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity879): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity879")
class Controller879(private val service: Service879) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity879) = service.save(entity)
    @DeleteMapping("/{id}")
