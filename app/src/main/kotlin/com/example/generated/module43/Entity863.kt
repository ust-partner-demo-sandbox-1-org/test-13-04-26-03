...
interface Entity863Repository : org.springframework.data.jpa.repository.JpaRepository<Entity863, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity863>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity863>
}
...
class Service863(private val repo: Entity863Repository) {
    ...
    fun findActive(): List<Entity863> = repo.findByField4(true)
    fun search(query: String): List<Entity863> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity863>): Map<String, List<Entity863>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity863): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity863")
class Controller863(private val service: Service863) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity863) = service.save(entity)
    @DeleteMapping("/{id}")
