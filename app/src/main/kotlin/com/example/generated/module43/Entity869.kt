...
interface Entity869Repository : org.springframework.data.jpa.repository.JpaRepository<Entity869, Long> {
    ...
    fun findByField4(active: Boolean): List<Entity869>
    fun findByField1ContainingIgnoreCase(search: String): List<Entity869>
}
...
class Service869(private val repo: Entity869Repository) {
    ...
    fun findActive(): List<Entity869> = repo.findByField4(true)
    fun search(query: String): List<Entity869> = repo.findByField1ContainingIgnoreCase(query)
    ...
    fun transform(items: List<Entity869>): Map<String, List<Entity869>> {
        return items.groupBy { it.field1 }
            .mapValues { (_, v) -> v.sortedByDescending { it.field8 } }
    }
    fun validate(entity: Entity869): Boolean {
        return entity.field1.isNotBlank() && entity.field3 >= 0 && entity.field6 > 0.0
    }
}

@RestController
@RequestMapping("/api/v1/entity869")
class Controller869(private val service: Service869) {
    @GetMapping
    ...
    @PostMapping
    fun create(@RequestBody entity: Entity869) = service.save(entity)
    @DeleteMapping("/{id}")
